...
impl Client {
    ...
    pub fn from_auth(base_url: impl Into<String>, auth: &NexalAuth) -> Result<Self> {
        let token = auth.get_token().map_err(anyhow::Error::from)?;
        let mut client = Self::new(base_url)?
            .with_user_agent(get_nexal_user_agent())
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("nexal-cli"));
        }
    ...
    pub async fn get_rate_limits_many(&self) -> Result<Vec<RateLimitSnapshot>> {
        let url = match self.path_style {
            PathStyle::NexalApi => format!("{}/api/codex/usage", self.base_url),
    ...
    pub async fn list_tasks(
        ...
        let url = match self.path_style {
            PathStyle::NexalApi => format!("{}/api/codex/tasks/list", self.base_url),
    ...
    pub async fn get_task_details_with_body(
        ...
        let url = match self.path_style {
            PathStyle::NexalApi => format!("{}/api/codex/tasks/{}", self.base_url, task_id),
    ...
    /// Fetch the managed requirements file from nexal-backend.
    ///
    /// `GET /api/codex/config/requirements` (Nexal API style) or
    ...
    pub async fn get_config_requirements_file(
        ...
        let url = match self.path_style {
            PathStyle::NexalApi => format!("{}/api/codex/config/requirements", self.base_url),
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::NexalApi => format!("{}/api/codex/tasks", self.base_url),
    ...
    fn usage_payload_maps_primary_and_additional_rate_limits() {
        let payload = RateLimitStatusPayload {
            ...
            additional_rate_limits: Some(Some(vec![crate::types::AdditionalRateLimitDetails {
                limit_name: "nexal_other".to_string(),
                metered_feature: "nexal_other".to_string(),
                rate_limit: Some(Some(Box::new(crate::types::RateLimitStatusDetails {
                    primary_window: Some(Some(Box::new(crate::types::RateLimitWindowSnapshot {
                        used_percent: 70,
        ...
        assert_eq!(snapshots[1].limit_id.as_deref(), Some("nexal_other"));
        assert_eq!(snapshots[1].limit_name.as_deref(), Some("nexal_other"));
        assert_eq!(
