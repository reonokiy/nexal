...
- The legacy rule matcher lives in `nexal-execpolicy-legacy`.
...
- From the Nexal CLI, run `codex execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:
...
```bash
cargo run -p nexal-execpolicy -- check --rules path/to/policy.rules git status
```
