...
pub(crate) fn set_chatgpt_auth(chat: &mut ChatWidget) {
    chat.auth_manager = nexal_core::test_support::auth_manager_from_auth(
...
async fn rate_limit_switch_prompt_skips_non_nexal_limit() {
    ...
    chat.auth_manager = nexal_core::test_support::auth_manager_from_auth(auth);
...
async fn terminal_title_can_render_app_name_item() {
    ...
    assert_eq!(chat.last_terminal_title, Some("nexal".to_string()));
}
        ...
        .or_else(|| {
            chat.config
                ...
                .find_map(|layer| match &layer.name {
                    ConfigLayerSource::Project { dot_nexal_folder } => {
                        dot_nexal_folder.as_path().parent().map(|path| {
                            path.file_name()
                                .map(|name| name.to_string_lossy().to_string())
