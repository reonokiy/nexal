use clap::Parser;
use clap::ValueHint;
use nexal_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `nexal resume` subcommand.
    // These are not exposed as user flags on the base `nexal` command.
    #[clap(skip)]
    ...
    /// top-level `nexal resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `nexal fork` subcommand.
    // These are not exposed as user flags on the base `nexal` command.
    #[clap(skip)]
    ...
    /// top-level `nexal fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    pub sandbox_mode: Option<nexal_utils_cli::SandboxModeCliArg>,
