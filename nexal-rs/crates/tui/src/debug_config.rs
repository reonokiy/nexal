...
use nexal_core::config_loader::ConfigLayerEntry;
...
use nexal_core::config_loader::ResidencyRequirement;
use nexal_core::config_loader::SandboxModeRequirement;
use nexal_core::config_loader::WebSearchModeRequirement;
    ...
    if let Some(proxy) = session_network_proxy {
        ...
        let all_proxy = session_all_proxy_url(
            ...
            config
                ...
                .network
                .as_ref()
                .is_some_and(nexal_core::config::NetworkProxySpec::socks_enabled),
        );
...
fn format_config_layer_source(source: &ConfigLayerSource) -> String {
    match source {
        ...
        ConfigLayerSource::Project { dot_nexal_folder } => {
            format!(
                "project ({}/config.toml)",
                dot_nexal_folder.as_path().display()
            )
        }
...
mod tests {
    ...
    use nexal_core::config_loader::ConstrainedWithSource;
    use nexal_core::config_loader::McpServerIdentity;
    use nexal_core::config_loader::McpServerRequirement;
    use nexal_core::config_loader::NetworkConstraints;
    use nexal_core::config_loader::RequirementSource;
    use nexal_core::config_loader::ResidencyRequirement;
    use nexal_core::config_loader::SandboxModeRequirement;
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_nexal_folder: project_folder,
                },
                empty_toml_table(),
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\OpenAI\\Nexal\\requirements.toml")
        } else {
