...
const PAID_TOOLTIP: &str = "*New* Try the **Nexal App** with 2x rate limits until *April 2nd*. Run 'nexal app' or visit https://chatgpt.com/nexal?app-landing-page=true";
const PAID_TOOLTIP_WINDOWS: &str = "*New* Try the **Nexal App**, now available on **Windows**, with 2x rate limits until *April 2nd*. Run 'nexal app' or visit https://chatgpt.com/nexal?app-landing-page=true";
...
const OTHER_TOOLTIP: &str = "*New* Build faster with the **Nexal App**. Run 'nexal app' or visit https://chatgpt.com/nexal?app-landing-page=true";
...
lazy_static! {
    static ref TOOLTIPS: Vec<&'static str> = RAW_TOOLTIPS
        ...
        .filter(|line| {
            ...
            if !IS_MACOS && !IS_WINDOWS && line.contains("nexal app") {
                return false;
            }
            true
        })
...
/// Pick a random tooltip to show to the user when starting Nexal.
...
pub(crate) mod announcement {
    ...
    pub(crate) fn parse_announcement_tip_toml(text: &str) -> Option<String> {
        ...
        for raw in announcements {
            ...
            if tip.version_matches(NEXAL_CLI_VERSION)
...
from_date = "2000-01-01"
    ...
    fn announcement_tip_toml_parse_comments() {
        let toml = r#"
# Example announcement tips for Nexal TUI.
...
[[announcements]]
content = "Welcome to Nexal! Check out the new onboarding flow."
