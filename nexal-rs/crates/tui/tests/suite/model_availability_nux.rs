...
async fn resume_startup_does_not_consume_model_availability_nux_count() -> Result<()> {
    ...
    let source_catalog_path = nexal_utils_cargo_bin::find_resource!("../core/models.json")?;
    ...
    let custom_catalog_path = nexal_home.path().join("catalog.json");
    std::fs::write(
        &custom_catalog_path,
...
"#
    ...
    std::fs::write(nexal_home.path().join("config.toml"), config_contents)?;

    let fixture_path =
        nexal_utils_cargo_bin::find_resource!("../core/tests/cli_responses_fixture.sse")?;
    let nexal = if let Ok(path) = nexal_utils_cargo_bin::cargo_bin("nexal") {
        path
    } else {
        let fallback = repo_root.join("nexal-rs/target/debug/nexal");
        if fallback.is_file() {
            fallback
        } else {
            eprintln!("skipping integration test because nexal binary is unavailable");
            return Ok(());
        }
    };

    let exec_output = std::process::Command::new(&nexal)
        .arg("exec")
        .arg("--skip-git-repo-check")
        .arg("-C")
        .arg(&repo_root)
        .arg("seed session for resume")
        .env("NEXAL_HOME", nexal_home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("NEXAL_RS_SSE_FIXTURE", fixture_path)
        .env("OPENAI_BASE_URL", "http://unused.local")
        .output()
        .context("failed to execute nexal exec")?;
    anyhow::ensure!(
    ...
    env.insert(
        ...
        nexal_home.path().display().to_string(),
    );
    ...
    let exit_code = match exit_code_result {
        ...
        Err(_) => {
            session.terminate();
            anyhow::bail!("timed out waiting for nexal resume to exit");
        }
    };
    ...
    anyhow::ensure!(
        ...
        "unexpected exit code from nexal resume: {exit_code}; output: {output_text}",
    );

    let config_contents = std::fs::read_to_string(nexal_home.path().join("config.toml"))?;
