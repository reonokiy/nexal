...
use nexal_protocol::protocol::HookRunSummary;
...
pub(crate) struct ConfiguredHandler {
    pub event_name: nexal_protocol::protocol::HookEventName,
...
impl ConfiguredHandler {
    ...
    fn event_name_label(&self) -> &'static str {
        match self.event_name {
            nexal_protocol::protocol::HookEventName::PreToolUse => "pre-tool-use",
            nexal_protocol::protocol::HookEventName::PostToolUse => "post-tool-use",
            nexal_protocol::protocol::HookEventName::SessionStart => "session-start",
            nexal_protocol::protocol::HookEventName::UserPromptSubmit => "user-prompt-submit",
    ...
    pub(crate) fn new(
        ...
        if cfg!(windows) {
            return Self {
                handlers: Vec::new(),
                warnings: vec![
                    "Disabled `nexal_hooks` for this session because `hooks.json` lifecycle hooks are not supported on Windows yet."
                        .to_string(),
                ],
                shell,
            };
