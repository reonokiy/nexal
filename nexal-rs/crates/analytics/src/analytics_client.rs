use nexal_git_utils::collect_git_info;
use nexal_git_utils::get_git_repo_root;
use nexal_login::AuthManager;
use nexal_login::default_client::create_client;
use nexal_login::default_client::originator;
...
enum TrackEventRequest {
    ...
    AppMentioned(NexalAppMentionedEventRequest),
    AppUsed(NexalAppUsedEventRequest),
    PluginUsed(NexalPluginUsedEventRequest),
    PluginInstalled(NexalPluginEventRequest),
    PluginUninstalled(NexalPluginEventRequest),
    PluginEnabled(NexalPluginEventRequest),
    PluginDisabled(NexalPluginEventRequest),
}
...
#[derive(Serialize)]
struct NexalAppMentionedEventRequest {
    event_type: &'static str,
    event_params: NexalAppMetadata,
}

#[derive(Serialize)]
struct NexalAppUsedEventRequest {
    event_type: &'static str,
    event_params: NexalAppMetadata,
}

#[derive(Serialize)]
struct NexalPluginMetadata {
...
#[derive(Serialize)]
struct NexalPluginUsedMetadata {
    #[serde(flatten)]
    plugin: NexalPluginMetadata,
    thread_id: Option<String>,
    turn_id: Option<String>,
    model_slug: Option<String>,
}
...
#[derive(Serialize)]
struct NexalPluginEventRequest {
    event_type: &'static str,
    event_params: NexalPluginMetadata,
}

#[derive(Serialize)]
struct NexalPluginUsedEventRequest {
    event_type: &'static str,
    event_params: NexalPluginUsedMetadata,
}
...
async fn send_track_app_mentioned(
    ...
    let events = mentions
        .into_iter()
        .map(|mention| {
            let event_params = nexal_app_metadata(&tracking, mention);
            TrackEventRequest::AppMentioned(NexalAppMentionedEventRequest {
                event_type: "nexal_app_mentioned",
                event_params,
            })
        })
        .collect::<Vec<_>>();
...
async fn send_track_app_used(auth_manager: &AuthManager, base_url: &str, job: TrackAppUsedJob) {
    ...
    let event_params = nexal_app_metadata(&tracking, app);
    let events = vec![TrackEventRequest::AppUsed(NexalAppUsedEventRequest {
        event_type: "nexal_app_used",
        event_params,
    })];
...
async fn send_track_plugin_used(
    ...
    let events = vec![TrackEventRequest::PluginUsed(NexalPluginUsedEventRequest {
        event_type: "nexal_plugin_used",
        event_params: nexal_plugin_used_metadata(&tracking, plugin),
    })];
...
async fn send_track_plugin_installed(
    ...
    send_track_plugin_management_event(auth_manager, base_url, job, "nexal_plugin_installed").await;
}
...
async fn send_track_plugin_uninstalled(
    ...
    send_track_plugin_management_event(auth_manager, base_url, job, "nexal_plugin_uninstalled")
        .await;
}
...
async fn send_track_plugin_enabled(
    ...
    send_track_plugin_management_event(auth_manager, base_url, job, "nexal_plugin_enabled").await;
}
...
async fn send_track_plugin_disabled(
    ...
    send_track_plugin_management_event(auth_manager, base_url, job, "nexal_plugin_disabled").await;
}
...
async fn send_track_plugin_management_event(
    ...
    let event_params = nexal_plugin_metadata(plugin);
    let event = NexalPluginEventRequest {
        event_type,
        event_params,
    };
    let events = vec![match event_type {
        "nexal_plugin_installed" => TrackEventRequest::PluginInstalled(event),
        "nexal_plugin_uninstalled" => TrackEventRequest::PluginUninstalled(event),
        "nexal_plugin_enabled" => TrackEventRequest::PluginEnabled(event),
        "nexal_plugin_disabled" => TrackEventRequest::PluginDisabled(event),
...
fn nexal_app_metadata(tracking: &TrackEventsContext, app: AppInvocation) -> NexalAppMetadata {
    NexalAppMetadata {
...
fn nexal_plugin_metadata(plugin: PluginTelemetryMetadata) -> NexalPluginMetadata {
...
fn nexal_plugin_used_metadata(
    tracking: &TrackEventsContext,
    plugin: PluginTelemetryMetadata,
) -> NexalPluginUsedMetadata {
    NexalPluginUsedMetadata {
        plugin: nexal_plugin_metadata(plugin),
        thread_id: Some(tracking.thread_id.clone()),
        turn_id: Some(tracking.turn_id.clone()),
        model_slug: Some(tracking.model_slug.clone()),
    }
