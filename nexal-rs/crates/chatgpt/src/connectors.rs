...
pub async fn list_cached_all_connectors(config: &Config) -> Option<Vec<AppInfo>> {
    ...
    if init_chatgpt_token_from_auth(&config.nexal_home, config.cli_auth_credentials_store_mode)
        .await
        .is_err()
    {
        return None;
    ...
    nexal_connectors::cached_all_connectors(&cache_key).map(|connectors| {
...
pub async fn list_all_connectors_with_options(
    ...
    init_chatgpt_token_from_auth(&config.nexal_home, config.cli_auth_credentials_store_mode)
        .await?;

    let token_data =
        get_chatgpt_token_data().ok_or_else(|| anyhow::anyhow!("ChatGPT token not available"))?;
    let cache_key = all_connectors_cache_key(config, &token_data);
    let connectors = nexal_connectors::list_all_connectors_with_options(
        cache_key,
...
fn plugin_apps_for_config(config: &Config) -> Vec<nexal_core::plugins::AppConnectorId> {
    PluginsManager::new(config.nexal_home.clone())
...
mod tests {
    use super::*;
    use nexal_core::connectors::connector_install_url;
    use nexal_core::plugins::AppConnectorId;
