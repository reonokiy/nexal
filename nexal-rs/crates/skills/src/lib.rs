...
const SYSTEM_SKILLS_MARKER_FILENAME: &str = ".nexal-system-skills.marker";
const SYSTEM_SKILLS_MARKER_SALT: &str = "v1";

/// Returns the on-disk cache location for embedded system skills.
///
/// This is typically located at `NEXAL_HOME/skills/.system`.
pub fn system_cache_root_dir(nexal_home: &Path) -> PathBuf {
    AbsolutePathBuf::try_from(nexal_home)
        .and_then(|nexal_home| system_cache_root_dir_abs(&nexal_home))
...
fn system_cache_root_dir_abs(nexal_home: &AbsolutePathBuf) -> std::io::Result<AbsolutePathBuf> {
    nexal_home
        .join(SKILLS_DIR_NAME)?
        .join(SYSTEM_SKILLS_DIR_NAME)
}

/// Installs embedded system skills into `NEXAL_HOME/skills/.system`.
///
...
/// install is skipped.
pub fn install_system_skills(nexal_home: &Path) -> Result<(), SystemSkillsError> {
    let nexal_home = AbsolutePathBuf::try_from(nexal_home)
        .map_err(|source| SystemSkillsError::io("normalize nexal home dir", source))?;
    let skills_root_dir = nexal_home
        .join(SKILLS_DIR_NAME)
    ...
    let dest_system = system_cache_root_dir_abs(&nexal_home)
