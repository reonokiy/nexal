...
use nexal_app_server_protocol::DynamicToolSpec;
...
use nexal_app_server_protocol::InitializeCapabilities;
...
use nexal_app_server_protocol::ReadOnlyAccess;
use nexal_app_server_protocol::RequestId;
use nexal_app_server_protocol::SandboxPolicy;
...
const OTEL_SERVICE_NAME: &str = "nexal-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $NEXAL_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Nexal app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Nexal", version, about = "Bootstrap Nexal app-server", long_about = None)]
struct Cli {
    ...
    nexal_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--nexal-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
...
enum CliCommand {
    ...
    ///
    /// Logs are written to:
    ///   `/tmp/nexal-app-server-test-client/`
    Serve {
    ...
    /// Send a user message through the Nexal app-server.
    SendMessage {
        /// User message to send to Nexal.
        user_message: String,
    },
    ...
    TestLogin,
    /// Fetch the current account rate limits from the Nexal app-server.
    GetAccountRateLimits,
    /// List the available models from the Nexal app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the Nexal app-server.
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let nexal_bin = nexal_bin.unwrap_or_else(|| PathBuf::from("codex"));
            serve(&nexal_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(nexal_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(nexal_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(nexal_bin, url, "thread-decrement-elicitation")?;
...
fn resolve_endpoint(nexal_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if nexal_bin.is_some() && url.is_some() {
        bail!("--nexal-bin and --url are mutually exclusive");
    }
    if let Some(nexal_bin) = nexal_bin {
        return Ok(Endpoint::SpawnNexal(nexal_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if nexal_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --nexal-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(nexal_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(nexal_bin);
        if let Some(nexal_bin_parent) = nexal_bin.parent() {
            let mut path = OsString::from(nexal_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", nexal_bin.display()))?;
...
fn serve(nexal_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/nexal-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,nexal_=trace {}",
        shell_quote(&nexal_bin.display().to_string())
    );
    for override_kv in config_overrides {
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnNexal(nexal_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut NexalClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = NexalClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = NexalClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (nexal_bin, url) {
        (Some(_), Some(_)) => bail!("--nexal-bin and --url are mutually exclusive"),
        (Some(nexal_bin), None) => {
            let server = BackgroundAppServer::spawn(&nexal_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve nexal-app-server-test-client binary path")?;
...
impl NexalClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnNexal(nexal_bin) => Self::spawn_stdio(nexal_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(nexal_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        let nexal_bin_display = nexal_bin.display();
        let mut cmd = Command::new(nexal_bin);
        if let Some(nexal_bin_parent) = nexal_bin.parent() {
            let mut path = OsString::from(nexal_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut nexal_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{nexal_bin_display}` app-server"))?;

        let stdin = nexal_app_server
            .stdin
            .take()
            .context("codex app-server stdin unavailable")?;
        let stdout = nexal_app_server
            .stdout
            .take()
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `nexal-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "nexal-toy-app-server".to_string(),
                    title: Some("Nexal Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: nexal_app_server_protocol::LoginAccountParams::Chatgpt,
        };
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = nexal_core::otel_init::build_provider(
            &config,
