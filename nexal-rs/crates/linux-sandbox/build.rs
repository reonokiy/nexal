...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=NEXAL_BWRAP_SOURCE_DIR");
...
fn try_build_vendored_bwrap() -> Result<(), String> {
    ...
    std::fs::write(
        &config_h,
        r#"#pragma once
#define PACKAGE_STRING "bubblewrap built at nexal build-time"
"#,
    )
...
///
/// Priority:
/// 1. `NEXAL_BWRAP_SOURCE_DIR` points at an existing bubblewrap checkout.
/// 2. The vendored bubblewrap tree under `nexal-rs/vendor/bubblewrap`.
fn resolve_bwrap_source_dir(manifest_dir: &Path) -> Result<PathBuf, String> {
    if let Ok(path) = env::var("NEXAL_BWRAP_SOURCE_DIR") {
        let src_dir = PathBuf::from(path);
        if src_dir.exists() {
            return Ok(src_dir);
        }
        return Err(format!(
            "NEXAL_BWRAP_SOURCE_DIR was set but does not exist: {}",
            src_dir.display()
        ));
    }
...
Set NEXAL_BWRAP_SOURCE_DIR to an existing checkout or vendor bubblewrap under nexal-rs/vendor.",
        vendor_dir.display()
    ))
}
