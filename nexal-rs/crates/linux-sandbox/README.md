...
- a `nexal-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Nexal CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `nexal-exec` CLI can check if its arg0 is `nexal-linux-sandbox` and, if so, execute as if it were `nexal-linux-sandbox`
...
this binary.
Nexal also surfaces a startup warning when `/usr/bin/bwrap` is missing so users
...
- If `/usr/bin/bwrap` is missing, the helper falls back to the vendored
  bubblewrap path.
- If `/usr/bin/bwrap` is missing, Nexal also surfaces a startup warning instead
