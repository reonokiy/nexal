//! Custom CA handling for Nexal outbound HTTP and websocket clients.
//!
//! Nexal constructs outbound reqwest clients and secure websocket connections in a few crates, but
...
//!
//! - read CA material from `NEXAL_CA_CERTIFICATE`, falling back to `SSL_CERT_FILE`
...
//!   because Nexal itself commonly runs spawned test processes under seatbelt, so this is not just
...
use nexal_utils_rustls_provider::ensure_rustls_crypto_provider;
...
pub const NEXAL_CA_CERT_ENV: &str = "NEXAL_CA_CERTIFICATE";
pub const SSL_CERT_FILE_ENV: &str = "SSL_CERT_FILE";
const CA_CERT_HINT: &str = "If you set NEXAL_CA_CERTIFICATE or SSL_CERT_FILE, ensure it points to a PEM file containing one or more CERTIFICATE blocks, or unset it to use system roots.";
...
/// Builds a reqwest client that honors Nexal custom CA environment variables.
///
/// Callers supply the baseline builder configuration they need, and this helper layers in custom
/// CA handling before finally constructing the client. `NEXAL_CA_CERTIFICATE` takes precedence
...
/// easy mistake to make when adding a new outbound Nexal HTTP path, and the resulting bug only
...
/// Builds a rustls client config when a Nexal custom CA bundle is configured.
///
/// This is the websocket-facing sibling of [`build_reqwest_client_with_custom_ca`]. When
/// `NEXAL_CA_CERTIFICATE` or `SSL_CERT_FILE` selects a CA bundle, the returned config starts from
...
fn maybe_build_rustls_client_config_with_env(
    ...
    // they would get from tungstenite's default rustls connector, then layer in the Nexal custom
...
fn build_reqwest_client_with_env(
    ...
    info!(
        nexal_ca_certificate_configured = false,
...
trait EnvSource {
    ...
    ///
    /// `NEXAL_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the Nexal-specific override.
    ...
    fn configured_ca_bundle(&self) -> Option<ConfiguredCaBundle> {
        self.non_empty_path(NEXAL_CA_CERT_ENV)
            .map(|path| ConfiguredCaBundle {
                source_env: NEXAL_CA_CERT_ENV,
                path,
            })
            .or_else(|| {
...
impl ConfiguredCaBundle {
    ...
    /// Loads every certificate block from a PEM file intended for Nexal CA overrides.
    ///
    /// This accepts a few common real-world variants so Nexal behaves like other CA-aware tooling:
...
impl NormalizedPem {
    ...
    ///
    /// Nexal only needs certificate DER bytes to seed `reqwest`'s root store, but operators may
...
mod tests {
    ...
    #[test]
    fn ca_path_prefers_nexal_env() {
        let env = map_env(&[
            (NEXAL_CA_CERT_ENV, "/tmp/nexal.pem"),
            (SSL_CERT_FILE_ENV, "/tmp/fallback.pem"),
        ]);

        assert_eq!(
            env.configured_ca_bundle().map(|bundle| bundle.path),
            Some(PathBuf::from("/tmp/nexal.pem"))
        );
    }
    ...
    fn rustls_config_uses_custom_ca_bundle_when_configured() {
        ...
        let env = map_env(&[(NEXAL_CA_CERT_ENV, cert_path.to_string_lossy().as_ref())]);
