...
use nexal_features::unstable_features_warning_event;
use nexal_hooks::HookEvent;
use nexal_hooks::HookEventAfterAgent;
use nexal_hooks::HookPayload;
use nexal_hooks::HookResult;
use nexal_hooks::Hooks;
use nexal_hooks::HooksConfig;
...
use nexal_protocol::approvals::NetworkPolicyAmendment;
use nexal_protocol::approvals::NetworkPolicyRuleAction;
...
use nexal_protocol::dynamic_tools::DynamicToolResponse;
use nexal_protocol::dynamic_tools::DynamicToolSpec;
...
use nexal_protocol::models::format_allow_prefixes;
...
use nexal_protocol::protocol::HasLegacyEvent;
use nexal_protocol::protocol::ItemCompletedEvent;
use nexal_protocol::protocol::ItemStartedEvent;
use nexal_protocol::protocol::RawResponseItemEvent;
...
use nexal_protocol::protocol::TurnContextNetworkItem;
...
use nexal_protocol::request_permissions::RequestPermissionsArgs;
use nexal_protocol::request_permissions::RequestPermissionsEvent;
use nexal_protocol::request_permissions::RequestPermissionsResponse;
use nexal_protocol::request_user_input::RequestUserInputArgs;
use nexal_protocol::request_user_input::RequestUserInputResponse;
use nexal_rmcp_client::ElicitationResponse;
use nexal_rmcp_client::OAuthCredentialsStoreMode;
use nexal_terminal_detection::user_agent;
use nexal_utils_output_truncation::TruncationPolicy;
use nexal_utils_stream_parser::AssistantTextChunk;
use nexal_utils_stream_parser::AssistantTextStreamParser;
use nexal_utils_stream_parser::ProposedPlanSegment;
use nexal_utils_stream_parser::extract_proposed_plan_text;
use nexal_utils_stream_parser::strip_citations;
...
use crate::nexal_thread::ThreadConfigSnapshot;
    ...
    fn to_error_event(&self) -> ErrorEvent {
        match self {
            Self::NoActiveTurn(_) => ErrorEvent {
                ...
                nexal_error_info: Some(NexalErrorInfo::BadRequest),
            },
...
use crate::mcp::with_nexal_apps_mcp;
use crate::mcp_connection_manager::McpConnectionManager;
use crate::mcp_connection_manager::nexal_apps_tools_cache_key;
use crate::mcp_connection_manager::filter_non_nexal_apps_mcp_tools_only;
use crate::memories;
...
use nexal_utils_readiness::Readiness;
use nexal_utils_readiness::ReadinessFlag;

/// The high-level interface to the Nexal system.
...
/// Wrapper returned by [`Nexal::spawn`] containing the spawned [`Nexal`],
/// the submission id for the initial `ConfigureSession` request and the
/// unique session id.
pub struct NexalSpawnOk {
    pub nexal: Nexal,
...
pub(crate) struct NexalSpawnArgs {
...
impl Nexal {
    ...
    async fn spawn_internal(args: NexalSpawnArgs) -> NexalResult<NexalSpawnOk> {
        let NexalSpawnArgs {
            mut config,
            auth_manager,
            models_manager,
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| NexalErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };
        ...
        let persisted_tools = if dynamic_tools.is_empty() {
            ...
            match thread_id {
                Some(thread_id) => {
                    ...
                    state_db::get_dynamic_tools(state_db_ctx.as_deref(), thread_id, "nexal_spawn")
                        .await
                }
                None => None,
            }
        ...
        // Generate a unique ID for the lifetime of this Nexal session.
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.nexal_home)
        })?;
    ...
    /// Use sparingly: prefer `submit()` so Nexal is responsible for generating
    ...
    pub async fn submit_with_id(&self, mut sub: Submission) -> NexalResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| NexalErr::InternalAgentDied)?;
        Ok(())
    }
    ...
    pub async fn shutdown_and_wait(&self) -> NexalResult<()> {
        ...
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(NexalErr::InternalAgentDied) => {}
...
pub(crate) struct TurnContext {
    ...
    pub(crate) nexal_self_exe: Option<PathBuf>,
    pub(crate) nexal_linux_sandbox_exe: Option<PathBuf>,
...
impl TurnContext {
    ...
    pub(crate) async fn with_model(&self, model: String, models_manager: &ModelsManager) -> Self {
        ...
        Self {
            ...
            nexal_self_exe: self.nexal_self_exe.clone(),
            nexal_linux_sandbox_exe: self.nexal_linux_sandbox_exe.clone(),
...
pub(crate) struct SessionConfiguration {
    ...
    cwd: AbsolutePathBuf,
    /// Directory containing all Nexal state for this session.
    nexal_home: PathBuf,
...
impl SessionConfiguration {
    pub(crate) fn nexal_home(&self) -> &PathBuf {
        &self.nexal_home
    }
...
impl Session {
    /// Builds the `x-nexal-beta-features` header value for this session.
    ///
    ...
    pub(crate) async fn nexal_home(&self) -> PathBuf {
        let state = self.state.lock().await;
        state.session_configuration.nexal_home().clone()
    }
    ...
    fn make_turn_context(
        ...
        TurnContext {
            ...
            nexal_self_exe: per_turn_config.nexal_self_exe.clone(),
            nexal_linux_sandbox_exe: per_turn_config.nexal_linux_sandbox_exe.clone(),
            tool_call_gate: Arc::new(ReadinessFlag::new()),
    ...
    async fn new(
        ...
        let config_path = config.nexal_home.join(CONFIG_TOML_FILE);
        ...
        let auth_env_telemetry = collect_auth_env_telemetry(
            &session_configuration.provider,
            auth_manager.nexal_api_key_env_enabled(),
        );
        ...
        let hooks = Hooks::new(HooksConfig {
            ...
            feature_enabled: config.features.enabled(Feature::NexalHooks),
        ...
        let (mcp_connection_manager, cancel_token) = McpConnectionManager::new(
            ...
            sandbox_state,
            config.nexal_home.clone(),
            nexal_apps_tools_cache_key(auth),
        ...
        let session_start_source = match &initial_history {
            InitialHistory::Resumed(_) => nexal_hooks::SessionStartSource::Resume,
            InitialHistory::New | InitialHistory::Forked(_) => {
                nexal_hooks::SessionStartSource::Startup
            }
        };
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Nexal performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&nexal_home, amendment)
            .await?;
    ...
    pub async fn request_mcp_server_elicitation(
        ...
        let id = match request_id {
            rmcp::model::NumberOrString::String(value) => {
                nexal_protocol::mcp::RequestId::String(value.to_string())
            }
    ...
    pub(crate) async fn record_model_warning(&self, message: impl Into<String>, ctx: &TurnContext) {
        self.services
            .session_telemetry
            .counter("nexal.model_warning", /*inc*/ 1, &[]);
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        let warning_message = format!(
            "Your account was flagged for potentially high-risk cyber activity and this request was routed to gpt-5.2 as a fallback. To regain access to gpt-5.3-nexal, apply for trusted access: {CYBER_VERIFY_URL} or learn more: {CYBER_SAFETY_URL}"
        );
    ...
    pub(crate) async fn build_initial_context(
        ...
        if turn_context.features.enabled(Feature::MemoryTool)
            ...
            && let Some(memory_prompt) =
                build_memory_tool_developer_instructions(&turn_context.config.nexal_home).await
        {
        ...
        if turn_context.features.enabled(Feature::NexalGitCommit)
    ...
    pub(crate) async fn notify_stream_error(
        ...
        turn_context: &TurnContext,
        message: impl Into<String>,
        nexal_error: NexalErr,
    ) {
        let additional_details = nexal_error.to_string();
        let nexal_error_info = NexalErrorInfo::ResponseStreamDisconnected {
            http_status_code: nexal_error.http_status_code_value(),
        };
        let event = EventMsg::StreamError(StreamErrorEvent {
            message: message.into(),
            nexal_error_info: Some(nexal_error_info),
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<nexal_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
    ...
    async fn refresh_mcp_servers_inner(
        ...
        let mcp_servers = with_nexal_apps_mcp(
            mcp_servers,
        ...
        let sandbox_state = SandboxState {
            ...
            nexal_linux_sandbox_exe: turn_context.nexal_linux_sandbox_exe.clone(),
...
mod handlers {
    ...
    use crate::nexal::SessionSettingsUpdate;
    use crate::nexal::SteerInputError;

    use crate::SkillError;
    use crate::nexal::spawn_review_thread;
    use crate::config::Config;
    ...
    use nexal_protocol::protocol::ReviewRequest;
    use nexal_protocol::protocol::RolloutItem;
    use nexal_protocol::protocol::SkillsListEntry;
    use nexal_protocol::protocol::ThreadNameUpdatedEvent;
    ...
    use nexal_protocol::config_types::Settings;
    use nexal_protocol::dynamic_tools::DynamicToolResponse;
    use nexal_protocol::mcp::RequestId as ProtocolRequestId;
    use nexal_protocol::user_input::UserInput;
    use nexal_rmcp_client::ElicitationAction;
    use nexal_rmcp_client::ElicitationResponse;
    use serde_json::Value;
    use std::path::PathBuf;
    use std::sync::Arc;
    ...
    pub async fn resolve_elicitation(
        ...
        decision: nexal_protocol::approvals::ElicitationAction,
        content: Option<Value>,
        meta: Option<Value>,
    ) {
        let action = match decision {
            nexal_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
            nexal_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
            nexal_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
        };
            ...
            let event = Event {
                ...
                msg: EventMsg::GetHistoryEntryResponse(
                    crate::protocol::GetHistoryEntryResponseEvent {
                        offset,
                        log_id,
                        entry: entry_opt.map(|e| nexal_protocol::message_history::HistoryEntry {
    ...
    pub async fn list_skills(
        ...
        let nexal_home = sess.nexal_home().await;
    ...
    pub async fn drop_memories(sess: &Arc<Session>, config: &Arc<Config>, sub_id: String) {
        ...
        let memory_root = crate::memories::memory_root(&config.nexal_home);
        ...
        if num_turns == 0 {
            sess.send_event_raw(Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    ...
                    nexal_error_info: Some(NexalErrorInfo::ThreadRollbackFailed),
                }),
            })
            .await;
            return;
    ...
    ///
    /// This appends the name to `NEXAL_HOME/sessions_index.jsonl` via `session_index::append_thread_name` for the
    ...
    pub async fn set_thread_name(sess: &Arc<Session>, sub_id: String, name: String) {
        ...
        let nexal_home = sess.nexal_home().await;
        if let Err(e) =
            session_index::append_thread_name(&nexal_home, sess.conversation_id, &name).await
        {
            let event = Event {
                id: sub_id,
                msg: EventMsg::Error(ErrorEvent {
    ...
    pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
        ...
        sess.services
            ...
            .await;
        sess.guardian_review_session.shutdown().await;
        info!("Shutting down Nexal instance");
...
async fn spawn_review_thread(
    ...
    let review_turn_context = TurnContext {
        ...
        final_output_json_schema: None,
        nexal_self_exe: parent_turn_context.nexal_self_exe.clone(),
        nexal_linux_sandbox_exe: parent_turn_context.nexal_linux_sandbox_exe.clone(),
        tool_call_gate: Arc::new(ReadinessFlag::new()),
...
pub(crate) async fn run_turn(
    ...
    // Although from the perspective of nexal.rs, TurnDiffTracker has the lifecycle of a Task which contains
        ...
        .await
        {
            Ok(sampling_request_output) => {
                ...
                if !needs_follow_up {
                    ...
                    .to_string();
                    let stop_request = nexal_hooks::StopRequest {
            ...
            Err(NexalErr::TurnAborted) => {
                // Aborted turn is reported via a different event.
                break;
            }
            Err(NexalErr::InvalidImageRequest()) => {
                let mut state = sess.state.lock().await;
                error_or_panic(
...
fn filter_nexal_apps_mcp_tools(
    ...
    mcp_tools
        .iter()
        .filter(|(_, tool)| {
            if tool.server_name != NEXAL_APPS_MCP_SERVER_NAME {
                return false;
            }
            let Some(connector_id) = nexal_apps_connector_id(tool) else {
                return false;
            };
            allowed.contains(connector_id) && connectors::nexal_app_tool_is_enabled(config, tool)
        })
        .map(|(name, tool)| (name.clone(), tool.clone()))
        .collect()
}

fn nexal_apps_connector_id(tool: &crate::mcp_connection_manager::ToolInfo) -> Option<&str> {
...
async fn run_sampling_request(
    ...
    cancellation_token: CancellationToken,
) -> NexalResult<SamplingRequestResult> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(NexalErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(NexalErr::ContextWindowExceeded);
            }
            Err(NexalErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, *rate_limits).await;
                }
                return Err(NexalErr::UsageLimitReached(e));
            }
            Err(err) => err,
        };
        ...
        if retries < max_retries {
            retries += 1;
            let delay = match &err {
                NexalErr::Stream(_, requested_delay) => {
...
pub(crate) async fn built_tools(
    ...
    let app_tools = connectors.as_ref().map(|connectors| {
        filter_nexal_apps_mcp_tools(&mcp_tools, connectors, &turn_context.config)
    });
    ...
    if let Some(connectors) = connectors.as_ref() {
        ...
        let mut selected_mcp_tools = filter_non_nexal_apps_mcp_tools_only(&mcp_tools);
        selected_mcp_tools.extend(filter_nexal_apps_mcp_tools(
            &mcp_tools,
...
fn agent_message_text(item: &nexal_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            nexal_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: nexal_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(nexal_protocol::items::AgentMessageItem {
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, NexalResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> NexalResult<()> {
...
async fn try_run_sampling_request(
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, NexalResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: NexalResult<SamplingRequestResult> = loop {
        ...
        {
            Ok(event) => event,
            Err(nexal_async_utils::CancelErr::Cancelled) => break Err(NexalErr::TurnAborted),
        };

        let event = match event {
            Some(res) => res?,
            None => {
                break Err(NexalErr::Stream(
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![nexal_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
    ...
    if cancellation_token.is_cancelled() {
        return Err(NexalErr::TurnAborted);
    }
