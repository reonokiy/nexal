...
"#,
    ...
    write_file(
        &repo_root.join(".nexal/config.toml"),
        r#"[[skills.config]]
  ...
  {"id":"2","name":"gmail","marketplace_name":"openai-curated","version":"1.0.0","enabled":false}
    ...
    let result = manager
        .sync_plugins_from_remote(
            &config,
            Some(&NexalAuth::create_dummy_chatgpt_auth_for_testing()),
            /*additive_only*/ false,
        )
        .await
        .unwrap();
...
#[tokio::test]
async fn featured_plugin_ids_for_config_defaults_query_param_to_nexal() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["nexal-plugin"]"#))
        .mount(&server)
        .await;
    ...
    assert_eq!(featured_plugin_ids, vec!["nexal-plugin".to_string()]);
}
...
fn load_plugins_ignores_project_config_files() {
    ...
    let outcome = load_plugins_from_layer_stack(
        &stack,
        &PluginStore::new(nexal_home.path().to_path_buf()),
        Some(Product::Nexal),
    );
