...
pub(crate) fn write_curated_plugin_sha(nexal_home: &Path) {
    write_curated_plugin_sha_with(nexal_home, TEST_CURATED_PLUGIN_SHA);
}

pub(crate) fn write_curated_plugin_sha_with(nexal_home: &Path, sha: &str) {
    write_file(&nexal_home.join(".tmp/plugins.sha"), &format!("{sha}\n"));
}
