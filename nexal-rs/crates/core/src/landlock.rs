...
use nexal_sandboxing::landlock::allow_network_for_proxy;
...
/// (nexal-linux-sandbox), which defaults to bubblewrap for filesystem
    ...
    P: AsRef<Path>,
{
    ...
    let nexal_linux_sandbox_exe = nexal_linux_sandbox_exe.as_ref();
    ...
    let arg0 = if nexal_linux_sandbox_exe
        .file_name()
        ...
        == Some(NEXAL_LINUX_SANDBOX_ARG0)
    {
        ...
        nexal_linux_sandbox_exe.to_string_lossy().into_owned()
    } else {
        NEXAL_LINUX_SANDBOX_ARG0.to_string()
    };
    spawn_child_async(SpawnChildRequest {
        program: nexal_linux_sandbox_exe.to_path_buf(),
        args,
        arg0: Some(&arg0),
        cwd: command_cwd,
        network_sandbox_policy,
