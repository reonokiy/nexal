...
use crate::nexal::make_session_and_context_with_dynamic_tools_and_rx;
use crate::protocol::AskForApproval;
use crate::protocol::EventMsg;
use crate::protocol::SandboxPolicy;
use crate::turn_diff_tracker::TurnDiffTracker;
use nexal_features::Feature;
use nexal_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
...
fn set_danger_full_access(turn: &mut crate::nexal::TurnContext) {
    turn.sandbox_policy
...
fn validate_emitted_image_url_rejects_non_data_scheme() {
    assert_eq!(
        ...
        Err("nexal.emitImage only accepts data URLs".to_string())
    );
}
...
async fn can_run_js_repl_runtime_tests() -> bool {
    ...
    // the nexal-linux-sandbox arg0 dispatch path, which is exercised in
...
async fn js_repl_uncaught_exception_returns_exec_error_and_recovers() -> anyhow::Result<()> {
    ...
    let (session, turn) = crate::nexal::make_session_and_context().await;
    let session = Arc::new(session);
    let turn = Arc::new(turn);
    let tracker = Arc::new(tokio::sync::Mutex::new(TurnDiffTracker::default()));
    let manager = turn.js_repl.manager().await?;
...
void nexal.tool("shell_command", {{ command: `sleep 0.35; printf js_repl_unawaited_done > "${{marker}}"` }});
...
const imagePath = path.join(nexal.tmpDir, "js-repl-view-image.png");
const png = Buffer.from(
  "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGP4z8DwHwAFAAH/iZk9HQAAAABJRU5ErkJggg==",
  "base64"
);
await fs.writeFile(imagePath, png);
const out = await nexal.tool("view_image", { path: imagePath });
...
const fs = await import("node:fs/promises");
const path = await import("node:path");
const imagePath = path.join(nexal.tmpDir, "js-repl-view-image-explicit.png");
const png = Buffer.from(
  "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGP4z8DwHwAFAAH/iZk9HQAAAABJRU5ErkJggg==",
  "base64"
);
...
await nexal.emitImage({ bytes: png, mimeType: "image/png" });
"#;
...
const savedEmitImage = nexal.emitImage;
...
async fn js_repl_unawaited_emit_image_errors_fail_cell() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let manager = turn.js_repl.manager().await?;
    let code = r#"
void nexal.emitImage({ bytes: new Uint8Array(), mimeType: "image/png" });
console.log("cell-complete");
"#;
...
async fn js_repl_caught_emit_image_error_does_not_fail_cell() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let code = r#"
try {
  await nexal.emitImage({ bytes: new Uint8Array(), mimeType: "image/png" });
} catch (error) {
...
await nexal.emitImage({ bytes: png });
"#;
...
async fn js_repl_emit_image_rejects_non_data_url() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let manager = turn.js_repl.manager().await?;
    let code = r#"
await nexal.emitImage("https://example.com/image.png");
"#;
...
async fn js_repl_emit_image_accepts_case_insensitive_data_url() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let manager = turn.js_repl.manager().await?;
    let code = r#"
await nexal.emitImage("DATA:image/png;base64,AAA");
"#;
...
await nexal.emitImage({ bytes: png, mimeType: "image/png", detail: "ultra" });
"#;
...
await nexal.emitImage({ bytes: png, mimeType: "image/png", detail: null });
"#;
...
async fn js_repl_emit_image_rejects_mixed_content() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let manager = turn.js_repl.manager().await?;
    let code = r#"
const out = await nexal.tool("inline_image", {});
await nexal.emitImage(out);
"#;
...
async fn js_repl_dynamic_tool_response_preserves_js_line_separator_text() -> anyhow::Result<()> {
    ...
    for (tool_name, description, expected_text, literal) in [
        ...
        *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
        ...
        let code = format!(
            r#"
const out = await nexal.tool("{tool_name}", {{}});
...
async fn js_repl_can_call_hidden_dynamic_tools() -> anyhow::Result<()> {
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let manager = turn.js_repl.manager().await?;
    let code = r#"
const out = await nexal.tool("hidden_dynamic_tool", { city: "Paris" });
...
async fn js_repl_prefers_env_node_module_dirs_over_config() -> anyhow::Result<()> {
    ...
    turn.shell_environment_policy.r#set.insert(
        "NEXAL_JS_REPL_NODE_MODULE_DIRS".to_string(),
...
async fn js_repl_resolves_from_first_config_dir() -> anyhow::Result<()> {
    ...
    turn.shell_environment_policy
        .r#set
        .remove("NEXAL_JS_REPL_NODE_MODULE_DIRS");
    turn.cwd = cwd_dir.abs();
    turn.js_repl = Arc::new(JsReplHandle::with_node_path(
        turn.config.js_repl_node_path.clone(),
        vec![
...
async fn js_repl_imported_local_files_can_access_repl_globals() -> anyhow::Result<()> {
    ...
    let cwd_dir = tempdir()?;
    let expected_home_dir = serde_json::to_string("/tmp/nexal-home")?;
    write_js_repl_test_module(
        cwd_dir.path(),
        "globals.js",
        &format!(
            "const expectedHomeDir = {expected_home_dir};\nconsole.log(`tmp:${{nexal.tmpDir === tmpDir}}`);\nconsole.log(`cwd:${{typeof nexal.cwd}}:${{nexal.cwd.length > 0}}`);\nconsole.log(`home:${{nexal.homeDir === expectedHomeDir}}`);\nconsole.log(`tool:${{typeof nexal.tool}}`);\nconsole.log(\"local-file-console-ok\");\n"
        ),
    )?;
