...
pub(crate) fn collab_spawn_error(err: NexalErr) -> FunctionCallError {
    match err {
        NexalErr::UnsupportedOperation(message) if message == "thread manager dropped" => {
            FunctionCallError::RespondToModel("collab manager unavailable".to_string())
        }
        NexalErr::UnsupportedOperation(message) => FunctionCallError::RespondToModel(message),
        err => FunctionCallError::RespondToModel(format!("collab spawn failed: {err}")),
    }
}

pub(crate) fn collab_agent_error(agent_id: ThreadId, err: NexalErr) -> FunctionCallError {
    match err {
        NexalErr::ThreadNotFound(id) => {
...
pub(crate) fn apply_spawn_agent_runtime_overrides(
    ...
    config.nexal_linux_sandbox_exe = turn.nexal_linux_sandbox_exe.clone();
...
fn find_spawn_agent_model_name(
    available_models: &[nexal_protocol::openai_models::ModelPreset],
    requested_model: &str,
