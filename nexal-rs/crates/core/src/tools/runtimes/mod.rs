...
fn build_override_exports(explicit_env_overrides: &HashMap<String, String>) -> (String, String) {
    ...
    let captures = keys
        ...
        .map(|(idx, key)| {
            format!(
                "__NEXAL_SNAPSHOT_OVERRIDE_SET_{idx}=\"${{{key}+x}}\"\n__NEXAL_SNAPSHOT_OVERRIDE_{idx}=\"${{{key}-}}\""
            )
        })
        .collect::<Vec<_>>()
        .join("\n");
    let restores = keys
        ...
        .map(|(idx, key)| {
            format!(
                "if [ -n \"${{__NEXAL_SNAPSHOT_OVERRIDE_SET_{idx}}}\" ]; then export {key}=\"${{__NEXAL_SNAPSHOT_OVERRIDE_{idx}}}\"; else unset {key}; fi"
            )
        })
        .collect::<Vec<_>>()
        .join("\n");
