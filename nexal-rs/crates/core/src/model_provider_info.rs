//! Registry of model providers supported by Nexal.
//!
//! Providers can be defined in two places:
//!   1. Built-in defaults compiled into the binary so Nexal works out-of-the-box.
...
pub enum WireApi {
    ...
    /// Supported by most OpenAI-compatible providers (Moonshot, DeepSeek, etc.).
    ChatCompletions,
}
        ...
        match &self.env_key {
            Some(env_key) => {
                let api_key = std::env::var(env_key)
                    ...
                    .ok_or_else(|| {
                        crate::error::NexalErr::EnvVar(EnvVarError {
                            var: env_key.clone(),
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    ...
    let default_nexal_oss_base_url = format!(
        "http://localhost:{nexal_oss_port}/v1",
        nexal_oss_port = std::env::var("NEXAL_OSS_PORT")
            .ok()
    ...
    let nexal_oss_base_url = std::env::var("NEXAL_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or(default_nexal_oss_base_url);
    create_oss_provider_with_base_url(&nexal_oss_base_url, wire_api)
}
