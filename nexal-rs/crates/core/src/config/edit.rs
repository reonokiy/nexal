...
pub async fn apply(
    ...
    task::spawn_blocking(move || apply_blocking(&nexal_home, profile.as_deref(), &edits))
        .await
...
impl ConfigEditsBuilder {
    pub fn new(nexal_home: &Path) -> Self {
        Self {
    ...
    pub fn apply_blocking(self) -> anyhow::Result<()> {
        apply_blocking(&self.nexal_home, self.profile.as_deref(), &self.edits)
    }
