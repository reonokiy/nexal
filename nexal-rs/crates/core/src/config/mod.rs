...
use nexal_features::FeatureConfigSource;
use nexal_features::FeatureOverrides;
use nexal_features::Features;
use nexal_features::FeaturesToml;
use nexal_git_utils::resolve_root_git_project_for_trust;
...
use nexal_utils_absolute_path::AbsolutePathBufGuard;
...
use crate::config::permissions::get_readable_roots_required_for_nexal_runtime;
...
pub mod service;
pub mod types;
pub use nexal_config_loader::Constrained;
pub use nexal_config_loader::ConstraintError;
pub use nexal_config_loader::ConstraintResult;
pub use nexal_network_proxy::NetworkProxyAuditMetadata;
...
pub use nexal_git_utils::GhostSnapshotConfig;
...
fn system_bwrap_warning_for_path(system_bwrap_path: &Path) -> Option<String> {
    if !system_bwrap_path.is_file() {
        return Some(format!(
            "Nexal could not find system bubblewrap at {}. Please install bubblewrap with your package manager. Nexal will use the vendored bubblewrap in the meantime.",
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<PathBuf> {
    let raw = std::env::var(nexal_rollout_state::SQLITE_HOME_ENV).ok()?;
...
pub(crate) fn test_config() -> Config {
    let nexal_home = tempfile::tempdir().expect("create temp dir");
...
pub struct Config {
    ...
    ///
    /// - `None`: use default attribution (`Nexal <noreply@openai.com>`)
    /// - `Some("")` or whitespace-only: disable commit attribution
    /// - `Some("...")`: use the provided attribution text verbatim
    pub commit_attribution: Option<String>,

    /// Optional external notifier command. When set, Nexal will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Nexal
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.nexal/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Nexal"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Nexal '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Nexal home directory.
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Nexal unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/nexal/blob/main/nexal-rs/rmcp-client/src/oauth.rs#L2
    /// file: NEXAL_HOME/.credentials.json
    ///       This file will be readable to Nexal and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, Nexal will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all Nexal state (defaults to `~/.nexal` but can be
    /// overridden by the `NEXAL_HOME` environment variable).
    pub nexal_home: PathBuf,

    /// Directory where Nexal stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where Nexal writes log files (defaults to `$NEXAL_HOME/log`).
    pub log_dir: PathBuf,

    /// Settings that govern if and what will be written to `~/.nexal/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current Nexal executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub nexal_self_exe: Option<PathBuf>,

    /// Path to the `nexal-linux-sandbox` executable. This must be set if
    /// [`nexal_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `nexal-linux-sandbox`.
    pub nexal_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `nexal-execve-wrapper` executable used for shell
    ...
    /// When `true`, checks for Nexal updates on startup and surfaces update prompts.
    /// Set to `false` only if your Nexal updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Nexal product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Nexal product surfaces.
    /// Defaults to `true`.
...
impl ConfigBuilder {
    pub fn nexal_home(mut self, nexal_home: PathBuf) -> Self {
        self.nexal_home = Some(nexal_home);
        self
    }
    ...
    pub async fn build(self) -> std::io::Result<Config> {
        ...
        let nexal_home = nexal_home.map_or_else(find_nexal_home, std::io::Result::Ok)?;
...
impl Config {
    ...
    pub fn load_default_with_cli_overrides(
        ...
        let config_toml = deserialize_config_toml_with_base(merged, &nexal_home)?;
    ...
    /// ignores user settings. For example, the `nexal exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `nexal_self_exe`, `nexal_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_overrides(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, nexal_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .nexal/ folders.
...
/// Patch `NEXAL_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(nexal_home: &Path, provider: &str) -> std::io::Result<()> {
...
/// Base config deserialized from ~/.nexal/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by Nexal will likely degrade model performance.
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          https://github.com/openai/nexal/blob/main/nexal-rs/rmcp-client/src/oauth.rs#L2
    /// file: Use a file in the Nexal home directory.
    ...
    /// Directory where Nexal stores the SQLite state DB.
    /// Defaults to `$NEXAL_SQLITE_HOME` when set. Otherwise uses `$NEXAL_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where Nexal writes log files, for example `nexal-tui.log`.
    /// Defaults to `$NEXAL_HOME/log`.
    ...
    /// directories for `.nexal` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub use nexal_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
impl Config {
    ...
    fn load_from_base_config_with_overrides(
        ...
        Self::load_config_with_layer_stack(cfg, overrides, nexal_home, config_layer_stack)
    }
    ...
    pub(crate) fn load_config_with_layer_stack(
        ...
        let user_instructions = Self::load_instructions(Some(&nexal_home));
        ...
        let sqlite_home = cfg
            ...
            .unwrap_or_else(|| nexal_home.to_path_buf());
        ...
        let helper_readable_roots = get_readable_roots_required_for_nexal_runtime(
            &nexal_home,
            zsh_path.as_ref(),
    ...
    fn load_instructions(nexal_dir: Option<&Path>) -> Option<String> {
        let base = nexal_dir?;
...
/// Returns the path to the Nexal configuration directory, which can be
/// specified by the `NEXAL_HOME` environment variable. If not set, defaults to
/// `~/.nexal`.
///
/// - If `NEXAL_HOME` is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If `NEXAL_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_nexal_home() -> std::io::Result<PathBuf> {
    nexal_utils_home_dir::find_nexal_home()
}

/// Returns the path to the folder where Nexal logs are stored. Does not verify
