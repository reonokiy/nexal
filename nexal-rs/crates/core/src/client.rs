...
//!
//! `ModelClient` is intended to live for the lifetime of a Nexal session and holds the stable
...
//! per-turn state such as the `x-nexal-turn-state` token used for sticky routing.
//!
...
use nexal_api::CompactClient as ApiCompactClient;
use nexal_api::CompactionInput as ApiCompactionInput;
use nexal_api::MemoriesClient as ApiMemoriesClient;
use nexal_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use nexal_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use nexal_api::RawMemory as ApiRawMemory;
use nexal_api::RequestTelemetry;
use nexal_api::ReqwestTransport;
use nexal_api::ResponseCreateWsRequest;
use nexal_api::ResponsesApiRequest;
use nexal_api::ResponsesClient as ApiResponsesClient;
use nexal_api::ResponsesOptions as ApiResponsesOptions;
use nexal_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use nexal_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use nexal_api::SseTelemetry;
use nexal_api::TransportError;
use nexal_api::WebsocketTelemetry;
use nexal_api::build_conversation_headers;
use nexal_api::common::Reasoning;
use nexal_api::common::ResponsesWsRequest;
use nexal_api::create_text_param_for_request;
...
use nexal_protocol::config_types::Verbosity as VerbosityConfig;
...
use crate::flags::NEXAL_RS_SSE_FIXTURE;
...
struct CurrentClientSetup {
    ...
    api_provider: nexal_api::Provider,
...
///
/// This holds configuration and state that should be shared across turns within a Nexal session
...
/// - The `x-nexal-turn-state` sticky-routing token, which must be replayed for all requests within
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each Nexal turn. Reusing it across turns would replay
...
pub struct ModelClientSession {
    ...
    /// on turn start via the `x-nexal-turn-state` response header. Once set, this value
    /// should be sent back to the server in the `x-nexal-turn-state` request header for
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a Nexal session. Per-turn values
    ...
    pub fn new(
        ...
        let nexal_api_key_env_enabled = auth_manager
            .as_ref()
            .is_some_and(|manager| manager.nexal_api_key_env_enabled());
        let auth_env_telemetry = collect_auth_env_telemetry(&provider, nexal_api_key_env_enabled);
        Self {
    ...
    pub(crate) fn force_http_fallback(
        ...
        if activated {
            ...
            session_telemetry.counter(
                "nexal.transport.fallback_to_http",
                /*inc*/ 1,
    ...
    pub async fn compact_conversation_history(
        ...
        let request_telemetry = Self::build_request_telemetry(
            ...
            AuthRequestTelemetryContext::new(
                client_setup.auth.as_ref().map(NexalAuth::auth_mode),
    ...
    async fn current_client_setup(&self) -> Result<CurrentClientSetup> {
        ...
        let api_provider = self
            .state
            .provider
            .to_api_provider(auth.as_ref().map(NexalAuth::auth_mode))?;
...
impl ModelClientSession {
    ...
    async fn stream_responses_api(
        ...
        if let Some(path) = &*NEXAL_RS_SSE_FIXTURE {
            warn!(path, "Streaming from fixture");
            let stream = nexal_api::stream_from_fixture(
                path,
...
fn build_ws_client_metadata(turn_metadata_header: Option<&str>) -> Option<HashMap<String, String>> {
    ...
    client_metadata.insert(X_NEXAL_TURN_METADATA_HEADER.to_string(), turn_metadata);
    Some(client_metadata)
}

/// Builds the extra headers attached to Responses API requests.
///
/// These headers implement Nexal-specific conventions:
///
/// - `x-nexal-beta-features`: comma-separated beta feature keys enabled for the session.
/// - `x-nexal-turn-state`: sticky routing token captured earlier in the turn.
/// - `x-nexal-turn-metadata`: optional per-turn metadata for observability.
fn build_responses_headers(
    ...
    {
        headers.insert("x-nexal-beta-features", header_value);
    }
    if let Some(turn_state) = turn_state
        && let Some(state) = turn_state.get()
        && let Ok(header_value) = HeaderValue::from_str(state)
    {
        headers.insert(X_NEXAL_TURN_STATE_HEADER, header_value);
    }
    if let Some(header_value) = turn_metadata_header {
        headers.insert(X_NEXAL_TURN_METADATA_HEADER, header_value.clone());
    }
    headers
}
...
/// the mapped `NexalErr` is returned to the caller.
        ...
        && recovery.has_next()
    {
        ...
        return match recovery.next().await {
            ...
            Err(RefreshTokenError::Permanent(failed)) => {
                ...
                Err(NexalErr::RefreshTokenFailed(failed))
            }
