...
impl ShellSnapshot {
    ...
    fn spawn_snapshot_task(
        ...
        tokio::spawn(
            async move {
                let timer = session_telemetry.start_timer("nexal.shell_snapshot.duration_ms", &[]);
                ...
                session_telemetry.counter("nexal.shell_snapshot", /*inc*/ 1, &counter_tags);
    ...
    async fn try_new(
        ...
        tokio::spawn(async move {
            if let Err(err) = cleanup_stale_snapshots(&nexal_home, cleanup_session_id).await {
