...
/// `nexal_feedback::NexalFeedback::metadata_layer()` is installed, these fields are captured and
...
///
/// ```rust
/// nexal_core::feedback_tags!(model = "gpt-5", cached = true);
/// nexal_core::feedback_tags!(provider = provider_id, request_id = request_id);
/// ```
#[macro_export]
...
pub(crate) fn emit_feedback_request_tags_with_auth_env(
    ...
    feedback_tags!(
        ...
        auth_env_nexal_api_key_present = auth_env.nexal_api_key_env_present,
        auth_env_nexal_api_key_enabled = auth_env.nexal_api_key_env_enabled,
