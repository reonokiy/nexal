...
#[tokio::test]
async fn save_image_generation_result_saves_base64_to_png_in_nexal_home() {
    let nexal_home = tempfile::tempdir().expect("create nexal home");
    let expected_path =
        image_generation_artifact_path(nexal_home.path(), "session-1", "ig_save_base64");
    let _ = std::fs::remove_file(&expected_path);

    let saved_path =
        save_image_generation_result(nexal_home.path(), "session-1", "ig_save_base64", "Zm9v")
            .await
...
async fn save_image_generation_result_rejects_data_url_payload() {
    ...
    let err = save_image_generation_result(nexal_home.path(), "session-1", "ig_456", result)
        .await
        .expect_err("data url payload should error");
    assert!(matches!(err, NexalErr::InvalidRequest(_)));
}
...
async fn save_image_generation_result_overwrites_existing_file() {
    let nexal_home = tempfile::tempdir().expect("create nexal home");
    let existing_path =
        image_generation_artifact_path(nexal_home.path(), "session-1", "ig_overwrite");
    ...
    let saved_path =
        save_image_generation_result(nexal_home.path(), "session-1", "ig_overwrite", "Zm9v")
            .await
            .expect("image should be saved");
...
#[tokio::test]
async fn save_image_generation_result_sanitizes_call_id_for_nexal_home_output_path() {
    let nexal_home = tempfile::tempdir().expect("create nexal home");
    let expected_path = image_generation_artifact_path(nexal_home.path(), "session-1", "../ig/..");
    let _ = std::fs::remove_file(&expected_path);

    let saved_path =
        save_image_generation_result(nexal_home.path(), "session-1", "../ig/..", "Zm9v")
            .await
            .expect("image should be saved");
...
async fn save_image_generation_result_rejects_non_standard_base64() {
    let nexal_home = tempfile::tempdir().expect("create nexal home");
    let err = save_image_generation_result(nexal_home.path(), "session-1", "ig_urlsafe", "_-8")
        .await
