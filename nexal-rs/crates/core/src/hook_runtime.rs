...
use nexal_hooks::PostToolUseOutcome;
use nexal_hooks::PostToolUseRequest;
use nexal_hooks::PreToolUseOutcome;
use nexal_hooks::PreToolUseRequest;
use nexal_hooks::SessionStartOutcome;
use nexal_hooks::UserPromptSubmitOutcome;
use nexal_hooks::UserPromptSubmitRequest;
...
use nexal_protocol::protocol::HookCompletedEvent;
...
pub(crate) async fn run_pending_session_start_hooks(
    ...
    let request = nexal_hooks::SessionStartRequest {
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    nexal_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
