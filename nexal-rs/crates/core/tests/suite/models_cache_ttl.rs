...
const REMOTE_MODEL: &str = "nexal-test-ttl";
const VERSIONED_MODEL: &str = "nexal-test-versioned";
const MISSING_VERSION_MODEL: &str = "nexal-test-missing-version";
const DIFFERENT_VERSION_MODEL: &str = "nexal-test-different-version";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let nexal = Arc::clone(&test.nexal);
    ...
    let cache_path = config.nexal_home.join(CACHE_FILE);
    ...
    let _ = wait_for_event(&nexal, |event| matches!(event, EventMsg::TurnComplete(_))).await;
    ...
    builder = builder
        .with_pre_build_hook(move |home| {
            let cache = ModelsCache {
                fetched_at: Utc::now(),
                etag: None,
                client_version: Some(nexal_core::models_manager::client_version_to_whole()),
...
async fn refreshes_when_cache_version_differs() -> Result<()> {
    ...
    builder = builder
        .with_pre_build_hook(move |home| {
            let client_version = nexal_core::models_manager::client_version_to_whole();
            let cache = ModelsCache {
                fetched_at: Utc::now(),
                etag: None,
