...
use nexal_core::config_loader::NetworkConstraints;
use nexal_core::config_loader::NetworkRequirementsToml;
use nexal_core::config_loader::RequirementSource;
use nexal_core::config_loader::Sourced;
use nexal_core::sandboxing::SandboxPermissions;
use nexal_features::Feature;
use nexal_protocol::approvals::NetworkApprovalProtocol;
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestNexal) -> (PathBuf, String) {
        match self {
...
impl Expectation {
    fn verify(&self, test: &TestNexal, result: &CommandResult) -> Result<()> {
        match self {
...
async fn expect_exec_approval(
    ...
    let event = wait_for_event(&test.nexal, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_approval(test: &TestNexal) {
    let event = wait_for_event(&test.nexal, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestNexal) {
...
async fn wait_for_spawned_thread(test: &TestNexal) -> Result<Arc<NexalThread>> {
...
fn scenarios() -> Vec<ScenarioSpec> {
    ...
    vec![
        ...
        ScenarioSpec {
            ...
            sandbox_permissions: SandboxPermissions::UseDefault,
            features: vec![],
            model_override: Some("gpt-5.1-nexal"),
            outcome: Outcome::Auto,
            expectation: Expectation::PatchApplied {
                target: TargetPath::Workspace("apply_patch_function.txt"),
                content: "function-apply-patch",
...
async fn run_scenario(scenario: &ScenarioSpec) -> Result<()> {
    ...
    let mut builder = test_nexal().with_model(model).with_config(move |config| {
...
async fn invalid_requested_prefix_rule_falls_back_for_compound_command() -> Result<()> {
    ...
    let command =
        "touch /tmp/nexal-fallback-rule-test.txt && echo hello > /tmp/nexal-fallback-rule-test.txt";
...
"#,
    ...
    let sandbox_policy_for_config = sandbox_policy.clone();
    let mut builder = test_nexal().with_home(home).with_config(move |config| {
        config.permissions.approval_policy = Constrained::allow_any(approval_policy);
        config.permissions.sandbox_policy = Constrained::allow_any(sandbox_policy_for_config);
        let layers = config
            .config_layer_stack
    ...
    let fetch_command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://nexal-network-test.invalid', timeout=30).read().decode(errors='replace'))""#
        .to_string();
