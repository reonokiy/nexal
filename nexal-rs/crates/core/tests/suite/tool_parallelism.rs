...
async fn run_turn(test: &TestNexal, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestNexal, prompt: &str) -> anyhow::Result<Duration> {
...
async fn build_nexal_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestNexal> {
    let mut builder = test_nexal().with_model("test-gpt-5.1-nexal");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_nexal_with_test_tool(&server).await?;
