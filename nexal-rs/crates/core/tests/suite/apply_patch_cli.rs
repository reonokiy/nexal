...
pub async fn apply_patch_harness() -> Result<TestNexalHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestNexalBuilder) -> TestNexalBuilder,
) -> Result<TestNexalHarness> {
    let builder = configure(test_nexal()).with_config(|config| {
    ...
    Box::pin(TestNexalHarness::with_builder(builder)).await
}
...
async fn apply_patch_cli_uses_nexal_self_exe_with_linux_sandbox_helper_alias() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let harness = apply_patch_harness().await?;
    let nexal_linux_sandbox_exe = harness
        .test()
        .config
        .nexal_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include nexal-linux-sandbox helper");
    assert_eq!(
        nexal_linux_sandbox_exe
            .file_name()
            .and_then(|name| name.to_str()),
        Some(NEXAL_LINUX_SANDBOX_ARG0),
    );
