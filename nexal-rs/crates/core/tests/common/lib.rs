...
use anyhow::ensure;
use nexal_arg0::Arg0PathEntryGuard;
use nexal_utils_cargo_bin::CargoBinError;
use ctor::ctor;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    nexal_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    nexal_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(nexal_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = nexal_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("nexal-rs"));
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\nexal\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.nexal`.
pub async fn load_default_config_for_test(nexal_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        nexal_linux_sandbox_exe: Some(
            find_nexal_linux_sandbox_exe().expect("should find binary for nexal-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_nexal_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    nexal_utils_cargo_bin::cargo_bin("nexal-linux-sandbox")
}
...
pub async fn wait_for_event<F>(
    nexal: &NexalThread,
    predicate: F,
) -> nexal_protocol::protocol::EventMsg
where
    F: FnMut(&nexal_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(nexal, predicate, Duration::from_secs(1)).await
}

pub async fn wait_for_event_match<T, F>(nexal: &NexalThread, matcher: F) -> T
where
    F: Fn(&nexal_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(nexal, |ev| matcher(ev).is_some()).await;
    matcher(&ev).unwrap()
}
    ...
    F: FnMut(&nexal_protocol::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(10)), nexal.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    nexal_core::spawn::NEXAL_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    nexal_core::spawn::NEXAL_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

const REMOTE_ENV_ENV_VAR: &str = "NEXAL_TEST_REMOTE_ENV";
...
pub fn format_with_current_shell(command: &str) -> Vec<String> {
    nexal_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    nexal_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    nexal_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
#[macro_export]
macro_rules! nexal_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_nexal_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("nexal-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
