...
pub enum Command {
    /// Submit a new Nexal Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Nexal Cloud task.
    Status(StatusCommand),
    /// List Nexal Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Nexal Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Nexal Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Nexal Cloud.
    ...
    /// Git branch to run in Nexal Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Nexal Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Nexal Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Nexal Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
