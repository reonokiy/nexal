...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    let expected_nexal_home = AbsolutePathBuf::try_from(nexal_home.path().canonicalize()?)?;
    create_config_toml(nexal_home.path(), &server.uri(), "never")?;
    let mut mcp = McpProcess::new(nexal_home.path()).await?;

    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "nexal_vscode".to_string(),
    ...
    let InitializeResponse {
        user_agent,
        nexal_home: response_nexal_home,
        platform_family,
        platform_os,
    } = to_response::<InitializeResponse>(response)?;

    assert!(user_agent.starts_with("nexal_vscode/"));
    assert_eq!(response_nexal_home, expected_nexal_home);
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        nexal_home.path(),
        &[(
            "NEXAL_INTERNAL_ORIGINATOR_OVERRIDE",
            Some("nexal_originator_via_env_var"),
        )],
    )
    .await?;
    ...
    assert!(user_agent.starts_with("nexal_originator_via_env_var/"));
    assert_eq!(response_nexal_home, expected_nexal_home);
    assert_eq!(platform_family, std::env::consts::FAMILY);
    assert_eq!(platform_os, std::env::consts::OS);
    Ok(())
...
async fn initialize_rejects_invalid_client_name() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        nexal_home.path(),
        &[("NEXAL_INTERNAL_ORIGINATOR_OVERRIDE", None)],
    )
    .await?;
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let nexal_home = TempDir::new()?;
    let notify_file = nexal_home.path().join("notify.json");
    let notify_capture = cargo_bin("nexal-app-server-test-notify-capture")?;
...
fn create_config_toml(
    ...
    create_config_toml_with_extra(nexal_home, server_uri, approval_policy, "")
}
