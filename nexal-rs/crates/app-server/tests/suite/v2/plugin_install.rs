    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: AbsolutePathBuf::try_from(
                nexal_home.path().join("missing-marketplace.json"),
            )?,
...
async fn plugin_install_returns_invalid_request_for_disallowed_product_plugin() -> Result<()> {
  ...
  "plugins": [
    ...
    let mut mcp =
        McpProcess::new_with_args(nexal_home.path(), &["--session-source", "atlas"]).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn plugin_install_force_remote_sync_enables_remote_plugin_before_local_install() -> Result<()>
{
    ...
    let nexal_home = TempDir::new()?;
    write_plugin_remote_sync_config(nexal_home.path(), &format!("{}/backend-api/", server.uri()))?;
    write_chatgpt_auth(
        nexal_home.path(),
    ...
    assert!(
        nexal_home
            .path()
            .join("plugins/cache/debug/sample-plugin/local/.nexal-plugin/plugin.json")
            .is_file()
    );
...
async fn plugin_install_tracks_analytics_event() -> Result<()> {
    ...
    let nexal_home = TempDir::new()?;
    write_analytics_config(nexal_home.path(), &analytics_server.uri())?;
    write_chatgpt_auth(
        nexal_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    let payload = timeout(DEFAULT_TIMEOUT, async {
        loop {
            ...
            if let Some(request) = requests.iter().find(|request| {
                request.method == "POST" && request.url.path() == "/nexal/analytics-events/events"
            }) {
...
async fn plugin_install_returns_apps_needing_auth() -> Result<()> {
    ...
    let nexal_home = TempDir::new()?;
    write_connectors_config(nexal_home.path(), &server_url)?;
    write_chatgpt_auth(
        nexal_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn start_apps_server(
    ...
    let router = Router::new()
        ...
        .with_state(state)
        .nest_service("/api/nexal/apps", mcp_service);
...
fn write_connectors_config(nexal_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
...
fn write_analytics_config(nexal_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        nexal_home.join("config.toml"),
