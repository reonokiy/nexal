...
use nexal_app_server_protocol::AppConfig;
use nexal_app_server_protocol::AppToolApproval;
use nexal_app_server_protocol::AppsConfig;
use nexal_app_server_protocol::AskForApproval;
use nexal_app_server_protocol::ConfigBatchWriteParams;
use nexal_app_server_protocol::ConfigEdit;
...
use nexal_app_server_protocol::ToolsV2;
use nexal_app_server_protocol::WriteStatus;
use nexal_core::config::set_project_trust_level;
use nexal_protocol::config_types::TrustLevel;
use nexal_protocol::config_types::WebSearchContextSize;
use nexal_protocol::config_types::WebSearchLocation;
...
fn write_config(nexal_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    let nexal_home_path = nexal_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(nexal_home_path.join("config.toml"))?;
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let nexal_home = TempDir::new()?;
    write_config(&nexal_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    set_project_trust_level(nexal_home.path(), workspace.path(), TrustLevel::Trusted)?;
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_nexal_folder: project_config
        }
    );
...
"#,
    ...
    let mut mcp = McpProcess::new_with_env(
        nexal_home.path(),
        &[(
            "NEXAL_APP_SERVER_MANAGED_CONFIG_PATH",
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let nexal_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path = AbsolutePathBuf::resolve_path_against_base("config.toml", nexal_home)?;
...
"#,
    ...
    let write_id = mcp
        .send_config_value_write_request(ConfigValueWriteParams {
            file_path: Some(nexal_home.path().join("config.toml").display().to_string()),
            key_path: "model".to_string(),
            value: json!("gpt-new"),
            merge_strategy: MergeStrategy::Replace,
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let nexal_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(nexal_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
fn assert_layers_user_then_optional_system(
    layers: &[nexal_app_server_protocol::ConfigLayer],
