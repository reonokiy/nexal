...
use nexal_backend_client as backend;
use nexal_backend_client::CodeTaskDetailsResponseExt;
use nexal_git_utils::ApplyGitRequest;
use nexal_git_utils::apply_git_patch;
...
mod api {
    ...
    impl<'a> Tasks<'a> {
        ...
        pub(crate) async fn create(
            ...
            if let Ok(diff) = std::env::var("NEXAL_STARTING_DIFF")
                && !diff.is_empty()
            {
                input_items.push(serde_json::json!({
