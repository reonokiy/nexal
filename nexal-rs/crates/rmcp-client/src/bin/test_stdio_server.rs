...
const MEMO_URI: &str = "memo://nexal/example-note";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Nexal TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Nexal TUI):
    /// - Build this binary: `cargo build -p nexal-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `nexal mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Nexal TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://nexal/{slug}".to_string(),
            name: "nexal-memo".to_string(),
            title: Some("Nexal Memo".to_string()),
            description: Some(
                "Template for memo://nexal/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
            icons: None,
        };
...
/// Scenarios for `image_scenario`, intended to exercise Nexal TUI handling of MCP image outputs.
///
