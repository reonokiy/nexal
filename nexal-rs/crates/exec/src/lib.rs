...
use nexal_app_server_client::InProcessServerEvent;
...
use nexal_app_server_protocol::Thread as AppServerThread;
...
use nexal_cloud_requirements::cloud_requirements_loader_for_storage;
use nexal_core::LMSTUDIO_OSS_PROVIDER_ID;
use nexal_core::OLLAMA_OSS_PROVIDER_ID;
...
use nexal_core::config::load_config_as_toml_with_cli_overrides;
use nexal_core::config::resolve_oss_provider;
use nexal_core::config_loader::ConfigLoadError;
use nexal_core::config_loader::LoaderOverrides;
use nexal_core::config_loader::format_config_error_with_source;
use nexal_core::format_exec_policy_error_with_source;
...
use crate::event_processor::NexalStatus;
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("nexal_exec".to_string()) {
        tracing::warn!(?err, "Failed to set nexal exec originator override {err:?}");
    }
    ...
    #[allow(clippy::print_stderr)]
    let nexal_home = match find_nexal_home() {
        Ok(nexal_home) => nexal_home,
        Err(err) => {
            eprintln!("Error finding nexal home: {err}");
            std::process::exit(1);
        }
    };
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        client_name: "nexal-exec".to_string(),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = nexal_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what Nexal
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode && let Some(message) = nexal_core::config::system_bwrap_warning() {
        event_processor.process_warning(message);
    }

    info!("Nexal initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(mut notification) => {
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        nexal_app_server_protocol::TurnStatus::Failed
                            | nexal_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
...
fn sandbox_mode_from_policy(
    ...
    match sandbox_policy {
        ...
        nexal_protocol::protocol::SandboxPolicy::ReadOnly { .. } => {
            Some(nexal_app_server_protocol::SandboxMode::ReadOnly)
        }
        nexal_protocol::protocol::SandboxPolicy::WorkspaceWrite { .. } => {
            Some(nexal_app_server_protocol::SandboxMode::WorkspaceWrite)
        }
        nexal_protocol::protocol::SandboxPolicy::ExternalSandbox { .. } => None,
    }
}
...
fn session_configured_from_thread_response(
    ...
    let session_id = nexal_protocol::ThreadId::from_string(thread_id)
...
mod tests {
    ...
    fn test_tracing_subscriber() -> impl tracing::Subscriber + Send + Sync {
        ...
        let tracer = provider.tracer("nexal-exec-tests");
    ...
    fn exec_root_span_can_be_parented_from_trace_context() {
        ...
        let parent = nexal_protocol::protocol::W3cTraceContext {
    ...
    async fn resume_lookup_model_providers_filters_only_last_lookup() {
        let nexal_home = tempdir().expect("create temp nexal home");
    ...
    fn turn_items_for_thread_returns_matching_turn_items() {
        let thread = AppServerThread {
            ...
            source: nexal_app_server_protocol::SessionSource::Exec,
            agent_nickname: None,
            agent_role: None,
            git_info: None,
            name: None,
            turns: vec![
                nexal_app_server_protocol::Turn {
                    id: "turn-1".to_string(),
                    ...
                    status: nexal_app_server_protocol::TurnStatus::Completed,
                    error: None,
                },
                nexal_app_server_protocol::Turn {
                    id: "turn-2".to_string(),
    ...
    async fn thread_start_params_include_review_policy_when_review_policy_is_manual_only() {
        ...
        assert_eq!(
            ...
            Some(nexal_app_server_protocol::ApprovalsReviewer::User)
        );
    }
    ...
    async fn thread_start_params_include_review_policy_when_auto_review_is_enabled() {
        ...
        let cwd = tempdir().expect("create temp cwd");
        std::fs::write(
            nexal_home.path().join("config.toml"),
        ...
        assert_eq!(
            params.approvals_reviewer,
            Some(nexal_app_server_protocol::ApprovalsReviewer::GuardianSubagent)
        );
    }
    ...
    fn session_configured_from_thread_response_uses_review_policy_from_response() {
        let response = ThreadStartResponse {
            thread: nexal_app_server_protocol::Thread {
                ...
                source: nexal_app_server_protocol::SessionSource::Cli,
                agent_nickname: None,
                agent_role: None,
                git_info: None,
            ...
            service_tier: None,
            cwd: PathBuf::from("/tmp"),
            approval_policy: nexal_app_server_protocol::AskForApproval::OnRequest,
            approvals_reviewer: nexal_app_server_protocol::ApprovalsReviewer::GuardianSubagent,
            sandbox: nexal_app_server_protocol::SandboxPolicy::WorkspaceWrite {
                writable_roots: vec![],
                read_only_access: nexal_app_server_protocol::ReadOnlyAccess::FullAccess,
