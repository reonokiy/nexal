...
**IMPORTANT:** `nexal-responses-api-proxy` is designed to be run by a privileged user with access to `OPENAI_API_KEY` so that an unprivileged user cannot inspect or tamper with the process. Though if `--http-shutdown` is specified, an unprivileged user _can_ make a `GET` request to `/shutdown` to shutdown the server, as an unprivileged user could not send `SIGTERM` to kill the process.

A privileged user (i.e., `root` or a user with `sudo`) who has access to `OPENAI_API_KEY` would run the following to start the server, as `nexal-responses-api-proxy` reads the auth token from `stdin`:

```shell
printenv OPENAI_API_KEY | env -u OPENAI_API_KEY nexal-responses-api-proxy --http-shutdown --server-info /tmp/server-info.json
```

A non-privileged user would then run Nexal as follows, specifying the `model_provider` dynamically:
...
- Reads the API key from `stdin`. All callers should pipe the key in (for example, `printenv OPENAI_API_KEY | nexal-responses-api-proxy`).
...
```
nexal-responses-api-proxy [--port <PORT>] [--server-info <FILE>] [--http-shutdown] [--upstream-url <URL>]
```
...
- Authentication is fixed to `Authorization: Bearer <key>` to match the Nexal CLI expectations.

For Azure, for example (ensure your deployment accepts `Authorization: Bearer <key>`):

```shell
printenv AZURE_OPENAI_API_KEY | env -u AZURE_OPENAI_API_KEY nexal-responses-api-proxy \
  --http-shutdown \
...
- We leverage [`nexal_process_hardening`](https://github.com/openai/codex/blob/main/nexal-rs/process-hardening/README.md) so `nexal-responses-api-proxy` is run with standard process-hardening techniques.
