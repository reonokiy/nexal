...
#[cfg(test)]
use nexal_protocol::items::ContextCompactionItem;
#[cfg(test)]
use nexal_protocol::items::ImageGenerationItem;
#[cfg(test)]
        ...
        ServerNotification::Error(notification) => Some((
            ...
            vec![Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    ...
                    nexal_error_info: notification
                        .error
                        .nexal_error_info
                        .and_then(app_server_nexal_error_info_to_core),
                }),
            }],
        )),
...
fn token_usage_from_app_server(
    value: nexal_app_server_protocol::TokenUsageBreakdown,
) -> TokenUsage {
    TokenUsage {
...
fn command_execution_completed_event(turn_id: &str, item: &ThreadItem) -> Option<Vec<Event>> {
    ...
    let status = match status {
        nexal_app_server_protocol::CommandExecutionStatus::InProgress => return Some(Vec::new()),
        nexal_app_server_protocol::CommandExecutionStatus::Completed => {
            ExecCommandStatus::Completed
        }
        nexal_app_server_protocol::CommandExecutionStatus::Failed => ExecCommandStatus::Failed,
        nexal_app_server_protocol::CommandExecutionStatus::Declined => ExecCommandStatus::Declined,
    };
...
) -> Option<nexal_protocol::models::WebSearchAction> {
    match action {
        nexal_app_server_protocol::WebSearchAction::Search { query, queries } => {
            Some(nexal_protocol::models::WebSearchAction::Search { query, queries })
        }
        nexal_app_server_protocol::WebSearchAction::OpenPage { url } => {
            Some(nexal_protocol::models::WebSearchAction::OpenPage { url })
        }
        nexal_app_server_protocol::WebSearchAction::FindInPage { url, pattern } => {
            Some(nexal_protocol::models::WebSearchAction::FindInPage { url, pattern })
        }
        nexal_app_server_protocol::WebSearchAction::Other => {
            Some(nexal_protocol::models::WebSearchAction::Other)
        }
    }
}

#[cfg(test)]
fn app_server_nexal_error_info_to_core(
    value: nexal_app_server_protocol::NexalErrorInfo,
) -> Option<nexal_protocol::protocol::NexalErrorInfo> {
...
mod tests {
    ...
    use nexal_app_server_protocol::NexalErrorInfo;
    use nexal_app_server_protocol::CommandAction;
    ...
    fn bridges_thread_snapshot_turns_for_resume_restore() {
        ...
        assert_eq!(
            error.nexal_error_info,
            Some(nexal_protocol::protocol::NexalErrorInfo::Other)
        );
            ...
            &Turn {
                ...
                items: vec![
                    ...
                    ThreadItem::WebSearch {
                        ...
                        action: Some(nexal_app_server_protocol::WebSearchAction::Other),
                    },
                    ThreadItem::ImageGeneration {
