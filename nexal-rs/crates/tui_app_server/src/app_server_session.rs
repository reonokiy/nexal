...
use nexal_app_server_protocol::ReviewDelivery;
...
use nexal_protocol::protocol::SessionNetworkProxyRuntime;
...
pub(crate) struct ThreadSessionState {
    ...
    pub(crate) service_tier: Option<nexal_protocol::config_types::ServiceTier>,
    pub(crate) approval_policy: AskForApproval,
    pub(crate) approvals_reviewer: nexal_protocol::config_types::ApprovalsReviewer,
    pub(crate) sandbox_policy: SandboxPolicy,
    pub(crate) cwd: PathBuf,
    pub(crate) reasoning_effort: Option<nexal_protocol::openai_models::ReasoningEffort>,
...
impl AppServerSession {
    ...
    pub(crate) async fn turn_start(
        &mut self,
        thread_id: ThreadId,
        items: Vec<nexal_protocol::user_input::UserInput>,
        cwd: PathBuf,
        approval_policy: AskForApproval,
        approvals_reviewer: nexal_protocol::config_types::ApprovalsReviewer,
        sandbox_policy: SandboxPolicy,
        model: String,
        effort: Option<nexal_protocol::openai_models::ReasoningEffort>,
        summary: Option<nexal_protocol::config_types::ReasoningSummary>,
        service_tier: Option<Option<nexal_protocol::config_types::ServiceTier>>,
        collaboration_mode: Option<nexal_protocol::config_types::CollaborationMode>,
        personality: Option<nexal_protocol::config_types::Personality>,
...
fn approvals_reviewer_override_from_config(
    config: &Config,
) -> Option<nexal_app_server_protocol::ApprovalsReviewer> {
...
) -> Option<nexal_app_server_protocol::SandboxMode> {
    match policy {
        SandboxPolicy::DangerFullAccess => {
            Some(nexal_app_server_protocol::SandboxMode::DangerFullAccess)
        }
        SandboxPolicy::ReadOnly { .. } => Some(nexal_app_server_protocol::SandboxMode::ReadOnly),
        SandboxPolicy::WorkspaceWrite { .. } => {
            Some(nexal_app_server_protocol::SandboxMode::WorkspaceWrite)
        }
...
) -> nexal_app_server_protocol::ReviewTarget {
    match target {
        CoreReviewTarget::UncommittedChanges => {
            nexal_app_server_protocol::ReviewTarget::UncommittedChanges
        }
        CoreReviewTarget::BaseBranch { branch } => {
            nexal_app_server_protocol::ReviewTarget::BaseBranch { branch }
        }
        CoreReviewTarget::Commit { sha, title } => {
            nexal_app_server_protocol::ReviewTarget::Commit { sha, title }
        }
        CoreReviewTarget::Custom { instructions } => {
            nexal_app_server_protocol::ReviewTarget::Custom { instructions }
        }
    }
}
...
async fn thread_session_state_from_thread_response(
    ...
    service_tier: Option<nexal_protocol::config_types::ServiceTier>,
    approval_policy: AskForApproval,
    approvals_reviewer: nexal_protocol::config_types::ApprovalsReviewer,
    sandbox_policy: SandboxPolicy,
    cwd: PathBuf,
    reasoning_effort: Option<nexal_protocol::openai_models::ReasoningEffort>,
    config: &Config,
) -> Result<ThreadSessionState, String> {
...
fn app_server_rate_limit_window_to_core(
    window: nexal_app_server_protocol::RateLimitWindow,
) -> RateLimitWindow {
    RateLimitWindow {
...
fn app_server_credits_snapshot_to_core(
    snapshot: nexal_app_server_protocol::CreditsSnapshot,
) -> CreditsSnapshot {
    CreditsSnapshot {
...
mod tests {
    ...
    async fn build_config(temp_dir: &TempDir) -> Config {
        ConfigBuilder::default()
            .nexal_home(temp_dir.path().to_path_buf())
            .build()
            .await
    ...
    async fn resume_response_restores_turns_from_thread_items() {
        ...
        let response = ThreadResumeResponse {
            thread: nexal_app_server_protocol::Thread {
                ...
                source: nexal_protocol::protocol::SessionSource::Cli.into(),
                agent_nickname: None,
                agent_role: None,
                git_info: None,
                name: None,
                turns: vec![Turn {
                    id: "turn-1".to_string(),
                    items: vec![
                        nexal_app_server_protocol::ThreadItem::UserMessage {
                            id: "user-1".to_string(),
                            content: vec![nexal_app_server_protocol::UserInput::Text {
                                text: "hello from history".to_string(),
                                text_elements: Vec::new(),
                            }],
                        },
                        nexal_app_server_protocol::ThreadItem::AgentMessage {
            ...
            service_tier: None,
            cwd: PathBuf::from("/tmp/project"),
            approval_policy: nexal_protocol::protocol::AskForApproval::Never.into(),
            approvals_reviewer: nexal_app_server_protocol::ApprovalsReviewer::User,
            sandbox: nexal_protocol::protocol::SandboxPolicy::new_read_only_policy().into(),
    ...
    async fn session_configured_populates_history_metadata() {
        ...
        let session = thread_session_state_from_thread_response(
            ...
            nexal_protocol::config_types::ApprovalsReviewer::User,
