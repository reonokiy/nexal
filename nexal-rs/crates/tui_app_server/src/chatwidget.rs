//! The main Nexal TUI chat surface.
//!
...
use nexal_app_server_protocol::NexalErrorInfo as AppServerNexalErrorInfo;
use nexal_app_server_protocol::CollabAgentState as AppServerCollabAgentState;
use nexal_app_server_protocol::CollabAgentStatus as AppServerCollabAgentStatus;
use nexal_app_server_protocol::CollabAgentTool;
use nexal_app_server_protocol::CollabAgentToolCallStatus;
use nexal_app_server_protocol::CommandExecutionRequestApprovalParams;
use nexal_app_server_protocol::ConfigLayerSource;
use nexal_app_server_protocol::ErrorNotification;
use nexal_app_server_protocol::FileChangeRequestApprovalParams;
...
use nexal_app_server_protocol::ThreadItem;
use nexal_app_server_protocol::ThreadTokenUsage;
use nexal_app_server_protocol::ToolRequestUserInputParams;
use nexal_app_server_protocol::Turn;
...
use nexal_core::config::types::ApprovalsReviewer;
use nexal_core::config::types::Notifications;
use nexal_core::config::types::WindowsSandboxModeToml;
use nexal_core::config_loader::ConfigLayerStackOrdering;
use nexal_core::find_thread_name_by_id;
use nexal_core::plugins::PluginsManager;
use nexal_core::project_doc::DEFAULT_PROJECT_DOC_FILENAME;
...
use nexal_git_utils::current_branch_name;
...
use nexal_protocol::items::AgentMessageContent;
use nexal_protocol::items::AgentMessageItem;
use nexal_protocol::models::MessagePhase;
use nexal_protocol::models::local_image_label_text;
use nexal_protocol::parse_command::ParsedCommand;
use nexal_protocol::plan_tool::PlanItemArg as UpdatePlanItemArg;
use nexal_protocol::plan_tool::StepStatus as UpdatePlanItemStatus;
#[cfg(test)]
use nexal_protocol::protocol::AgentMessageDeltaEvent;
#[cfg(test)]
use nexal_protocol::protocol::AgentMessageEvent;
#[cfg(test)]
use nexal_protocol::protocol::AgentReasoningDeltaEvent;
#[cfg(test)]
use nexal_protocol::protocol::AgentReasoningEvent;
#[cfg(test)]
use nexal_protocol::protocol::AgentReasoningRawContentDeltaEvent;
#[cfg(test)]
use nexal_protocol::protocol::AgentReasoningRawContentEvent;
use nexal_protocol::protocol::AgentStatus;
use nexal_protocol::protocol::ApplyPatchApprovalRequestEvent;
#[cfg(test)]
use nexal_protocol::protocol::BackgroundEventEvent;
#[cfg(test)]
use nexal_protocol::protocol::NexalErrorInfo as CoreNexalErrorInfo;
use nexal_protocol::protocol::CollabAgentRef;
#[cfg(test)]
use nexal_protocol::protocol::CollabAgentSpawnBeginEvent;
use nexal_protocol::protocol::CollabAgentStatusEntry;
use nexal_protocol::protocol::CreditsSnapshot;
use nexal_protocol::protocol::DeprecationNoticeEvent;
#[cfg(test)]
...
use nexal_protocol::protocol::ExecApprovalRequestEvent;
use nexal_protocol::protocol::ExecCommandBeginEvent;
use nexal_protocol::protocol::ExecCommandEndEvent;
use nexal_protocol::protocol::ExecCommandOutputDeltaEvent;
use nexal_protocol::protocol::ExecCommandSource;
#[cfg(test)]
use nexal_protocol::protocol::ExitedReviewModeEvent;
use nexal_protocol::protocol::GuardianAssessmentEvent;
use nexal_protocol::protocol::GuardianAssessmentStatus;
use nexal_protocol::protocol::ImageGenerationBeginEvent;
use nexal_protocol::protocol::ImageGenerationEndEvent;
use nexal_protocol::protocol::ListSkillsResponseEvent;
#[cfg(test)]
use nexal_protocol::protocol::McpListToolsResponseEvent;
#[cfg(test)]
use nexal_protocol::protocol::McpStartupCompleteEvent;
use nexal_protocol::protocol::McpStartupStatus;
#[cfg(test)]
use nexal_protocol::protocol::McpStartupUpdateEvent;
use nexal_protocol::protocol::McpToolCallBeginEvent;
use nexal_protocol::protocol::McpToolCallEndEvent;
use nexal_protocol::protocol::Op;
use nexal_protocol::protocol::PatchApplyBeginEvent;
use nexal_protocol::protocol::RateLimitSnapshot;
use nexal_protocol::protocol::ReviewRequest;
use nexal_protocol::protocol::ReviewTarget;
use nexal_protocol::protocol::SkillMetadata as ProtocolSkillMetadata;
#[cfg(test)]
use nexal_protocol::protocol::StreamErrorEvent;
use nexal_protocol::protocol::TerminalInteractionEvent;
use nexal_protocol::protocol::TokenUsage;
use nexal_protocol::protocol::TokenUsageInfo;
use nexal_protocol::protocol::TurnAbortReason;
#[cfg(test)]
use nexal_protocol::protocol::TurnCompleteEvent;
#[cfg(test)]
use nexal_protocol::protocol::TurnDiffEvent;
#[cfg(test)]
use nexal_protocol::protocol::UndoCompletedEvent;
#[cfg(test)]
use nexal_protocol::protocol::UndoStartedEvent;
use nexal_protocol::protocol::UserMessageEvent;
use nexal_protocol::protocol::ViewImageToolCallEvent;
#[cfg(test)]
use nexal_protocol::protocol::WarningEvent;
use nexal_protocol::protocol::WebSearchBeginEvent;
use nexal_protocol::protocol::WebSearchEndEvent;
use nexal_protocol::request_permissions::RequestPermissionsEvent;
use nexal_protocol::request_user_input::RequestUserInputEvent;
use nexal_protocol::request_user_input::RequestUserInputQuestionOption;
use nexal_protocol::user_input::TextElement;
use nexal_protocol::user_input::UserInput;
use nexal_terminal_detection::Multiplexer;
use nexal_terminal_detection::TerminalInfo;
...
use nexal_utils_sleep_inhibitor::SleepInhibitor;
...
use nexal_protocol::plan_tool::StepStatus;
use nexal_protocol::plan_tool::UpdatePlanArgs;
...
use nexal_utils_approval_presets::builtin_approval_presets;
...
const NUDGE_MODEL_SLUG: &str = "gpt-5.1-nexal-mini";
...
#[cfg(test)]
fn core_rate_limit_error_kind(info: &CoreNexalErrorInfo) -> Option<RateLimitErrorKind> {
    match info {
        CoreNexalErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        CoreNexalErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        CoreNexalErrorInfo::ResponseTooManyFailedAttempts {
...
fn app_server_rate_limit_error_kind(info: &AppServerNexalErrorInfo) -> Option<RateLimitErrorKind> {
    match info {
        AppServerNexalErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        AppServerNexalErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        AppServerNexalErrorInfo::ResponseTooManyFailedAttempts {
            http_status_code: Some(429),
        } => Some(RateLimitErrorKind::Generic),
        _ => None,
    }
...
/// and by sending requests back to nexal-core.
///
...
pub(crate) struct ChatWidget {
    ...
    // Latest completed user-visible Nexal output that `/copy` should place on the clipboard.
    ...
    /// Tracks whether nexal-core currently considers an agent turn to be in progress.
    ///
    ...
    session_network_proxy: Option<nexal_protocol::protocol::SessionNetworkProxyRuntime>,
    // Shared latch so we only warn once about invalid status-line item IDs.
...
) -> nexal_protocol::protocol::SessionConfiguredEvent {
    nexal_protocol::protocol::SessionConfiguredEvent {
...
fn app_server_request_id_to_mcp_request_id(
    request_id: &nexal_app_server_protocol::RequestId,
) -> nexal_protocol::mcp::RequestId {
    match request_id {
        nexal_app_server_protocol::RequestId::String(value) => {
            nexal_protocol::mcp::RequestId::String(value.clone())
        }
        nexal_app_server_protocol::RequestId::Integer(value) => {
            nexal_protocol::mcp::RequestId::Integer(*value)
        }
    }
}
...
fn exec_approval_request_from_params(
    ...
    ExecApprovalRequestEvent {
        ...
        proposed_execpolicy_amendment: params
            ...
            .map(nexal_app_server_protocol::ExecPolicyAmendment::into_core),
        proposed_network_policy_amendments: params.proposed_network_policy_amendments.map(
            |amendments| {
                amendments
                    .into_iter()
                    .map(nexal_app_server_protocol::NetworkPolicyAmendment::into_core)
                    .collect()
            },
        ),
        skill_metadata: params.skill_metadata.map(|metadata| {
            nexal_protocol::approvals::ExecApprovalRequestSkillMetadata {
        ...
        available_decisions: params.available_decisions.map(|decisions| {
            decisions
                ...
                .map(|decision| match decision {
                    nexal_app_server_protocol::CommandExecutionApprovalDecision::Accept => {
                        nexal_protocol::protocol::ReviewDecision::Approved
                    }
                    nexal_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession => {
                        nexal_protocol::protocol::ReviewDecision::ApprovedForSession
                    }
                    nexal_app_server_protocol::CommandExecutionApprovalDecision::AcceptWithExecpolicyAmendment {
                        execpolicy_amendment,
                    } => nexal_protocol::protocol::ReviewDecision::ApprovedExecpolicyAmendment {
                        proposed_execpolicy_amendment: execpolicy_amendment.into_core(),
                    },
                    nexal_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                        network_policy_amendment,
                    } => nexal_protocol::protocol::ReviewDecision::NetworkPolicyAmendment {
                        network_policy_amendment: network_policy_amendment.into_core(),
                    },
                    nexal_app_server_protocol::CommandExecutionApprovalDecision::Decline => {
                        nexal_protocol::protocol::ReviewDecision::Denied
                    }
                    nexal_app_server_protocol::CommandExecutionApprovalDecision::Cancel => {
                        nexal_protocol::protocol::ReviewDecision::Abort
                    }
                })
                .collect()
        }),
        parsed_cmd: params
            ...
            .unwrap_or_default()
            .into_iter()
            .map(nexal_app_server_protocol::CommandAction::into_core)
            .collect(),
    }
}
...
fn app_server_patch_changes_to_core(
    changes: Vec<nexal_app_server_protocol::FileUpdateChange>,
) -> HashMap<PathBuf, nexal_protocol::protocol::FileChange> {
    changes
        .into_iter()
        .map(|change| {
            let path = PathBuf::from(change.path);
            let file_change = match change.kind {
                nexal_app_server_protocol::PatchChangeKind::Add => {
                    nexal_protocol::protocol::FileChange::Add {
                        content: change.diff,
                    }
                }
                nexal_app_server_protocol::PatchChangeKind::Delete => {
                    nexal_protocol::protocol::FileChange::Delete {
                        content: change.diff,
                    }
                }
                nexal_app_server_protocol::PatchChangeKind::Update { move_path } => {
                    nexal_protocol::protocol::FileChange::Update {
...
fn request_permissions_from_params(
    params: nexal_app_server_protocol::PermissionsRequestApprovalParams,
...
fn request_user_input_from_params(params: ToolRequestUserInputParams) -> RequestUserInputEvent {
    RequestUserInputEvent {
        ...
        questions: params
            ...
            .into_iter()
            .map(
                |question| nexal_protocol::request_user_input::RequestUserInputQuestion {
                    id: question.id,
...
fn web_search_action_to_core(
    action: nexal_app_server_protocol::WebSearchAction,
) -> nexal_protocol::models::WebSearchAction {
    match action {
        nexal_app_server_protocol::WebSearchAction::Search { query, queries } => {
            nexal_protocol::models::WebSearchAction::Search { query, queries }
        }
        nexal_app_server_protocol::WebSearchAction::OpenPage { url } => {
            nexal_protocol::models::WebSearchAction::OpenPage { url }
        }
        nexal_app_server_protocol::WebSearchAction::FindInPage { url, pattern } => {
            nexal_protocol::models::WebSearchAction::FindInPage { url, pattern }
        }
        nexal_app_server_protocol::WebSearchAction::Other => {
            nexal_protocol::models::WebSearchAction::Other
        }
    }
}
...
impl ChatWidget {
    ...
    fn on_session_configured(&mut self, event: nexal_protocol::protocol::SessionConfiguredEvent) {
        self.bottom_pane
    ...
    fn emit_forked_thread_event(&self, forked_from_id: ThreadId) {
        ...
        tokio::spawn(async move {
            ...
            match find_thread_name_by_id(&nexal_home, &forked_from_id).await {
    ...
    fn on_thread_name_updated(&mut self, event: nexal_protocol::protocol::ThreadNameUpdatedEvent) {
    ...
    #[cfg(test)]
    fn handle_steer_rejected_error(&mut self, nexal_error_info: &CoreNexalErrorInfo) -> bool {
        matches!(
            nexal_error_info,
            CoreNexalErrorInfo::ActiveTurnNotSteerable { .. }
        ) && self.enqueue_rejected_steer()
    }

    fn handle_app_server_steer_rejected_error(
        &mut self,
        nexal_error_info: &AppServerNexalErrorInfo,
    ) -> bool {
        matches!(
            nexal_error_info,
            AppServerNexalErrorInfo::ActiveTurnNotSteerable { .. }
        ) && self.enqueue_rejected_steer()
    }
    ...
    pub(crate) fn on_rate_limit_snapshot(&mut self, snapshot: Option<RateLimitSnapshot>) {
        if let Some(mut snapshot) = snapshot {
            ...
            let is_nexal_limit = limit_id.eq_ignore_ascii_case("nexal");
            let warnings = if is_nexal_limit {
    ...
    fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "Nexal is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    fn handle_non_retry_error(
        &mut self,
        message: String,
        nexal_error_info: Option<AppServerNexalErrorInfo>,
    ) {
        if nexal_error_info
            .as_ref()
            .is_some_and(|info| self.handle_app_server_steer_rejected_error(info))
        {
        } else if let Some(info) = nexal_error_info
            .as_ref()
            ...
            match tool {
                ...
                Some("mcp_tool_call") => {
                    ...
                    history_cell::new_guardian_denied_action_request(format!(
                        "nexal to call MCP tool {server}.{tool_name}"
                    ))
                }
                Some("network_access") => {
                    let target = action
    ...
    fn on_patch_apply_end(&mut self, event: nexal_protocol::protocol::PatchApplyEndEvent) {
        let ev2 = event.clone();
        self.defer_or_handle(
            ...
            CollabAgentTool::SpawnAgent => {
                ...
                if !matches!(status, CollabAgentToolCallStatus::InProgress) {
                    ...
                    self.on_collab_event(multi_agents::spawn_end(
                        nexal_protocol::protocol::CollabAgentSpawnEndEvent {
                            call_id: id,
                            sender_thread_id,
            ...
            CollabAgentTool::SendInput => {
                    ...
                    && !matches!(status, CollabAgentToolCallStatus::InProgress)
                {
                    self.on_collab_event(multi_agents::interaction_end(
                        nexal_protocol::protocol::CollabAgentInteractionEndEvent {
                            call_id: id,
                            sender_thread_id,
                            receiver_thread_id,
                ...
                if let Some(receiver_thread_id) = first_receiver {
                    if matches!(status, CollabAgentToolCallStatus::InProgress) {
                        self.on_collab_event(multi_agents::resume_begin(
                            nexal_protocol::protocol::CollabResumeBeginEvent {
                                call_id: id,
                                sender_thread_id,
                                receiver_thread_id,
                                receiver_agent_nickname: first_receiver_metadata
                        ...
                        self.on_collab_event(multi_agents::resume_end(
                            nexal_protocol::protocol::CollabResumeEndEvent {
                                call_id: id,
                                sender_thread_id,
                                receiver_thread_id,
                                receiver_agent_nickname: first_receiver_metadata
            ...
            CollabAgentTool::Wait => {
                if matches!(status, CollabAgentToolCallStatus::InProgress) {
                    self.on_collab_event(multi_agents::waiting_begin(
                        nexal_protocol::protocol::CollabWaitingBeginEvent {
                            sender_thread_id,
                    ...
                    self.on_collab_event(multi_agents::waiting_end(
                        nexal_protocol::protocol::CollabWaitingEndEvent {
                            sender_thread_id,
                            call_id: id,
                            agent_statuses,
                            statuses,
            ...
            CollabAgentTool::CloseAgent => {
                    ...
                    && !matches!(status, CollabAgentToolCallStatus::InProgress)
                {
                    self.on_collab_event(multi_agents::close_end(
                        nexal_protocol::protocol::CollabCloseEndEvent {
                            call_id: id,
                            sender_thread_id,
                            receiver_thread_id,
                            receiver_agent_nickname: first_receiver_metadata
    ...
    pub(crate) fn handle_history_entry_response(
        &mut self,
        event: nexal_protocol::protocol::GetHistoryEntryResponseEvent,
    ) {
        let nexal_protocol::protocol::GetHistoryEntryResponseEvent {
            offset,
            log_id,
            entry,
        } = event;
    ...
    fn on_hook_started(&mut self, event: nexal_protocol::protocol::HookStartedEvent) {
    ...
    fn on_hook_completed(&mut self, event: nexal_protocol::protocol::HookCompletedEvent) {
        ...
        for entry in event.run.entries {
            let prefix = match entry.kind {
                nexal_protocol::protocol::HookOutputEntryKind::Warning => "warning: ",
                nexal_protocol::protocol::HookOutputEntryKind::Stop => "stop: ",
                nexal_protocol::protocol::HookOutputEntryKind::Feedback => "feedback: ",
                nexal_protocol::protocol::HookOutputEntryKind::Context => "hook context: ",
                nexal_protocol::protocol::HookOutputEntryKind::Error => "error: ",
            };
    ...
    pub(crate) fn handle_patch_apply_end_now(
        &mut self,
        event: nexal_protocol::protocol::PatchApplyEndEvent,
    ) {
    ...
    fn new_with_op_target(common: ChatWidgetInit, nexal_op_target: NexalOpTarget) -> Self {
        ...
        widget.bottom_pane.set_windows_degraded_sandbox_active(
            nexal_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
                && matches!(
        ...
        match cmd {
            ...
            SlashCommand::Rename => {
                self.session_telemetry
                    .counter("nexal.thread.rename", /*inc*/ 1, &[]);
            ...
            SlashCommand::ElevateSandbox => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    self.session_telemetry.counter(
                        "nexal.windows_sandbox.setup_elevated_sandbox_command",
                        /*inc*/ 1,
                        &[],
                    );
                    self.app_event_tx
            ...
            SlashCommand::Logout => {
                if let Err(e) = nexal_core::auth::logout(
            ...
            // SlashCommand::Undo => {
            //     self.app_event_tx.send(AppEvent::NexalOp(Op::Undo));
            // }
            SlashCommand::Diff => {
            ...
            SlashCommand::Copy => {
                let Some(text) = self.last_copyable_output.as_deref() else {
                    self.add_info_message(
                        "`/copy` is unavailable before the first Nexal output or right after a rollback."
                            .to_string(),
                        /*hint*/ None,
                    );
                    return;
                ...
                match copy_result {
                    Ok(()) => {
                        ...
                        self.add_info_message(
                            "Copied latest Nexal output to clipboard.".to_string(),
                            hint,
                        );
                    }
                    Err(err) => {
    ...
    fn dispatch_command_with_args(
        ...
        match cmd {
            ...
            SlashCommand::Rename if !trimmed.is_empty() => {
                ...
                let Some(name) = nexal_core::util::normalize_thread_name(&prepared_args) else {
    ...
    fn show_rename_prompt(&mut self) {
        ...
        let view = CustomPromptView::new(
            ...
            Box::new(move |name: String| {
                let Some(name) = nexal_core::util::normalize_thread_name(&name) else {
                    tx.send(AppEvent::InsertHistoryCell(Box::new(
    ...
    fn submit_user_message(&mut self, user_message: UserMessage) {
        ...
        if let Some(apps) = self.connectors_for_mentions() {
            ...
            for app in app_mentions {
                let slug = nexal_core::connectors::connector_mention_slug(&app);
    ...
    fn handle_thread_item(
        ...
        match item {
            ThreadItem::UserMessage { id, content } => {
                let user_message = nexal_protocol::items::UserMessageItem {
                    id,
                    content: content
                        .into_iter()
                        .map(nexal_app_server_protocol::UserInput::into_core)
                        .collect(),
                };
                let nexal_protocol::protocol::EventMsg::UserMessage(event) =
            ...
            ThreadItem::AgentMessage {
                ...
                self.on_agent_message_item_completed(AgentMessageItem {
                    ...
                    memory_citation: memory_citation.map(|citation| {
                        nexal_protocol::memory_citation::MemoryCitation {
                            entries: citation
                                .entries
                                .into_iter()
                                .map(
                                    |entry| nexal_protocol::memory_citation::MemoryCitationEntry {
                                        path: entry.path,
            ...
            ThreadItem::CommandExecution {
                ...
                if matches!(
                    status,
                    nexal_app_server_protocol::CommandExecutionStatus::InProgress
                ) {
                    ...
                    self.on_exec_command_end(ExecCommandEndEvent {
                        ...
                        status: match status {
                            nexal_app_server_protocol::CommandExecutionStatus::Completed => {
                                nexal_protocol::protocol::ExecCommandStatus::Completed
                            }
                            nexal_app_server_protocol::CommandExecutionStatus::Failed => {
                                nexal_protocol::protocol::ExecCommandStatus::Failed
                            }
                            nexal_app_server_protocol::CommandExecutionStatus::Declined => {
                                nexal_protocol::protocol::ExecCommandStatus::Declined
                            }
                            nexal_app_server_protocol::CommandExecutionStatus::InProgress => {
                                nexal_protocol::protocol::ExecCommandStatus::Failed
                            }
            ...
            ThreadItem::FileChange {
                ...
                if !matches!(
                    status,
                    nexal_app_server_protocol::PatchApplyStatus::InProgress
                ) {
                    self.on_patch_apply_end(nexal_protocol::protocol::PatchApplyEndEvent {
                        call_id: id,
                        turn_id: turn_id.clone(),
                        stdout: String::new(),
                        stderr: String::new(),
                        success: !matches!(
                            status,
                            nexal_app_server_protocol::PatchApplyStatus::Failed
                        ),
                        changes: app_server_patch_changes_to_core(changes),
                        status: match status {
                            nexal_app_server_protocol::PatchApplyStatus::Completed => {
                                nexal_protocol::protocol::PatchApplyStatus::Completed
                            }
                            nexal_app_server_protocol::PatchApplyStatus::Failed => {
                                nexal_protocol::protocol::PatchApplyStatus::Failed
                            }
                            nexal_app_server_protocol::PatchApplyStatus::Declined => {
                                nexal_protocol::protocol::PatchApplyStatus::Declined
                            }
                            nexal_app_server_protocol::PatchApplyStatus::InProgress => {
                                nexal_protocol::protocol::PatchApplyStatus::Failed
                            }
            ...
            ThreadItem::McpToolCall {
                ...
                self.on_mcp_tool_call_end(nexal_protocol::protocol::McpToolCallEndEvent {
                    call_id: id,
                    invocation: nexal_protocol::protocol::McpInvocation {
                        server,
                        tool,
                    ...
                    result: match (result, error) {
                        ...
                        (Some(result), None) => Ok(nexal_protocol::mcp::CallToolResult {
            ...
            ThreadItem::WebSearch { id, query, action } => {
                ...
                self.on_web_search_end(WebSearchEndEvent {
                    ...
                    action: action
                        ...
                        .unwrap_or(nexal_protocol::models::WebSearchAction::Other),
                });
            }
        ...
        match notification {
            ...
            ServerNotification::ThreadNameUpdated(notification) => {
                match ThreadId::from_string(&notification.thread_id) {
                    Ok(thread_id) => self.on_thread_name_updated(
                        nexal_protocol::protocol::ThreadNameUpdatedEvent {
                            thread_id,
            ...
            ServerNotification::CommandExecutionOutputDelta(notification) => {
                self.on_exec_command_output_delta(ExecCommandOutputDeltaEvent {
                    call_id: notification.item_id,
                    stream: nexal_protocol::protocol::ExecOutputStream::Stdout,
            ...
            ServerNotification::HookStarted(notification) => {
                if let Some(run) = convert_via_json(notification.run) {
                    self.on_hook_started(nexal_protocol::protocol::HookStartedEvent {
            ...
            ServerNotification::HookCompleted(notification) => {
                if let Some(run) = convert_via_json(notification.run) {
                    self.on_hook_completed(nexal_protocol::protocol::HookCompletedEvent {
                        turn_id: notification.turn_id,
                        run,
                    });
                }
            ...
            ServerNotification::Error(notification) => {
                if notification.will_retry {
                    ...
                    self.handle_non_retry_error(
                        notification.error.message,
                        notification.error.nexal_error_info,
                    );
                }
            }
            ...
            ServerNotification::ThreadRealtimeStarted(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_started(
                        nexal_protocol::protocol::RealtimeConversationStartedEvent {
            ...
            ServerNotification::ThreadRealtimeItemAdded(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_realtime(
                        nexal_protocol::protocol::RealtimeConversationRealtimeEvent {
                            payload: nexal_protocol::protocol::RealtimeEvent::ConversationItemAdded(
                                notification.item,
                            ),
                        },
                    );
            ...
            ServerNotification::ThreadRealtimeOutputAudioDelta(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_realtime(
                        nexal_protocol::protocol::RealtimeConversationRealtimeEvent {
                            payload: nexal_protocol::protocol::RealtimeEvent::AudioOut(
            ...
            ServerNotification::ThreadRealtimeError(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_realtime(
                        nexal_protocol::protocol::RealtimeConversationRealtimeEvent {
                            payload: nexal_protocol::protocol::RealtimeEvent::Error(
            ...
            ServerNotification::ThreadRealtimeClosed(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_closed(
                        nexal_protocol::protocol::RealtimeConversationClosedEvent {
    ...
    fn on_mcp_server_elicitation_request(
        &mut self,
        request_id: nexal_protocol::mcp::RequestId,
        params: nexal_app_server_protocol::McpServerElicitationRequestParams,
    ) {
        let request = nexal_protocol::approvals::ElicitationRequestEvent {
            turn_id: params.turn_id,
            server_name: params.server_name,
            id: request_id,
            request: match params.request {
                nexal_app_server_protocol::McpServerElicitationRequest::Form {
                    meta,
                    message,
                    requested_schema,
                } => nexal_protocol::approvals::ElicitationRequest::Form {
                    meta,
                    message,
                    requested_schema: serde_json::to_value(requested_schema)
                        .unwrap_or(serde_json::Value::Null),
                },
                nexal_app_server_protocol::McpServerElicitationRequest::Url {
                    meta,
                    message,
                    url,
                    elicitation_id,
                } => nexal_protocol::approvals::ElicitationRequest::Url {
                    meta,
                    message,
                    url,
                    elicitation_id,
            ...
            TurnStatus::Failed => {
                if let Some(error) = notification.turn.error {
                        ...
                        == Some(&(notification.turn.id.clone(), error.message.clone()))
                    {
                        ...
                        self.handle_non_retry_error(error.message, error.nexal_error_info);
                    }
                } else {
                    self.last_non_retry_error = None;
                    self.finalize_turn();
    ...
    fn on_guardian_review_notification(
        ...
        id: String,
        turn_id: String,
        review: nexal_app_server_protocol::GuardianApprovalReview,
        ...
        self.on_guardian_assessment(GuardianAssessmentEvent {
            ...
            status: match review.status {
                nexal_app_server_protocol::GuardianApprovalReviewStatus::InProgress => {
                    GuardianAssessmentStatus::InProgress
                }
                nexal_app_server_protocol::GuardianApprovalReviewStatus::Approved => {
                    GuardianAssessmentStatus::Approved
                }
                nexal_app_server_protocol::GuardianApprovalReviewStatus::Denied => {
                    GuardianAssessmentStatus::Denied
                }
                nexal_app_server_protocol::GuardianApprovalReviewStatus::Aborted => {
                    GuardianAssessmentStatus::Aborted
                }
            },
            risk_score: review.risk_score,
            risk_level: review.risk_level.map(|risk_level| match risk_level {
                nexal_app_server_protocol::GuardianRiskLevel::Low => {
                    nexal_protocol::protocol::GuardianRiskLevel::Low
                }
                nexal_app_server_protocol::GuardianRiskLevel::Medium => {
                    nexal_protocol::protocol::GuardianRiskLevel::Medium
                }
                nexal_app_server_protocol::GuardianRiskLevel::High => {
                    nexal_protocol::protocol::GuardianRiskLevel::High
                }
            }),
    ...
    #[cfg(test)]
    pub(crate) fn handle_nexal_event(&mut self, event: Event) {
        let Event { id, msg } = event;
        self.dispatch_event_msg(Some(id), msg, /*replay_kind*/ None);
    }

    #[cfg(test)]
    pub(crate) fn handle_nexal_event_replay(&mut self, event: Event) {
        let Event { msg, .. } = event;
    ...
    fn dispatch_event_msg(
        ...
        match msg {
            ...
            _ => {
                tracing::trace!("handle_nexal_event: {:?}", msg);
            }
        }
        ...
        match msg {
            ...
            EventMsg::Error(ErrorEvent {
                ...
                {
                } else if let Some(kind) = nexal_error_info
                    .as_ref()
            ...
            EventMsg::ItemCompleted(event) => {
                let item = event.item;
                if !from_replay && let nexal_protocol::items::TurnItem::UserMessage(item) = &item {
                ...
                if let nexal_protocol::items::TurnItem::Plan(plan_item) = &item {
                    self.on_plan_item_completed(plan_item.text.clone());
                }
                if let nexal_protocol::items::TurnItem::AgentMessage(item) = item {
    ...
    fn on_entered_review_mode(&mut self, review: ReviewRequest, from_replay: bool) {
        let hint = review
            .user_facing_hint
            .unwrap_or_else(|| nexal_core::review_prompts::user_facing_hint(&review.target));
    ...
    fn open_theme_picker(&mut self) {
        let nexal_home = nexal_core::config::find_nexal_home().ok();
    ...
    fn prefetch_connectors_with_options(&mut self, force_refetch: bool) {
        ...
        tokio::spawn(async move {
            ...
            let should_schedule_force_refetch =
                !force_refetch && !accessible_result.nexal_apps_ready;
    ...
    fn open_personality_popup_for_current_model(&mut self) {
        ...
        header.push(Line::from("Choose a communication style for Nexal.".dim()));
    ...
    pub(crate) fn open_realtime_audio_popup(&mut self) {
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            ...
            subtitle: Some("Configure settings for Nexal.".to_string()),
            footer_hint: Some(standard_popup_hint_line()),
            items,
            ..Default::default()
        });
    ...
    fn is_auto_model(model: &str) -> bool {
        model.starts_with("nexal-auto-")
    }

    fn auto_model_order(model: &str) -> usize {
        match model {
            "nexal-auto-fast" => 0,
            "nexal-auto-balanced" => 1,
            "nexal-auto-thorough" => 2,
            _ => 3,
        }
    }
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        let header = self.model_menu_header(
            ...
            "Access legacy models by running nexal -m <model_name> or in your config.toml",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
    ...
    pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        let warn_for_model = preset.model.starts_with("gpt-5.1-nexal")
            || preset.model.starts_with("gpt-5.1-nexal-max")
    ...
    pub(crate) fn open_permissions_popup(&mut self) {
        ...
        let show_elevate_sandbox_hint = nexal_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
        ...
        for preset in presets.into_iter() {
            ...
            let default_actions: Vec<SelectionAction> = if requires_confirmation {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    {
                        let preset_clone = preset.clone();
                        if nexal_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
                            && nexal_core::windows_sandbox::sandbox_setup_is_complete(
                                self.config.nexal_home.as_path(),
                            )
                        {
                            vec![Box::new(move |tx| {
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        match nexal_windows_sandbox::apply_world_writable_scan_and_denies(
            self.config.nexal_home.as_path(),
            cwd.as_path(),
            &env_map,
            self.config.permissions.sandbox_policy.get(),
            Some(self.config.nexal_home.as_path()),
        ) {
            Ok(_) => None,
    ...
    pub(crate) fn open_full_access_confirmation(
        ...
        let info_line = Line::from(vec![
            "When Nexal runs with full access, it can edit any file on your computer and run commands with network, without your approval. "
                .into(),
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(&mut self, preset: ApprovalPreset) {
        ...
        self.session_telemetry.counter(
            "nexal.windows_sandbox.elevated_prompt_shown",
            /*inc*/ 1,
            &[],
        );
        ...
        let items = vec![
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    accept_otel.counter(
                        "nexal.windows_sandbox.elevated_prompt_accept",
                        /*inc*/ 1,
                        &[],
                    );
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    legacy_otel.counter(
                        "nexal.windows_sandbox.elevated_prompt_use_legacy",
                        /*inc*/ 1,
                        &[],
                    );
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    quit_otel.counter(
                        "nexal.windows_sandbox.elevated_prompt_quit",
                        /*inc*/ 1,
                        &[],
                    );
        ...
        let items = vec![
            SelectionItem {
                ...
                actions: vec![Box::new({
                    ...
                    move |tx| {
                        otel.counter(
                            "nexal.windows_sandbox.fallback_retry_elevated",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsSandboxElevatedSetup {
            ...
            SelectionItem {
                name: "Use Nexal with non-admin sandbox".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.session_telemetry.clone();
                    ...
                    move |tx| {
                        otel.counter(
                            "nexal.windows_sandbox.fallback_use_legacy",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsSandboxLegacySetup {
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    quit_otel.counter(
                        "nexal.windows_sandbox.fallback_prompt_quit",
                        /*inc*/ 1,
                        &[],
                    );
                    tx.send(AppEvent::Exit(ExitMode::ShutdownFirst));
    ...
    pub(crate) fn set_collaboration_mask(&mut self, mut mask: CollaborationModeMask) {
            ...
            && (previous_model != next_model || previous_effort != next_effort)
        {
            ...
            if !next_model.starts_with("nexal-auto-") {
    ...
    fn plugins_for_mentions(&self) -> Option<&[nexal_core::plugins::PluginCapabilitySummary]> {
    ...
    fn rename_confirmation_cell(name: &str, thread_id: Option<ThreadId>) -> PlainHistoryCell {
        let resume_cmd = nexal_core::util::resume_command(Some(name), thread_id)
            .unwrap_or_else(|| format!("nexal resume {name}"));
    ...
    fn connectors_popup_params(
        ...
        for connector in connectors {
            ...
            let instructions = if connector.is_accessible {
                ...
                "Install this app in your browser, then reload Nexal."
            };
    ...
    /// Forward a command directly to nexal.
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.nexal_op_target {
            NexalOpTarget::Direct(nexal_op_tx) => {
                crate::session_log::log_outbound_op(&op);
                if let Err(e) = nexal_op_tx.send(op.into_core()) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            NexalOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::NexalOp(op.into()));
            }
        }
        true
    }
    ...
    pub(crate) fn refresh_plugin_mentions(&mut self) {
        ...
        let plugins = PluginsManager::new(self.config.nexal_home.clone())
...
impl Notification {
    ...
    fn user_input_request_summary(
        questions: &[nexal_protocol::request_user_input::RequestUserInputQuestion],
    ) -> Option<String> {
...
fn hook_event_label(event_name: nexal_protocol::protocol::HookEventName) -> &'static str {
    match event_name {
        nexal_protocol::protocol::HookEventName::PreToolUse => "PreToolUse",
        nexal_protocol::protocol::HookEventName::PostToolUse => "PostToolUse",
        nexal_protocol::protocol::HookEventName::SessionStart => "SessionStart",
        nexal_protocol::protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        nexal_protocol::protocol::HookEventName::Stop => "Stop",
    }
}
