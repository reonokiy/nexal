...
use nexal_login::complete_device_code_login;
use nexal_login::request_device_code;
...
pub(super) fn start_headless_chatgpt_login(widget: &mut AuthModeWidget) {
    ...
    let nexal_home = widget.nexal_home.clone();
...
async fn fallback_to_browser_login(
    request_handle: nexal_app_server_client::AppServerRequestHandle,
    sign_in_state: Arc<RwLock<SignInState>>,
    request_frame: FrameRequester,
    error: Arc<RwLock<Option<String>>>,
    cancel: Arc<Notify>,
