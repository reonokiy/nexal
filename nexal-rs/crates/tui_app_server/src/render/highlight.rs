...
//! | `NEXAL_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
static NEXAL_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
/// Set the user-configured syntax theme override and nexal home path.
///
/// Call this with the **final resolved config** (after onboarding, resume, and
/// fork reloads complete). The first call persists `name` and `nexal_home` in
...
pub(crate) fn set_theme_override(
    ...
    let warning = validate_theme_name(name.as_deref(), nexal_home.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let nexal_home_set_ok = NEXAL_HOME.set(nexal_home.clone()).is_ok();
    ...
    if !override_set_ok || !nexal_home_set_ok {
...
pub(crate) fn validate_theme_name(name: Option<&str>, nexal_home: Option<&Path>) -> Option<String> {
    ...
    let custom_theme_path_display = nexal_home
        ...
        .unwrap_or_else(|| format!("$NEXAL_HOME/themes/{name}.tmTheme"));
...
fn custom_theme_path(name: &str, nexal_home: &Path) -> PathBuf {
    nexal_home.join("themes").join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{nexal_home}/themes/{name}.tmTheme`.
fn load_custom_theme(name: &str, nexal_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, nexal_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, nexal_home: Option<&Path>) -> Theme {
    ...
    if let Some(name) = name {
        ...
        // 2. Try loading {NEXAL_HOME}/themes/{name}.tmTheme from disk.
        if let Some(home) = nexal_home
...
fn build_default_theme() -> Theme {
    ...
    let nexal_home = NEXAL_HOME
        .get()
        .and_then(|nexal_home| nexal_home.as_deref());
    resolve_theme_with_override(name, nexal_home)
}
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        ...
        if let Some(Some(home)) = NEXAL_HOME.get()
