//! Cloud-hosted config requirements for Nexal.
//!
...
//! requirements cannot be loaded for those accounts, Nexal fails configuration loading rather than
...
use nexal_core::auth::RefreshTokenError;
...
const CLOUD_REQUIREMENTS_CACHE_WRITE_HMAC_KEY: &[u8] =
    b"nexal-cloud-requirements-cache-v3-064f8542-75b4-494c-a294-97d3ce597271";
...
fn auth_identity(auth: &NexalAuth) -> (Option<String>, Option<String>) {
...
impl CloudRequirementsService {
    fn new(
        ...
        Self {
            auth_manager,
            fetcher,
            cache_path: nexal_home.join(CLOUD_REQUIREMENTS_CACHE_FILENAME),
            timeout,
        }
    }
    ...
    async fn fetch_with_timeout(
        ...
        let _timer =
            nexal_otel::start_global_timer("codex.cloud_requirements.fetch.duration_ms", &[]);
...
pub fn cloud_requirements_loader_for_storage(
    ...
    cloud_requirements_loader(auth_manager, chatgpt_base_url, nexal_home)
}
...
mod tests {
    ...
    fn write_auth_json(nexal_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
        std::fs::write(nexal_home.join("auth.json"), serde_json::to_string(&value)?)?;
        Ok(())
    }
...
"#,
        ...
        assert_eq!(
            result,
            Some(ConfigRequirementsToml {
                apps: Some(nexal_core::config_loader::AppsRequirementsToml {
                    apps: BTreeMap::from([(
                        "connector_5f3c8c41a1e54ad7a76272c89e2554fa".to_string(),
                        nexal_core::config_loader::AppRequirementToml {
                            enabled: Some(false),
                        },
                    )]),
                }),
    ...
    async fn fetch_cloud_requirements_recovers_after_unauthorized_reload_updates_cache_identity() {
        ...
        let path = nexal_home.path().join(CLOUD_REQUIREMENTS_CACHE_FILENAME);
