...
`nexal-otel` is the OpenTelemetry integration crate for Nexal. It provides:

- Provider wiring for log/trace/metric exporters (`nexal_otel::OtelProvider`
  and `nexal_otel::provider`).
- Session-scoped business event emission via `nexal_otel::SessionTelemetry`.
- Low-level metrics APIs via `nexal_otel::metrics`.
- Trace-context helpers via `nexal_otel::trace_context` and crate-root re-exports.
...
```rust
use nexal_otel::config::OtelExporter;
use nexal_otel::config::OtelHttpProtocol;
use nexal_otel::config::OtelSettings;
use nexal_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "nexal-cli".to_string(),
    service_version: env!("CARGO_PKG_VERSION").to_string(),
    nexal_home: std::path::PathBuf::from("/tmp"),
...
Nexal-specific session events. Rich session/business events should go through
...
`nexal-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using Nexal-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use nexal_otel::config::{OtelExporter, OtelHttpProtocol};
