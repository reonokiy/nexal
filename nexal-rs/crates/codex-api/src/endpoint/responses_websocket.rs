...
use nexal_client::TransportError;
use nexal_client::maybe_build_rustls_client_config_with_custom_ca;
...
const X_NEXAL_TURN_STATE_HEADER: &str = "x-nexal-turn-state";
...
async fn connect_websocket(
    ...
    // If a Nexal-specific CA bundle is configured, build an explicit rustls connector so this
...
mod tests {
    ...
    fn parse_wrapped_websocket_error_event_maps_to_transport_http() {
        ...
        assert_eq!(
            headers
                .get("x-nexal-primary-used-percent")
                .and_then(|value| value.to_str().ok()),
            Some("100.0")
        );
        assert_eq!(
            headers
                .get("x-nexal-primary-window-minutes")
                .and_then(|value| value.to_str().ok()),
            Some("15")
        );
