...
The Web UI defaults to `wss://nexal.nokiy.net/ws` as the backend URL. Users can override it with `VITE_NEXAL_BACKEND` or from the Settings page. Browser-side settings are stored in `localStorage` with the `nexal.` prefix.
