...
//! - sensitive subpaths such as `.git` and `.nexal` remain read-only even when
...
/// inside a writable root (e.g., `.nexal -> ./decoy`). In that case we mount
...
mod tests {
    use super::*;
    use nexal_protocol::protocol::FileSystemAccessMode;
    use nexal_protocol::protocol::FileSystemPath;
    use nexal_protocol::protocol::FileSystemSandboxEntry;
    use nexal_protocol::protocol::FileSystemSandboxPolicy;
    use nexal_protocol::protocol::FileSystemSpecialPath;
