...
use nexal_core::config::types::ShellEnvironmentPolicy;
use nexal_core::error::NexalErr;
use nexal_core::error::Result;
use nexal_core::error::SandboxErr;
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> nexal_core::exec::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<nexal_core::exec::ExecToolCallOutput> {
...
async fn run_cmd_result_with_policies(
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_nexal-linux-sandbox");
    let nexal_linux_sandbox_exe = Some(PathBuf::from(sandbox_program));
...
fn is_bwrap_unavailable_output(output: &nexal_core::exec::ExecToolCallOutput) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(NexalErr::Sandbox(SandboxErr::Denied { output, .. })) => {
        ...
        Err(NexalErr::Sandbox(SandboxErr::Timeout { .. })) => true,
        Err(err) => panic!("bwrap availability probe failed unexpectedly: {err:?}"),
    }
}

fn expect_denied(
    result: Result<nexal_core::exec::ExecToolCallOutput>,
    context: &str,
) -> nexal_core::exec::ExecToolCallOutput {
    match result {
        Ok(output) => {
        ...
        Err(NexalErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_nexal-linux-sandbox");
    let nexal_linux_sandbox_exe: Option<PathBuf> = Some(PathBuf::from(sandbox_program));
...
#[tokio::test]
async fn sandbox_blocks_git_and_nexal_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_nexal = tmpdir.path().join(".nexal");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_nexal).expect("create .nexal");

    let git_target = dot_git.join("config");
    let nexal_target = dot_nexal.join("config.toml");
    ...
    let nexal_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", nexal_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            false,
        ...
        .await,
        ".nexal write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(nexal_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_nexal_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-nexal");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_nexal = tmpdir.path().join(".nexal");
    symlink(&decoy, &dot_nexal).expect("create .nexal symlink");
    ...
    let nexal_output = expect_denied(
        ...
        .await,
        ".nexal symlink replacement should be denied",
    );
    assert_ne!(nexal_output.exit_code, 0);
}
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = PathBuf::from(env!("CARGO_BIN_EXE_nexal-linux-sandbox"))
        .parent()
