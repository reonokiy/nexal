//! Pre-[`Connection`] msgpack message channel over WebSocket or stdio.
//!
//! Yields a stream of typed messages (`T`) without the request/response
//! correlation that [`crate::connection::Connection`] provides. Still
//! used by the stdio bridges in `nexal-agent` and the agent-spawn flow
//! in `nexal-gateway` where the higher-level `Connection` API is not
//! yet wired up.
...
#[derive(Debug)]
pub enum MessageChannelEvent<T> {
    Message(T),
...
pub struct MessageChannel<T> {
    ...
    incoming_rx: mpsc::Receiver<MessageChannelEvent<T>>,
    ...
    T: DeserializeOwned + Serialize + Send + Sync + 'static,
{
                    ...
                    Some(Ok(Message::Binary(bytes))) => match rmp_serde::from_slice::<T>(&bytes) {
                        Ok(message) => {
                            if incoming_tx_for_reader
                                .send(MessageChannelEvent::Message(message))
                                .await
                                .is_err()
                            {
                                break;
    ...
    pub fn into_parts(
        ...
        mpsc::Sender<T>,
        mpsc::Receiver<MessageChannelEvent<T>>,
...
async fn send_disconnected<T>(
    incoming_tx: &mpsc::Sender<MessageChannelEvent<T>>,
    reason: Option<String>,
) {
    let _ = incoming_tx
        .send(MessageChannelEvent::Disconnected { reason })
        .await;
}
...
async fn send_malformed_message<T>(
    ...
    let _ = incoming_tx
        .send(MessageChannelEvent::MalformedMessage {
