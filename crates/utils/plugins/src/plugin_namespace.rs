...
pub const PLUGIN_MANIFEST_PATH: &str = ".nexal-plugin/plugin.json";
...
/// plugin manifest (same `name` rules as full manifest loading in nexal-core).
...
mod tests {
    ...
    fn uses_manifest_name() {
        ...
        fs::create_dir_all(plugin_root.join(".nexal-plugin")).expect("mkdir manifest");
        fs::write(
