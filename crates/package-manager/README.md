# nexal-package-manager

`nexal-package-manager` is the shared installer used for versioned runtime bundles and other cached artifacts in `nexal-rs`.
...
```text
<nexal_home>/<default_cache_root_relative>
```
...
```rust,ignore
let config = PackageManagerConfig::new(nexal_home, MyPackage::new(...));
