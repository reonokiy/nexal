...
impl OllamaClient {
    ...
    async fn try_from_provider_with_base_url(base_url: &str) -> io::Result<Self> {
        let provider =
            nexal_core::create_oss_provider_with_base_url(base_url, nexal_core::WireApi::Responses);
        Self::try_from_provider(&provider).await
    }
...
mod tests {
    ...
    async fn test_fetch_models_happy_path() {
        if std::env::var(nexal_core::spawn::NEXAL_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
            tracing::info!(
