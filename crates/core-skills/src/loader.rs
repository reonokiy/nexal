...
use nexal_config_loader::ConfigLayerStackOrdering;
use nexal_config_loader::default_project_root_markers;
...
use nexal_utils_plugins::plugin_namespace_for_skill_path;
use dirs::home_dir;
    ...
    for layer in config_layer_stack.get_layers(
        ...
        match &layer.name {
            ...
            ConfigLayerSource::User { .. } => {
                // Deprecated user skills location (`$NEXAL_HOME/skills`), kept for backward
                // compatibility.
                roots.push(SkillRoot {
                    path: config_folder.as_path().join(SKILLS_DIR_NAME),
                    scope: SkillScope::User,
                ...
                // Embedded system skills are cached under `$NEXAL_HOME/skills/.system` and are a
            ...
            ConfigLayerSource::System { .. } => {
                // The system config layer lives under `/etc/nexal/` on Unix, so treat
                // `/etc/nexal/skills` as admin-scoped skills.
                roots.push(SkillRoot {
                    path: config_folder.as_path().join(SKILLS_DIR_NAME),
...
fn repo_agents_skill_roots(config_layer_stack: &ConfigLayerStack, cwd: &Path) -> Vec<SkillRoot> {
    ...
    // Also check for a top-level `skills/` directory in each ancestor.
    // This supports project layouts where skills live at `<root>/skills/`
    // rather than `<root>/.agents/skills/`.
    for dir in &dirs {
        let plain_skills = dir.join(SKILLS_DIR_NAME);
...
fn discover_skills_under_root(root: &Path, scope: SkillScope, outcome: &mut SkillLoadOutcome) {
    ...
    // Follow symlinked directories for user, admin, and repo skills. System skills are written by Nexal itself.
