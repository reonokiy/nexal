...
//!
//! If the keyring is not available or fails, we fall back to NEXAL_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "Nexal MCP Credentials";
...
/// Determine where Nexal should store and read MCP credentials.
...
mod tests {
    ...
    use nexal_keyring_store::tests::MockKeyringStore;

    struct TempNexalHome {
    ...
    impl TempNexalHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create NEXAL_HOME temp dir");
            unsafe {
                std::env::set_var("NEXAL_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
    ...
    impl Drop for TempNexalHome {
        fn drop(&mut self) {
            unsafe {
                std::env::remove_var("NEXAL_HOME");
            }
        }
    }
