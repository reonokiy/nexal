...
use nexal_chatgpt::apply_command::ApplyCommand;
use nexal_chatgpt::apply_command::run_apply_command;
use nexal_cli::LandlockCommand;
use nexal_cli::SeatbeltCommand;
use nexal_cli::WindowsCommand;
use nexal_cli::login::read_api_key_from_stdin;
use nexal_cli::login::run_login_status;
use nexal_cli::login::run_login_with_api_key;
use nexal_cli::login::run_login_with_chatgpt;
use nexal_cli::login::run_login_with_device_code;
use nexal_cli::login::run_logout;
use nexal_cloud_tasks::Cli as CloudTasksCli;
use nexal_exec::Cli as ExecCli;
use nexal_exec::Command as ExecCommand;
use nexal_exec::ReviewArgs;
use nexal_execpolicy::ExecPolicyCheckCommand;
use nexal_responses_api_proxy::Args as ResponsesApiProxyArgs;
use nexal_rollout_state::StateRuntime;
use nexal_rollout_state::state_db_path;
...
use nexal_features::is_known_feature_key;
...
#[clap(
    ...
    // `nexal-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `nexal` command name that users run.
    bin_name = "nexal",
    override_usage = "nexal [OPTIONS] [PROMPT]\n       nexal [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    ...
    /// Manage external MCP servers for Nexal.
    Mcp(McpCli),

    /// Start Nexal as an MCP server (stdio).
    McpServer,

    /// [experimental] Run the app server or related tooling.
    AppServer(AppServerCommand),

    /// Launch the Nexal desktop app (downloads the macOS installer if missing).
    #[cfg(target_os = "macos")]
    ...
    /// Run commands within a Nexal-provided sandbox.
    Sandbox(SandboxArgs),
    ...
    /// Apply the latest diff produced by Nexal agent as a `git apply` to your local working tree.
    ...
    /// [EXPERIMENTAL] Browse tasks from Nexal Cloud and apply changes locally.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | nexal login --with-api-key`)"
    )]
    with_api_key: bool,
...
struct AppServerCommand {
    ...
    #[arg(
        long = "listen",
        value_name = "URL",
        default_value = nexal_app_server::AppServerTransport::DEFAULT_LISTEN_URL
    )]
    listen: nexal_app_server::AppServerTransport,
    ...
    /// ```
    ///
    /// See https://developers.openai.com/nexal/config-advanced/#metrics for more details.
    #[arg(long = "analytics-default-enabled")]
    analytics_default_enabled: bool,

    #[command(flatten)]
    auth: nexal_app_server::AppServerWebsocketAuthArgs,
}
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for Nexal tooling.
    #[clap(hide = true)]
...
fn format_exit_messages(exit_info: AppExitInfo, color_enabled: bool) -> Vec<String> {
    ...
    if let Some(resume_cmd) =
        nexal_core::util::resume_command(thread_name.as_deref(), conversation_id)
    {
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating Nexal via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart Nexal.");
    Ok(())
}
...
async fn run_debug_app_server_command(cmd: DebugAppServerCommand) -> anyhow::Result<()> {
    match cmd.subcommand {
        DebugAppServerSubcommand::SendMessageV2(cmd) => {
            let nexal_bin = std::env::current_exe()?;
            nexal_app_server_test_client::send_message_v2(&nexal_bin, &[], cmd.user_message, &None)
                .await
        }
    }
}
...
async fn cli_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    match subcommand {
        ...
        Some(Subcommand::Exec(mut exec_cli)) => {
            ...
            nexal_exec::run_main(exec_cli, arg0_paths.clone()).await?;
        }
        Some(Subcommand::Review(review_args)) => {
            ...
            let mut exec_cli = ExecCli::try_parse_from(["nexal", "exec"])?;
        ...
        Some(Subcommand::McpServer) => {
            ...
            nexal_mcp_server::run_main(arg0_paths.clone(), root_config_overrides).await?;
        }
        ...
        Some(Subcommand::AppServer(app_server_cli)) => {
            ...
            match subcommand {
                None => {
                    ...
                    nexal_app_server::run_main_with_transport(
                        arg0_paths.clone(),
                        root_config_overrides,
                        nexal_core::config_loader::LoaderOverrides::default(),
                        analytics_default_enabled,
                        transport,
                        nexal_protocol::protocol::SessionSource::VSCode,
                        auth,
                    )
                    .await?;
                }
                Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                    let options = nexal_app_server_protocol::GenerateTsOptions {
                        experimental_api: gen_cli.experimental,
                        ..Default::default()
                    };
                    nexal_app_server_protocol::generate_ts_with_options(
                        &gen_cli.out_dir,
                ...
                Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                    nexal_app_server_protocol::generate_json_with_experimental(
                        &gen_cli.out_dir,
                        gen_cli.experimental,
                    )?;
                }
                Some(AppServerSubcommand::GenerateInternalJsonSchema(gen_cli)) => {
                    nexal_app_server_protocol::generate_internal_json_schema(&gen_cli.out_dir)?;
                }
            }
        }
        #[cfg(target_os = "macos")]
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.use_device_code {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | nexal login --with-api-key`."
                        );
                        std::process::exit(1);
        ...
        Some(Subcommand::Cloud(mut cloud_cli)) => {
            ...
            nexal_cloud_tasks::run_main(cloud_cli, arg0_paths.nexal_linux_sandbox_exe.clone())
                .await?;
        }
        Some(Subcommand::Sandbox(sandbox_args)) => match sandbox_args.cmd {
            SandboxCommand::Macos(mut seatbelt_cli) => {
                ...
                nexal_cli::debug_sandbox::run_command_under_seatbelt(
                    seatbelt_cli,
                    arg0_paths.nexal_linux_sandbox_exe.clone(),
                )
                .await?;
            }
            SandboxCommand::Linux(mut landlock_cli) => {
                ...
                nexal_cli::debug_sandbox::run_command_under_landlock(
                    landlock_cli,
                    arg0_paths.nexal_linux_sandbox_exe.clone(),
                )
                .await?;
            ...
            SandboxCommand::Windows(mut windows_cli) => {
                ...
                nexal_cli::debug_sandbox::run_command_under_windows(
                    windows_cli,
                    arg0_paths.nexal_linux_sandbox_exe.clone(),
                )
                .await?;
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            ...
            tokio::task::spawn_blocking(move || nexal_responses_api_proxy::run_main(args))
                .await??;
        }
        Some(Subcommand::StdioToUds(cmd)) => {
            ...
            tokio::task::spawn_blocking(move || nexal_stdio_to_uds::run(socket_path.as_path()))
                .await??;
        }
...
async fn enable_feature_in_config(interactive: &TuiCli, feature: &str) -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(&nexal_home)
        .with_profile(interactive.config_profile.as_deref())
        .set_feature_enabled(feature, /*enabled*/ true)
        .apply()
        .await?;
    println!("Enabled feature `{feature}` in config.toml.");
    maybe_print_under_development_feature_warning(&nexal_home, interactive, feature);
    Ok(())
}
...
fn maybe_print_under_development_feature_warning(
    ...
    let config_path = nexal_home.join(nexal_config_loader::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_debug_clear_memories_command(
    ...
    let memory_root = config.nexal_home.join("memories");
...
fn reject_remote_mode_for_subcommand(
    ...
    if let Some(remote) = remote {
        anyhow::bail!(
            "`--remote {remote}` is only supported for interactive TUI commands, not `nexal {subcommand}`"
        );
    }
    if remote_auth_token_env.is_some() {
        anyhow::bail!(
            "`--remote-auth-token-env` is only supported for interactive TUI commands, not `nexal {subcommand}`"
        );
    }
    Ok(())
}
...
async fn run_interactive_tui(
    ...
    let terminal_info = nexal_terminal_detection::terminal_info();
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". Nexal's interactive TUI may not work in this terminal."
        );
    ...
    let use_app_server_tui = nexal_tui::should_use_app_server_tui(&interactive).await?;
    let normalized_remote = remote
        .as_deref()
        .map(nexal_tui_app_server::normalize_remote_addr)
        .transpose()
...
fn into_app_server_tui_cli(cli: TuiCli) -> nexal_tui_app_server::Cli {
...
fn into_legacy_app_exit_info(exit_info: nexal_tui_app_server::AppExitInfo) -> AppExitInfo {
    AppExitInfo {
...
/// Build the final `TuiCli` for a `nexal resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `nexal` without additional flags.
...
/// Build the final `TuiCli` for a `nexal fork` invocation.
...
/// Merge flags provided to `nexal resume`/`nexal fork` so they take precedence over any
...
mod tests {
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        let cli =
            MultitoolCli::try_parse_from(["nexal", "exec", "--json", "resume", "--last", "2+2"])
                .expect("parse should succeed");

        let Some(Subcommand::Exec(exec)) = cli.subcommand else {
            panic!("expected exec subcommand");
        };
        let Some(nexal_exec::Command::Resume(args)) = exec.command else {
    ...
    fn format_exit_messages_includes_resume_hint_without_color() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run nexal resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_prefers_thread_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run nexal resume my-thread".to_string(),
            ]
        );
    }
    ...
    fn resume_model_flag_applies_when_no_root_flags() {
        let interactive =
            finalize_resume_from_args(["nexal", "resume", "-m", "gpt-5.1-test"].as_ref());
    ...
    fn resume_picker_logic_none_and_not_last() {
        let interactive = finalize_resume_from_args(["nexal", "resume"].as_ref());
        assert!(interactive.resume_picker);
        assert!(!interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
    ...
    fn resume_picker_logic_last() {
        let interactive = finalize_resume_from_args(["nexal", "resume", "--last"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
        assert!(!interactive.resume_show_all);
    ...
    fn resume_picker_logic_with_session_id() {
        let interactive = finalize_resume_from_args(["nexal", "resume", "1234"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(!interactive.resume_last);
    ...
    fn resume_all_flag_sets_show_all() {
        let interactive = finalize_resume_from_args(["nexal", "resume", "--all"].as_ref());
        assert!(interactive.resume_picker);
        assert!(interactive.resume_show_all);
    }
    ...
    fn resume_include_non_interactive_flag_sets_source_filter_override() {
        let interactive =
            finalize_resume_from_args(["nexal", "resume", "--include-non-interactive"].as_ref());
    ...
    fn resume_merges_option_flags_and_full_auto() {
        ...
        assert_matches!(
            ...
            Some(nexal_utils_cli::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(nexal_utils_cli::ApprovalModeCliArg::OnRequest)
        );
    ...
    fn fork_picker_logic_none_and_not_last() {
        let interactive = finalize_fork_from_args(["nexal", "fork"].as_ref());
    ...
    fn fork_picker_logic_last() {
        let interactive = finalize_fork_from_args(["nexal", "fork", "--last"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(interactive.fork_last);
        assert_eq!(interactive.fork_session_id, None);
        assert!(!interactive.fork_show_all);
    ...
    fn fork_picker_logic_with_session_id() {
        let interactive = finalize_fork_from_args(["nexal", "fork", "1234"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(!interactive.fork_last);
    ...
    fn fork_all_flag_sets_show_all() {
        let interactive = finalize_fork_from_args(["nexal", "fork", "--all"].as_ref());
        assert!(interactive.fork_picker);
        assert!(interactive.fork_show_all);
    }
    ...
    fn app_server_analytics_default_disabled_without_flag() {
        let app_server = app_server_from_args(["nexal", "app-server"].as_ref());
        assert!(!app_server.analytics_default_enabled);
        assert_eq!(
            app_server.listen,
            nexal_app_server::AppServerTransport::Stdio
        );
    }
    ...
    fn app_server_analytics_default_enabled_with_flag() {
        let app_server =
            app_server_from_args(["nexal", "app-server", "--analytics-default-enabled"].as_ref());
        assert!(app_server.analytics_default_enabled);
    }

    #[test]
    fn remote_flag_parses_for_interactive_root() {
        let cli = MultitoolCli::try_parse_from(["nexal", "--remote", "ws://127.0.0.1:4500"])
            .expect("parse");
    ...
    fn remote_flag_parses_for_resume_subcommand() {
        let cli =
            MultitoolCli::try_parse_from(["nexal", "resume", "--remote", "ws://127.0.0.1:4500"])
                .expect("parse");
    ...
    fn reject_remote_auth_token_env_for_non_interactive_subcommands() {
        let err = reject_remote_mode_for_subcommand(None, Some("NEXAL_REMOTE_AUTH_TOKEN"), "exec")
    ...
    fn read_remote_auth_token_from_env_var_reports_missing_values() {
        let err = read_remote_auth_token_from_env_var_with("NEXAL_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn read_remote_auth_token_from_env_var_trims_values() {
        let auth_token =
            read_remote_auth_token_from_env_var_with("NEXAL_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn app_server_listen_websocket_url_parses() {
        let app_server = app_server_from_args(
            ["nexal", "app-server", "--listen", "ws://127.0.0.1:4500"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            nexal_app_server::AppServerTransport::WebSocket {
    ...
    fn app_server_listen_stdio_url_parses() {
        let app_server =
            app_server_from_args(["nexal", "app-server", "--listen", "stdio://"].as_ref());
        assert_eq!(
            app_server.listen,
            nexal_app_server::AppServerTransport::Stdio
        );
    ...
    fn app_server_listen_invalid_url_fails_to_parse() {
        let parse_result =
            MultitoolCli::try_parse_from(["nexal", "app-server", "--listen", "http://foo"]);
    ...
    fn app_server_capability_token_flags_parse() {
        ...
        assert_eq!(
            ...
            Some(nexal_app_server::WebsocketAuthCliMode::CapabilityToken)
        );
        assert_eq!(
            app_server.auth.ws_token_file,
            Some(PathBuf::from("/tmp/nexal-token"))
        );
    }
    ...
    fn app_server_signed_bearer_flags_parse() {
        ...
        assert_eq!(
            app_server.auth.ws_auth,
            Some(nexal_app_server::WebsocketAuthCliMode::SignedBearerToken)
        );
        assert_eq!(
            app_server.auth.ws_shared_secret_file,
            Some(PathBuf::from("/tmp/nexal-secret"))
        );
    ...
    fn features_enable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["nexal", "features", "enable", "unified_exec"])
            .expect("parse should succeed");
    ...
    fn features_disable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["nexal", "features", "disable", "shell_tool"])
            .expect("parse should succeed");
        let Some(Subcommand::Features(FeaturesCli { sub })) = cli.subcommand else {
            panic!("expected features subcommand");
        };
