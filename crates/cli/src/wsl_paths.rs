...
mod tests {
    ...
    fn win_to_wsl_basic() {
        assert_eq!(
            win_path_to_wsl(r"C:\Temp\nexal.zip").as_deref(),
            Some("/mnt/c/Temp/nexal.zip")
        );
        assert_eq!(
            win_path_to_wsl("D:/Work/nexal.tgz").as_deref(),
            Some("/mnt/d/Work/nexal.tgz")
        );
        assert!(win_path_to_wsl("/home/user/nexal").is_none());
    }
