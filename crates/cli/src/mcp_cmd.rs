...
use nexal_core::config::load_global_mcp_servers;
...
use nexal_core::mcp::auth::McpOAuthLoginSupport;
use nexal_core::mcp::auth::ResolvedMcpOAuthScopes;
use nexal_core::mcp::auth::compute_auth_statuses;
use nexal_core::mcp::auth::discover_supported_scopes;
use nexal_core::mcp::auth::oauth_login_support;
use nexal_core::mcp::auth::resolve_oauth_scopes;
use nexal_core::mcp::auth::should_retry_without_scopes;
use nexal_core::plugins::PluginsManager;
use nexal_protocol::protocol::McpAuthStatus;
use nexal_rmcp_client::delete_oauth_tokens;
use nexal_rmcp_client::perform_oauth_login;
...
/// - `add`    — add a server launcher entry to `~/.nexal/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "nexal mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: nexal_rmcp_client::OAuthCredentialsStoreMode,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let nexal_home = find_nexal_home().context("failed to resolve NEXAL_HOME")?;
    let mut servers = load_global_mcp_servers(&nexal_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", nexal_home.display()))?;
    ...
    ConfigEditsBuilder::new(&nexal_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", nexal_home.display()))?;
    ...
    match oauth_login_support(&transport).await {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `nexal mcp login {name}` to login."
        ),
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `nexal mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: nexal mcp remove {}", get_args.name);
