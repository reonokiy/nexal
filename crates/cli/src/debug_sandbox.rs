...
#[cfg(target_os = "macos")]
use nexal_core::spawn::NEXAL_SANDBOX_ENV_VAR;
...
use nexal_sandboxing::landlock::create_linux_sandbox_command_args_for_policies;
#[cfg(target_os = "macos")]
use nexal_sandboxing::seatbelt::create_seatbelt_command_args_for_policies_with_extensions;
...
pub async fn run_command_under_seatbelt(
    ...
    _nexal_linux_sandbox_exe: Option<PathBuf>,
) -> anyhow::Result<()> {
...
async fn run_command_under_sandbox(
    ...
    if let SandboxType::Windows = sandbox_type {
        #[cfg(target_os = "windows")]
        {
            ...
            use nexal_windows_sandbox::run_windows_sandbox_capture;
            use nexal_windows_sandbox::run_windows_sandbox_capture_elevated;
            ...
            let base_dir = config.nexal_home.clone();
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Seatbelt => {
            ...
            spawn_debug_sandbox_child(
                ...
                env,
                |env_map| {
                    env_map.insert(NEXAL_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
        ...
        SandboxType::Landlock => {
            ...
            let nexal_linux_sandbox_exe = config
                .nexal_linux_sandbox_exe
                .expect("nexal-linux-sandbox executable not found");
...
async fn spawn_debug_sandbox_child(
    ...
    if !network_sandbox_policy.is_enabled() {
        cmd.env(NEXAL_SANDBOX_NETWORK_DISABLED_ENV_VAR, "1");
    }
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_nexal_home(
        cli_overrides,
        nexal_linux_sandbox_exe,
        full_auto,
        /*nexal_home*/ None,
...
async fn load_debug_sandbox_config_with_nexal_home(
    cli_overrides: Vec<(String, TomlValue)>,
    nexal_linux_sandbox_exe: Option<PathBuf>,
    full_auto: bool,
    ...
    let config = build_debug_sandbox_config(
        cli_overrides.clone(),
        ConfigOverrides {
            nexal_linux_sandbox_exe: nexal_linux_sandbox_exe.clone(),
            ..Default::default()
        },
        nexal_home.clone(),
    )
    ...
    if config_uses_permission_profiles(&config) {
        if full_auto {
            anyhow::bail!(
                "`nexal sandbox --full-auto` is only supported for legacy `sandbox_mode` configs; choose a writable `[permissions]` profile instead"
            );
        }
        return Ok(config);
    }
...
async fn build_debug_sandbox_config(
    ...
    if let Some(nexal_home) = nexal_home {
        builder = builder
            .nexal_home(nexal_home.clone())
            .fallback_cwd(Some(nexal_home));
    }
    builder.build().await
}
...
mod tests {
    ...
    fn write_permissions_profile_config(
        ...
        std::fs::write(nexal_home.path().join("config.toml"), config)?;
        Ok(())
    }
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_nexal_home(
            Vec::new(),
            None,
            false,
            Some(nexal_home_path),
        )
        .await?;
    ...
    async fn debug_sandbox_rejects_full_auto_for_permission_profiles() -> anyhow::Result<()> {
        ...
        let err = load_debug_sandbox_config_with_nexal_home(
            Vec::new(),
            None,
            true,
            Some(nexal_home.path().to_path_buf()),
        )
        .await
