...
pub fn sandbox_setup_is_complete(nexal_home: &Path) -> bool {
    nexal_windows_sandbox::sandbox_setup_is_complete(nexal_home)
}

#[cfg(not(target_os = "windows"))]
pub fn sandbox_setup_is_complete(_nexal_home: &Path) -> bool {
    false
}

#[cfg(target_os = "windows")]
pub fn elevated_setup_failure_details(err: &anyhow::Error) -> Option<(String, String)> {
    let failure = nexal_windows_sandbox::extract_setup_failure(err)?;
    let code = failure.code.as_str().to_string();
    let message = nexal_windows_sandbox::sanitize_setup_metric_tag_value(&failure.message);
    Some((code, message))
}
...
pub fn elevated_setup_failure_metric_name(err: &anyhow::Error) -> &'static str {
    if nexal_windows_sandbox::extract_setup_failure(err).is_some_and(|failure| {
        matches!(
            failure.code,
            nexal_windows_sandbox::SetupErrorCode::OrchestratorHelperLaunchCanceled
        )
    }) {
        "nexal.windows_sandbox.elevated_setup_canceled"
    } else {
        "nexal.windows_sandbox.elevated_setup_failure"
    }
}
...
pub fn run_elevated_setup(
    ...
    nexal_windows_sandbox::run_elevated_setup(
        policy,
        policy_cwd,
        command_cwd,
        env_map,
...
pub fn run_legacy_setup_preflight(
    ...
    nexal_windows_sandbox::run_windows_sandbox_legacy_preflight(
        policy,
        policy_cwd,
        nexal_home,
        command_cwd,
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    nexal_windows_sandbox::run_setup_refresh_with_extra_read_roots(
        policy,
        policy_cwd,
        command_cwd,
        env_map,
...
async fn run_windows_sandbox_setup_and_persist(
    ...
    let nexal_home = request.nexal_home;
    let active_profile = request.active_profile;
    let setup_nexal_home = nexal_home.clone();

    let setup_result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
        match mode {
            WindowsSandboxSetupMode::Elevated => {
                if !sandbox_setup_is_complete(setup_nexal_home.as_path()) {
                    run_elevated_setup(
                        &policy,
                        policy_cwd.as_path(),
                        command_cwd.as_path(),
                        &env_map,
                        setup_nexal_home.as_path(),
                    )?;
                }
            }
    ...
    ConfigEditsBuilder::new(nexal_home.as_path())
