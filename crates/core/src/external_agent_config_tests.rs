...
fn fixture_paths() -> (TempDir, PathBuf, PathBuf) {
    ...
    let nexal_home = root.path().join(".nexal");
    (root, claude_home, nexal_home)
}

fn service_for_paths(claude_home: PathBuf, nexal_home: PathBuf) -> ExternalAgentConfigService {
    ExternalAgentConfigService::new_for_test(nexal_home, claude_home)
}

#[test]
fn detect_home_lists_config_skills_and_agents_md() {
    let (_root, claude_home, nexal_home) = fixture_paths();
    let agents_skills = nexal_home
        .parent()
    ...
    let items = service_for_paths(claude_home.clone(), nexal_home.clone())
    ...
    let expected = vec![
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                nexal_home.join("config.toml").display()
            ),
            cwd: None,
        },
        ExternalAgentConfigMigrationItem {
        ...
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                nexal_home.join("AGENTS.md").display()
            ),
            cwd: None,
        },
    ];
...
fn detect_repo_lists_agents_md_for_each_cwd() {
    ...
    let items = service_for_paths(root.path().join(".claude"), root.path().join(".nexal"))
        .detect(ExternalAgentConfigDetectOptions {
            include_home: false,
...
fn import_home_migrates_supported_config_fields_skills_and_agents_md() {
    ...
    fs::write(
            claude_home.join("settings.json"),
            r#"{"model":"claude","permissions":{"ask":["git push"]},"env":{"FOO":"bar","CI":false,"MAX_RETRIES":3,"MY_TEAM":"nexal","IGNORED":null,"LIST":["a","b"],"MAP":{"x":1}},"sandbox":{"enabled":true,"network":{"allowLocalBinding":true}}}"#,
        )
        .expect("write settings");
    fs::write(
    ...
    service_for_paths(claude_home, nexal_home.clone())
        .import(vec![
            ExternalAgentConfigMigrationItem {
                item_type: ExternalAgentConfigMigrationItemType::AgentsMd,
    ...
    assert_eq!(
        fs::read_to_string(nexal_home.join("AGENTS.md")).expect("read agents"),
        "Nexal guidance"
    );

    assert_eq!(
        fs::read_to_string(nexal_home.join("config.toml")).expect("read config"),
        "sandbox_mode = \"workspace-write\"\n\n[shell_environment_policy]\ninherit = \"core\"\n\n[shell_environment_policy.set]\nCI = \"false\"\nFOO = \"bar\"\nMAX_RETRIES = \"3\"\nMY_TEAM = \"nexal\"\n"
    );
    assert_eq!(
        fs::read_to_string(agents_skills.join("skill-a").join("SKILL.md"))
            .expect("read copied skill"),
        "Use Nexal and Nexal utilities."
    );
}
...
fn import_home_skips_empty_config_migration() {
    ...
    assert!(!nexal_home.join("config.toml").exists());
}
...
fn detect_home_skips_config_when_target_already_has_supported_fields() {
    let (_root, claude_home, nexal_home) = fixture_paths();
    fs::create_dir_all(&claude_home).expect("create claude home");
    fs::create_dir_all(&nexal_home).expect("create nexal home");
    fs::write(
        claude_home.join("settings.json"),
    ...
    let items = service_for_paths(claude_home, nexal_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: true,
            cwds: None,
        })
...
fn import_repo_agents_md_rewrites_terms_and_skips_non_empty_targets() {
    ...
    service_for_paths(root.path().join(".claude"), root.path().join(".nexal"))
        .import(vec![
            ExternalAgentConfigMigrationItem {
                item_type: ExternalAgentConfigMigrationItemType::AgentsMd,
                description: String::new(),
...
fn import_skills_returns_only_new_skill_directory_count() {
    ...
    let copied_count = service_for_paths(claude_home, nexal_home)
        .import_skills(None)
