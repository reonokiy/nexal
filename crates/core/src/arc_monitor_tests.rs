    ...
    assert_eq!(
        ...
        ArcMonitorRequest {
            metadata: ArcMonitorMetadata {
                nexal_thread_id: session.conversation_id.to_string(),
                nexal_turn_id: turn_context.sub_id.clone(),
...
async fn monitor_action_uses_env_url_and_token_overrides() {
    ...
    let _url_guard = EnvVarGuard::set(
        NEXAL_ARC_MONITOR_ENDPOINT_OVERRIDE,
        OsStr::new(&format!("{}/override/arc", server.uri())),
    );
    let _token_guard = EnvVarGuard::set(NEXAL_ARC_MONITOR_TOKEN, OsStr::new("override-token"));
