...
use nexal_otel::config::OtelTlsConfig as OtelTlsSettings;
use std::error::Error;
...
/// Filter predicate for exporting only Nexal-owned events via OTEL.
/// Keeps events that originated from nexal_otel module
pub fn nexal_export_filter(meta: &tracing::Metadata<'_>) -> bool {
    meta.target().starts_with("nexal_otel")
}
