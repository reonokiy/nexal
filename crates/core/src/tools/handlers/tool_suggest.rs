...
struct ToolSuggestMeta<'a> {
    nexal_approval_kind: &'static str,
    tool_type: DiscoverableToolType,
...
impl ToolHandler for ToolSuggestHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        if args.tool_type == DiscoverableToolType::Plugin
            && turn.app_server_client_name.as_deref() == Some("nexal-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin tool suggestions are not available in nexal-tui yet".to_string(),
            ));
        }
...
fn build_tool_suggestion_meta<'a>(
    ...
    ToolSuggestMeta {
        nexal_approval_kind: TOOL_SUGGEST_APPROVAL_KIND_VALUE,
        tool_type,
...
async fn verify_tool_suggestion_completed(
    session: &crate::nexal::Session,
    turn: &crate::nexal::TurnContext,
    tool: &DiscoverableTool,
...
async fn refresh_missing_suggested_connectors(
    ...
    match manager.hard_refresh_nexal_apps_tools_cache().await {
        Ok(mcp_tools) => {
            let accessible_connectors = connectors::with_app_enabled_state(
                connectors::accessible_connectors_from_mcp_tools(&mcp_tools),
                &turn.config,
        ...
        Err(err) => {
            warn!(
                "failed to refresh nexal apps tools cache after tool suggestion for {tool_id}: {err:#}"
            );
            None
        }
    }
