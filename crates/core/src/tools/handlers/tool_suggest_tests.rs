...
#[test]
fn filter_tool_suggest_discoverable_tools_for_nexal_tui_omits_plugins() {
    ...
    assert_eq!(
        filter_tool_suggest_discoverable_tools_for_client(discoverable_tools, Some("nexal-tui"),),
...
async fn verified_plugin_suggestion_completed_requires_installed_plugin() {
    ...
    let curated_root = crate::plugins::curated_plugins_repo_path(nexal_home.path());
    write_openai_curated_marketplace(&curated_root, &["sample"]);
    write_curated_plugin_sha(nexal_home.path());
    write_plugins_feature_config(nexal_home.path());

    let config = load_plugins_config(nexal_home.path()).await;
    ...
    let refreshed_config = load_plugins_config(nexal_home.path()).await;
    assert!(verified_plugin_suggestion_completed(
        "sample@openai-curated",
        &refreshed_config,
        &plugins_manager,
