...
use nexal_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use nexal_sandboxing::policy_transforms::merge_permission_profiles;
use nexal_sandboxing::policy_transforms::normalize_additional_permissions;
...
async fn effective_patch_permissions(
    ...
    nexal_protocol::permissions::FileSystemSandboxPolicy,
) {
...
impl ToolHandler for ApplyPatchHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        match nexal_apply_patch::maybe_parse_apply_patch_verified(&command, &cwd) {
            nexal_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            ...
            nexal_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            nexal_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            nexal_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    match nexal_apply_patch::maybe_parse_apply_patch_verified(command, cwd) {
        ...
        nexal_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
