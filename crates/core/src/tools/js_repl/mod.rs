...
pub(crate) const JS_REPL_PRAGMA_PREFIX: &str = "// nexal-js-repl:";
...
impl JsReplManager {
    ...
    async fn start_kernel(
        ...
        env.insert(
            "NEXAL_JS_TMP_DIR".to_string(),
            self.tmp_dir.path().to_string_lossy().to_string(),
        );
        let node_module_dirs_key = "NEXAL_JS_REPL_NODE_MODULE_DIRS";
...
pub(crate) async fn resolve_compatible_node(config_path: Option<&Path>) -> Result<PathBuf, String> {
    let node_path = resolve_node(config_path).ok_or_else(|| {
        "Node runtime not found; install Node or set NEXAL_JS_REPL_NODE_PATH".to_string()
    })?;
...
pub(crate) fn resolve_node(config_path: Option<&Path>) -> Option<PathBuf> {
    if let Some(path) = std::env::var_os("NEXAL_JS_REPL_NODE_PATH") {
