...
#[cfg(not(target_os = "windows"))]
#[test]
fn build_sandbox_command_prefers_configured_nexal_self_exe_for_apply_patch() {
    ...
    let nexal_self_exe = PathBuf::from("/tmp/nexal");

    let command = ApplyPatchRuntime::build_sandbox_command(&request, Some(&nexal_self_exe))
        .expect("build sandbox command");

    assert_eq!(
        command.program,
        nexal_self_exe.to_string_lossy().to_string()
    );
}
