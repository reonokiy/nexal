...
//! `nexal --nexal-run-as-apply-patch`, and runs under the current
...
impl ApplyPatchRuntime {
    ...
    fn build_sandbox_command(
        ...
        let exe = Self::resolve_apply_patch_program(nexal_self_exe)?;
        Ok(Self::build_sandbox_command_with_program(req, exe))
    }

    #[cfg(not(target_os = "windows"))]
    fn resolve_apply_patch_program(nexal_self_exe: Option<&PathBuf>) -> Result<PathBuf, ToolError> {
        if let Some(path) = nexal_self_exe {
            return Ok(path.clone());
        }

        std::env::current_exe()
            .map_err(|e| ToolError::Rejected(format!("failed to determine nexal exe: {e}")))
    }
    ...
    fn build_sandbox_command_with_program(req: &ApplyPatchRequest, exe: PathBuf) -> SandboxCommand {
        SandboxCommand {
            ...
            args: vec![
                NEXAL_CORE_APPLY_PATCH_ARG1.to_string(),
...
impl ToolRuntime<ApplyPatchRequest, ExecToolCallOutput> for ApplyPatchRuntime {
    async fn run(
        ...
        let command = Self::build_sandbox_command(req, &ctx.turn.config.nexal_home)?;
        #[cfg(not(target_os = "windows"))]
        let command = Self::build_sandbox_command(req, ctx.turn.nexal_self_exe.as_ref())?;
