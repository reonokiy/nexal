...
use nexal_shell_command::bash::parse_shell_lc_plain_commands;
use nexal_shell_command::bash::parse_shell_lc_single_command_prefix;
use nexal_shell_escalation::EscalateServer;
use nexal_shell_escalation::EscalationDecision;
use nexal_shell_escalation::EscalationExecution;
use nexal_shell_escalation::EscalationPermissions;
use nexal_shell_escalation::EscalationPolicy;
use nexal_shell_escalation::EscalationSession;
use nexal_shell_escalation::ExecParams;
use nexal_shell_escalation::ExecResult;
use nexal_shell_escalation::Permissions as EscalatedPermissions;
use nexal_shell_escalation::PreparedExec;
use nexal_shell_escalation::ShellCommandExecutor;
use nexal_shell_escalation::Stopwatch;
...
struct CoreShellActionProvider {
    ...
    session: Arc<crate::nexal::Session>,
    turn: Arc<crate::nexal::TurnContext>,
    call_id: String,
...
impl ShellCommandExecutor for CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["NEXAL_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::Nexal(NexalErr::Sandbox(SandboxErr::Timeout {
            output: Box::new(output),
        })));
    }

    if is_likely_sandbox_denied(sandbox, &output) {
        return Err(ToolError::Nexal(NexalErr::Sandbox(SandboxErr::Denied {
            output: Box::new(output),
