...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err, NexalErr::ServerOverloaded));
}
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("nexal_other"),
    );
    headers.insert(
        "x-nexal-other-limit-name",
        http::HeaderValue::from_static("nexal_other"),
    );
    let body = serde_json::json!({
        "error": {
    ...
    let NexalErr::UsageLimitReached(usage_limit) = err else {
        panic!("expected NexalErr::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_extracts_identity_auth_details_from_headers() {
    ...
    let NexalErr::UnexpectedStatus(err) = err else {
        panic!("expected NexalErr::UnexpectedStatus, got {err:?}");
    };
