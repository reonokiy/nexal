...
//!
//! The history is stored at `~/.nexal/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.nexal`.
...
fn history_filepath(config: &Config) -> PathBuf {
    let mut path = config.nexal_home.clone();
...
pub async fn append_entry(text: &str, conversation_id: &ThreadId, config: &Config) -> Result<()> {
    ...
    // Resolve `~/.nexal/history.jsonl` and ensure the parent directory exists.
