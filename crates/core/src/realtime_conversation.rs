...
use nexal_api::RealtimeAudioFrame;
use nexal_api::RealtimeEvent;
use nexal_api::RealtimeEventParser;
use nexal_api::RealtimeSessionConfig;
use nexal_api::RealtimeSessionMode;
use nexal_api::RealtimeWebsocketClient;
use nexal_api::endpoint::realtime_websocket::RealtimeWebsocketEvents;
use nexal_api::endpoint::realtime_websocket::RealtimeWebsocketWriter;
...
use nexal_protocol::protocol::RealtimeHandoffRequested;
use http::HeaderMap;
...
impl RealtimeConversationManager {
    ...
    ) -> NexalResult<(Receiver<RealtimeEvent>, Arc<AtomicBool>)> {
        let previous_state = {
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> NexalResult<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(NexalErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, text: String) -> NexalResult<()> {
        let sender = {
            let guard = self.state.lock().await;
        ...
        sender
            .send(text)
            .await
            .map_err(|_| NexalErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }

    pub(crate) async fn handoff_out(&self, output_text: String) -> NexalResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn handoff_complete(&self) -> NexalResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn shutdown(&self) -> NexalResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> NexalResult<PreparedRealtimeConversationStart> {
...
pub(crate) async fn handle_audio(
    ...
    if let Err(err) = sess.conversation.audio_in(params.frame).await {
        ...
        if sess.conversation.running_state().await.is_some() {
            ...
            send_conversation_error(sess, sub_id, err.to_string(), NexalErrorInfo::BadRequest)
                .await;
        }
    }
}
...
fn realtime_request_headers(
    ...
    let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
        NexalErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
    })?;
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    nexal_error_info: NexalErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,
