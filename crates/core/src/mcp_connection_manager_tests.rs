use super::*;
use nexal_protocol::protocol::GranularApprovalConfig;
...
fn create_test_tool(server_name: &str, tool_name: &str) -> ToolInfo {
    ToolInfo {
        ...
        tool_namespace: if server_name == NEXAL_APPS_MCP_SERVER_NAME {
...
#[test]
fn nexal_apps_tools_cache_is_overwritten_by_last_write() {
    let nexal_home = tempdir().expect("tempdir");
    let cache_context = create_nexal_apps_tools_cache_context(
        nexal_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let tools_gateway_1 = vec![create_test_tool(NEXAL_APPS_MCP_SERVER_NAME, "one")];
    let tools_gateway_2 = vec![create_test_tool(NEXAL_APPS_MCP_SERVER_NAME, "two")];

    write_cached_nexal_apps_tools(&cache_context, &tools_gateway_1);
    let cached_gateway_1 =
        read_cached_nexal_apps_tools(&cache_context).expect("cache entry exists for first write");
    assert_eq!(cached_gateway_1[0].tool_name, "one");

    write_cached_nexal_apps_tools(&cache_context, &tools_gateway_2);
    let cached_gateway_2 =
        read_cached_nexal_apps_tools(&cache_context).expect("cache entry exists for second write");
    assert_eq!(cached_gateway_2[0].tool_name, "two");
}

#[test]
fn nexal_apps_tools_cache_is_scoped_per_user() {
    let nexal_home = tempdir().expect("tempdir");
    let cache_context_user_1 = create_nexal_apps_tools_cache_context(
        nexal_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let cache_context_user_2 = create_nexal_apps_tools_cache_context(
        nexal_home.path().to_path_buf(),
        Some("account-two"),
        Some("user-two"),
    );
    let tools_user_1 = vec![create_test_tool(NEXAL_APPS_MCP_SERVER_NAME, "one")];
    let tools_user_2 = vec![create_test_tool(NEXAL_APPS_MCP_SERVER_NAME, "two")];

    write_cached_nexal_apps_tools(&cache_context_user_1, &tools_user_1);
    write_cached_nexal_apps_tools(&cache_context_user_2, &tools_user_2);

    let read_user_1 =
        read_cached_nexal_apps_tools(&cache_context_user_1).expect("cache entry for user one");
    let read_user_2 =
        read_cached_nexal_apps_tools(&cache_context_user_2).expect("cache entry for user two");
...
#[test]
fn nexal_apps_tools_cache_filters_disallowed_connectors() {
    let nexal_home = tempdir().expect("tempdir");
    let cache_context = create_nexal_apps_tools_cache_context(
        nexal_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let cached = read_cached_nexal_apps_tools(&cache_context).expect("cache entry exists for user");
...
#[test]
fn nexal_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
    let nexal_home = tempdir().expect("tempdir");
    let cache_context = create_nexal_apps_tools_cache_context(
        nexal_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": NEXAL_APPS_TOOLS_CACHE_SCHEMA_VERSION + 1,
        "tools": [create_test_tool(NEXAL_APPS_MCP_SERVER_NAME, "one")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");

    assert!(read_cached_nexal_apps_tools(&cache_context).is_none());
}

#[test]
fn nexal_apps_tools_cache_is_ignored_when_json_is_invalid() {
    let nexal_home = tempdir().expect("tempdir");
    let cache_context = create_nexal_apps_tools_cache_context(
        nexal_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn startup_cached_nexal_apps_tools_loads_from_disk_cache() {
    let nexal_home = tempdir().expect("tempdir");
    let cache_context = create_nexal_apps_tools_cache_context(
        nexal_home.path().to_path_buf(),
        Some("account-one"),
    ...
    write_cached_nexal_apps_tools(&cache_context, &cached_tools);
    ...
    assert_eq!(startup_tools[0].server_name, NEXAL_APPS_MCP_SERVER_NAME);
...
async fn list_all_tools_uses_startup_snapshot_while_client_is_pending() {
    ...
    let tool = tools
        .get("mcp__nexal_apps__calendar_create_event")
        .expect("tool from startup cache");
    assert_eq!(tool.server_name, NEXAL_APPS_MCP_SERVER_NAME);
...
#[test]
fn elicitation_capability_enabled_only_for_nexal_apps() {
    let nexal_apps_capability = elicitation_capability_for_server(NEXAL_APPS_MCP_SERVER_NAME);
    assert!(matches!(
        nexal_apps_capability,
