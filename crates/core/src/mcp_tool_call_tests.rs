...
async fn approval_elicitation_request_uses_message_override_and_preserves_tool_params_keys() {
    ...
    let request = build_mcp_tool_approval_elicitation_request(
        ...
        McpToolApprovalElicitationRequest {
            server: NEXAL_APPS_MCP_SERVER_NAME,
    ...
    assert_eq!(
        ...
        McpServerElicitationRequestParams {
            ...
            server_name: NEXAL_APPS_MCP_SERVER_NAME.to_string(),
...
#[test]
fn nexal_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        NEXAL_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn trusted_nexal_apps_tool_question_offers_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        NEXAL_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn nexal_apps_tool_question_without_elicitation_omits_always_allow() {
    let session_key = McpToolApprovalKey {
        server: NEXAL_APPS_MCP_SERVER_NAME.to_string(),
...
#[test]
fn nexal_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: NEXAL_APPS_MCP_SERVER_NAME.to_string(),
...
#[tokio::test]
async fn nexal_apps_tool_call_request_meta_includes_turn_metadata_and_nexal_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = serde_json::from_str::<serde_json::Value>(
        &turn_context
            .turn_metadata_state
    ...
    let metadata = McpToolApprovalMetadata {
        ...
        nexal_apps_meta: Some(
            ...
            .as_object()
            .cloned()
            .expect("_nexal_apps metadata should be an object"),
        ),
    };
...
#[test]
fn approval_elicitation_meta_includes_connector_source_for_nexal_apps() {
    assert_eq!(
        build_mcp_tool_approval_elicitation_meta(
            NEXAL_APPS_MCP_SERVER_NAME,
            Some(&approval_metadata(
...
#[tokio::test]
async fn persist_nexal_app_tool_approval_writes_tool_override() {
    let tmp = tempdir().expect("tempdir");

    persist_nexal_app_tool_approval(tmp.path(), "calendar", "calendar/list_events")
        .await
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_nexal_dir = project_dir.path().join(".nexal");
    std::fs::create_dir_all(&project_nexal_dir).expect("create project .nexal dir");
    std::fs::write(
        project_nexal_dir.join(CONFIG_TOML_FILE),
        "[mcp_servers.docs]\ncommand = \"docs-server\"\n",
    )
    ...
    ConfigEditsBuilder::new(&nexal_home)
        .set_project_trust_level(
            project_dir.path(),
            nexal_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
    ...
    let contents = std::fs::read_to_string(project_nexal_dir.join(CONFIG_TOML_FILE))
...
async fn approve_mode_blocks_when_arc_returns_interrupt_for_model() {
    ...
    turn_context.auth_manager = Some(crate::test_support::auth_manager_from_auth(
        crate::NexalAuth::create_dummy_chatgpt_auth_for_testing(),
    ));
