...
use nexal_protocol::models::FunctionCallOutputBody;
...
#[path = "nexal_tests_guardian.rs"]
mod guardian_tests;

use nexal_protocol::models::function_call_output_content_items_to_text;
...
fn make_mcp_tool(
    ...
    let tool_namespace = if server_name == NEXAL_APPS_MCP_SERVER_NAME {
        connector_name
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let nexal_home = session.nexal_home().await;
    std::fs::create_dir_all(&nexal_home).expect("create nexal home");
    let config_toml_path = nexal_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
...
fn non_app_mcp_tools_remain_visible_without_search_selection() {
    let mcp_tools = HashMap::from([
        (
            "mcp__nexal_apps__calendar_create_event".to_string(),
            make_mcp_tool(
...
#[test]
fn search_tool_selection_keeps_nexal_apps_tools_without_mentions() {
...
#[test]
fn apps_mentions_add_nexal_apps_tools_to_search_selected_set() {
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_nexal().with_config(|config| {
        config.permissions.approval_policy =
            nexal_config_loader::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    initial
        ...
        .await?;
    wait_for_event(&initial.nexal, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    fork_config.permissions.approval_policy =
        nexal_config_loader::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "nexal_core__nexal_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let rollout_items = vec![
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            nexal_protocol::protocol::UserMessageEvent {
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            nexal_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_rollout_path() {
    ...
    assert_eq!(
        error_event.nexal_error_info,
        Some(NexalErrorInfo::ThreadRollbackFailed)
    );
...
async fn set_rate_limits_retains_previous_credits() {
    ...
    let config = build_test_config(nexal_home.path()).await;
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(nexal_protocol::account::PlanType::Plus),
    };
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        secondary: None,
        credits: None,
        plan_type: Some(nexal_protocol::account::PlanType::Pro),
    };
    state.set_rate_limits(update.clone());
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.nexal_error_info == Some(NexalErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn build_test_config(nexal_home: &Path) -> Config {
...
async fn session_configuration_apply_preserves_split_file_system_policy_on_cwd_only_update() {
    ...
    session_configuration.sandbox_policy =
        nexal_config_loader::Constrained::allow_any(SandboxPolicy::WorkspaceWrite {
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let nexal_home = parent_config.nexal_home.clone();
    let skill_dir = nexal_home.join("skills").join("demo");
    ...
    let role_path = nexal_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_zsh_path() {
    let nexal_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(nexal_home.path()).await;
    config
        .features
    ...
    let plugins_manager = Arc::new(PluginsManager::new(config.nexal_home.clone()));
    let mcp_manager = Arc::new(McpManager::new(Arc::clone(&plugins_manager)));
    let skills_manager = Arc::new(SkillsManager::new(config.nexal_home.clone(), true));
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let environment = Arc::new(
        nexal_exec_server::Environment::create(/*exec_server_url*/ None)
            .await
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            nexal_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(nexal_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = nexal_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(nexal_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            session
                .request_permissions(
                    turn_context.as_ref(),
                    call_id,
                    nexal_protocol::request_permissions::RequestPermissionsArgs {
...
async fn shutdown_and_wait_allows_multiple_waiters() {
    ...
    let waiter_1 = {
        let nexal = Arc::clone(&nexal);
        tokio::spawn(async move { nexal.shutdown_and_wait().await })
    };
    let waiter_2 = {
        let nexal = Arc::clone(&nexal);
        tokio::spawn(async move { nexal.shutdown_and_wait().await })
...
async fn shutdown_and_wait_shuts_down_tracked_ephemeral_guardian_review() {
    ...
    parent_session
        .guardian_review_session
        .register_ephemeral_for_test(child_nexal)
        .await;
...
async fn handle_output_item_done_records_image_save_history_message() {
    ...
    let expected_saved_path = crate::stream_events_utils::image_generation_artifact_path(
        turn_context.config.nexal_home.as_path(),
