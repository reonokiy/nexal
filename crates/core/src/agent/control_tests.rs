...
impl AgentControlHarness {
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<NexalThread>) {
        let new_thread = self
            .manager
...
async fn wait_for_subagent_notification(parent_thread: &Arc<NexalThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<NexalThread>, message: &str) {
    thread
...
async fn send_input_errors_when_thread_missing() {
    ...
    assert_matches!(err, NexalErr::ThreadNotFound(id) if id == thread_id);
}
...
async fn spawn_agent_can_fork_parent_thread_history() {
    ...
    parent_thread
        ...
        .await;
    let turn_context = parent_thread.nexal.session.new_default_turn().await;
    ...
    let history = child_thread.nexal.session.clone_history().await;
...
async fn spawn_agent_respects_max_threads_limit() {
    ...
    let NexalErr::AgentLimitReached {
        max_threads: seen_max_threads,
    } = err
    else {
        panic!("expected NexalErr::AgentLimitReached");
    };
...
async fn spawn_agent_limit_shared_across_clones() {
    ...
    let NexalErr::AgentLimitReached { max_threads } = err else {
        panic!("expected NexalErr::AgentLimitReached");
    };
...
async fn multi_agent_v2_completion_ignores_dead_direct_parent() {
    ...
    let tester_turn = tester_thread.nexal.session.new_default_turn().await;
    tester_thread
        .nexal
        .session
        .send_event(
