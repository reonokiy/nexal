...
const MCP_CALL_COUNT_METRIC: &str = "nexal.mcp.call";
const MCP_CALL_DURATION_METRIC: &str = "nexal.mcp.call.duration_ms";
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == NEXAL_APPS_MCP_SERVER_NAME {
    ...
    let approval_mode = if server == NEXAL_APPS_MCP_SERVER_NAME {
        app_tool_policy.approval
    } else {
        custom_mcp_tool_approval_mode(turn_context.as_ref(), &server, &tool_name)
    };

    if server == NEXAL_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    nexal_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
}

const MCP_TOOL_NEXAL_APPS_META_KEY: &str = "_nexal_apps";
...
fn build_mcp_tool_call_request_meta(
    ...
    if server == NEXAL_APPS_MCP_SERVER_NAME
        && let Some(nexal_apps_meta) =
            metadata.and_then(|metadata| metadata.nexal_apps_meta.clone())
    {
        request_meta.insert(
            MCP_TOOL_NEXAL_APPS_META_KEY.to_string(),
            serde_json::Value::Object(nexal_apps_meta),
        );
    }
...
pub(crate) const MCP_TOOL_APPROVAL_DECLINE_SYNTHETIC: &str = "__nexal_mcp_decline__";
const MCP_TOOL_APPROVAL_ACCEPT_AND_REMEMBER: &str = "Allow and don't ask me again";
const MCP_TOOL_APPROVAL_CANCEL: &str = "Cancel";
const MCP_TOOL_APPROVAL_KIND_KEY: &str = "nexal_approval_kind";
...
fn session_mcp_tool_approval_key(
    ...
    if invocation.server == NEXAL_APPS_MCP_SERVER_NAME && connector_id.is_none() {
        return None;
    }
...
pub(crate) async fn lookup_mcp_tool_metadata(
    ...
    let connector_description = if server == NEXAL_APPS_MCP_SERVER_NAME {
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == NEXAL_APPS_MCP_SERVER_NAME {
        let Some(connector_id) = key.connector_id.clone() else {
            remember_mcp_tool_approval(sess, key).await;
            return;
        };
        persist_nexal_app_tool_approval(&turn_context.config.nexal_home, &connector_id, &tool_name)
            .await
    } else {
...
async fn persist_nexal_app_tool_approval(
    nexal_home: &Path,
    connector_id: &str,
    tool_name: &str,
) -> anyhow::Result<()> {
...
async fn persist_custom_mcp_tool_approval(
        ...
        project_mcp_tool_approval_config_folder(config, server)
    {
        ...
        let servers = load_global_mcp_servers(&config.nexal_home).await?;
