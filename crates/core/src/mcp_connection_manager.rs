...
//!
//! The [`McpConnectionManager`] owns one [`nexal_rmcp_client::RmcpClient`] per
...
use nexal_config_loader::Constrained;
use nexal_protocol::approvals::ElicitationRequest;
...
use nexal_protocol::protocol::McpStartupFailure;
...
use nexal_rmcp_client::RmcpClient;
use nexal_rmcp_client::SendElicitation;
...
use crate::nexal::INITIAL_SUBMIT_ID;
    ...
    I: IntoIterator<Item = ToolInfo>,
{
    ...
    for tool in tools {
        let qualified_name_raw = if tool.server_name != NEXAL_APPS_MCP_SERVER_NAME {
            format!(
                "mcp{}{}{}{}",
...
struct ManagedClient {
    ...
    nexal_apps_tools_cache_context: Option<NexalAppsToolsCacheContext>,
}
...
impl AsyncManagedClient {
    ...
    fn new(
        ...
        let startup_snapshot = load_startup_cached_nexal_apps_tools_snapshot(
            &server_name,
            ...
            let outcome = async {
                ...
                match start_server_task(
                    ...
                    StartServerTaskParams {
                        ...
                        nexal_apps_tools_cache_context,
                    },
                )
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        nexal_home: PathBuf,
        nexal_apps_tools_cache_key: NexalAppsToolsCacheKey,
        ...
        for (server_name, cfg) in mcp_servers.into_iter().filter(|(_, cfg)| cfg.enabled) {
            ...
            .await;
            let nexal_apps_tools_cache_context = if server_name == NEXAL_APPS_MCP_SERVER_NAME {
                Some(NexalAppsToolsCacheContext {
                    nexal_home: nexal_home.clone(),
                    user_key: nexal_apps_tools_cache_key.clone(),
                })
            } else {
                None
            };
    ...
    /// Force-refresh nexal apps tools by bypassing the in-process cache.
    ///
    ...
    pub async fn hard_refresh_nexal_apps_tools_cache(&self) -> Result<HashMap<String, ToolInfo>> {
        let managed_client = self
            .clients
            .get(NEXAL_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{NEXAL_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
            .context("failed to get client")?;
...
pub(crate) fn filter_non_nexal_apps_mcp_tools_only(
    mcp_tools: &HashMap<String, ToolInfo>,
) -> HashMap<String, ToolInfo> {
    mcp_tools
        .iter()
        .filter(|(_, tool)| tool.server_name != NEXAL_APPS_MCP_SERVER_NAME)
        .map(|(name, tool)| (name.clone(), tool.clone()))
        .collect()
}

fn normalize_nexal_apps_tool_title(
    server_name: &str,
...
fn normalize_nexal_apps_tool_name(
    server_name: &str,
    tool_name: &str,
...
fn normalize_nexal_apps_namespace(server_name: &str, connector_name: Option<&str>) -> String {
    if server_name != NEXAL_APPS_MCP_SERVER_NAME {
...
async fn start_server_task(
    ...
    let params = InitializeRequestParams {
        ...
        client_info: Implementation {
            name: "nexal-mcp-client".to_owned(),
            version: env!("CARGO_PKG_VERSION").to_owned(),
            title: Some("Nexal".into()),
            description: None,
            icons: None,
            website_url: None,
        },
...
fn write_cached_nexal_apps_tools_if_needed(
    server_name: &str,
    cache_context: Option<&NexalAppsToolsCacheContext>,
    tools: &[ToolInfo],
) {
    if server_name != NEXAL_APPS_MCP_SERVER_NAME {
        return;
...
fn load_startup_cached_nexal_apps_tools_snapshot(
    server_name: &str,
    cache_context: Option<&NexalAppsToolsCacheContext>,
) -> Option<Vec<ToolInfo>> {
    if server_name != NEXAL_APPS_MCP_SERVER_NAME {
    ...
    match load_cached_nexal_apps_tools(cache_context) {
        CachedNexalAppsToolsLoad::Hit(tools) => Some(tools),
        CachedNexalAppsToolsLoad::Missing | CachedNexalAppsToolsLoad::Invalid => None,
    }
}

#[cfg(test)]
fn read_cached_nexal_apps_tools(
    cache_context: &NexalAppsToolsCacheContext,
) -> Option<Vec<ToolInfo>> {
    match load_cached_nexal_apps_tools(cache_context) {
        CachedNexalAppsToolsLoad::Hit(tools) => Some(tools),
        CachedNexalAppsToolsLoad::Missing | CachedNexalAppsToolsLoad::Invalid => None,
...
fn load_cached_nexal_apps_tools(
    cache_context: &NexalAppsToolsCacheContext,
) -> CachedNexalAppsToolsLoad {
    let cache_path = cache_context.cache_path();
    let bytes = match std::fs::read(cache_path) {
        Ok(bytes) => bytes,
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
            return CachedNexalAppsToolsLoad::Missing;
        }
        Err(_) => return CachedNexalAppsToolsLoad::Invalid,
    };
    let cache: NexalAppsToolsDiskCache = match serde_json::from_slice(&bytes) {
        Ok(cache) => cache,
        Err(_) => return CachedNexalAppsToolsLoad::Invalid,
    };
    if cache.schema_version != NEXAL_APPS_TOOLS_CACHE_SCHEMA_VERSION {
        return CachedNexalAppsToolsLoad::Invalid;
    }
    CachedNexalAppsToolsLoad::Hit(filter_disallowed_nexal_apps_tools(cache.tools))
}

fn write_cached_nexal_apps_tools(cache_context: &NexalAppsToolsCacheContext, tools: &[ToolInfo]) {
    let cache_path = cache_context.cache_path();
    ...
    let tools = filter_disallowed_nexal_apps_tools(tools.to_vec());
    let Ok(bytes) = serde_json::to_vec_pretty(&NexalAppsToolsDiskCache {
        schema_version: NEXAL_APPS_TOOLS_CACHE_SCHEMA_VERSION,
        tools,
    }) else {
        return;
    };
...
fn filter_disallowed_nexal_apps_tools(tools: Vec<ToolInfo>) -> Vec<ToolInfo> {
    tools
        .into_iter()
        .filter(|tool| {
            tool.connector_id
...
async fn list_tools_for_client_uncached(
    ...
    let tools = resp
        ...
        .into_iter()
        .map(|tool| {
            let tool_name = normalize_nexal_apps_tool_name(
                server_name,
                &tool.tool.name,
            ...
            let tool_namespace =
                normalize_nexal_apps_namespace(server_name, tool.connector_name.as_deref());
            let connector_name = tool.connector_name;
            let connector_description = tool.connector_description;
            let mut tool_def = tool.tool;
            if let Some(title) = tool_def.title.as_deref() {
                let normalized_title =
                    normalize_nexal_apps_tool_title(server_name, connector_name.as_deref(), title);
        ...
        .collect();
    if server_name == NEXAL_APPS_MCP_SERVER_NAME {
        return Ok(filter_disallowed_nexal_apps_tools(tools));
    }
    Ok(tools)
}
...
fn mcp_init_error_display(
    ...
    {
        format!(
            "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = NEXAL_GITHUB_PERSONAL_ACCESS_TOKEN"
        )
    } else if is_mcp_client_auth_required_error(err) {
        format!(
            "The {server_name} MCP server is not logged in. Run `nexal mcp login {server_name}`."
        )
