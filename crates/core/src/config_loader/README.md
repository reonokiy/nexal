# `nexal-core` config loader

This module is the canonical place to **load and describe Nexal configuration layers** (user config, CLI/session overrides, managed config, and MDM-managed preferences) and to produce:
...
Exported from `nexal_core::config_loader`:

- `load_config_layers_state(nexal_home, cwd_opt, cli_overrides, overrides, cloud_requirements) -> ConfigLayerStack`
- `ConfigLayerStack`
...
```rust
use nexal_core::config_loader::{load_config_layers_state, LoaderOverrides};
