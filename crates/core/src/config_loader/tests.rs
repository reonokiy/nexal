...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .nexal_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = nexal_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        nexal_config_loader::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
fn schema_error_points_to_feature_value() {
    ...
    let _guard = nexal_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = nexal_config_loader::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_user_layer()
        .expect("expected a user layer even when NEXAL_HOME/config.toml does not exist");
    assert_eq!(
        &ConfigLayerEntry {
...
async fn load_config_layers_includes_cloud_requirements() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let nexal_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&nexal_home).await?;
    ...
    let err = load_config_layers_state(
        ...
        CloudRequirementsLoader::new(async {
            Err(CloudRequirementsLoadError::new(
                nexal_config_loader::CloudRequirementsLoadErrorCode::RequestFailed,
                None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".nexal")).await?;
    tokio::fs::create_dir_all(project_root.join(".nexal")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".nexal").join(CONFIG_TOML_FILE),
        "foo = \"root\"\n",
    )
    .await?;
    tokio::fs::write(
        nested.join(".nexal").join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;
    ...
    let nexal_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&nexal_home).await?;
    make_config_for_test(&nexal_home, &project_root, TrustLevel::Trusted, None).await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            super::ConfigLayerSource::Project { dot_nexal_folder } => Some(dot_nexal_folder),
            _ => None,
        })
        .collect();
    assert_eq!(project_layers.len(), 2);
    assert_eq!(project_layers[0].as_path(), nested.join(".nexal").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".nexal").as_path()
    );
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_nexal_and_override_in_order() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    tokio::fs::write(project_root.join(".nexal").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".nexal").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".nexal").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".nexal").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let nexal_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&nexal_home).await?;
    tokio::fs::write(nexal_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_nexal_exists_without_config_toml() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: super::ConfigLayerSource::Project {
                dot_nexal_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".nexal"))?,
            },
            config: TomlValue::Table(toml::map::Map::new()),
            raw_toml: None,
            version: version_for_toml(&TomlValue::Table(toml::map::Map::new())),
...
#[tokio::test]
async fn nexal_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let nexal_home = home_dir.join(".nexal");
    tokio::fs::create_dir_all(&nexal_home).await?;
    tokio::fs::write(nexal_home.join(CONFIG_TOML_FILE), "foo = \"user\"\n").await?;
...
#[tokio::test]
async fn nexal_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_nexal = project_root.join(".nexal");
    let nested_dot_nexal = nested.join(".nexal");

    tokio::fs::create_dir_all(&nested_dot_nexal).await?;
    tokio::fs::create_dir_all(project_root.join(".git")).await?;
    tokio::fs::write(nested_dot_nexal.join(CONFIG_TOML_FILE), "foo = \"child\"\n").await?;

    tokio::fs::create_dir_all(&project_dot_nexal).await?;
    make_config_for_test(&project_dot_nexal, &project_root, TrustLevel::Trusted, None).await?;
    let user_config_path = project_dot_nexal.join(CONFIG_TOML_FILE);
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: super::ConfigLayerSource::Project {
                dot_nexal_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_nexal)?,
            },
...
async fn project_layers_disabled_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    let nexal_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&nexal_home_untrusted).await?;
    make_config_for_test(
        &nexal_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        None,
    )
    .await?;
    let untrusted_config_path = nexal_home_untrusted.join(CONFIG_TOML_FILE);
    ...
    let nexal_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&nexal_home_unknown).await?;
    tokio::fs::write(
        nexal_home_unknown.join(CONFIG_TOML_FILE),
        "foo = \"user\"\n",
    )
    .await?;
...
-> std::io::Result<()> {
    ...
    let nexal_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&nested).await?;
    tokio::fs::create_dir_all(&dot_nexal).await?;
    tokio::fs::create_dir_all(&nexal_home).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(
        dot_nexal.join(CONFIG_TOML_FILE),
        r#"
[mcp_servers.sentry]
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".nexal")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".nexal").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let nexal_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&nexal_home).await?;
        let config_path = nexal_home.join(CONFIG_TOML_FILE);

        if let Some(trust_level) = trust_level {
            make_config_for_test(&nexal_home, &project_root, trust_level, None).await?;
...
mod requirements_exec_policy_tests {
    ...
    use nexal_config_loader::RequirementsExecPolicyDecisionToml;
    use nexal_config_loader::RequirementsExecPolicyParseError;
    ...
    fn config_stack_for_dot_nexal_folder_with_requirements(
        dot_nexal_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_nexal_folder = AbsolutePathBuf::from_absolute_path(dot_nexal_folder)
            .expect("absolute dot_nexal_folder");
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_nexal_folder_with_requirements(temp_dir.path(), requirements);
