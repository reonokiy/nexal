...
use nexal_config_loader::ConfigRequirementsWithSources;
...
pub use nexal_config_loader::AppsRequirementsToml;
pub use nexal_config_loader::CloudRequirementsLoadError;
pub use nexal_config_loader::CloudRequirementsLoadErrorCode;
pub use nexal_config_loader::CloudRequirementsLoader;
pub use nexal_config_loader::ConfigError;
pub use nexal_config_loader::ConfigLayerEntry;
pub use nexal_config_loader::ConfigLayerStack;
pub use nexal_config_loader::ConfigLayerStackOrdering;
pub use nexal_config_loader::ConfigLoadError;
pub use nexal_config_loader::ConfigRequirements;
pub use nexal_config_loader::ConfigRequirementsToml;
pub use nexal_config_loader::ConstrainedWithSource;
pub use nexal_config_loader::FeatureRequirementsToml;
pub use nexal_config_loader::LoaderOverrides;
pub use nexal_config_loader::McpServerIdentity;
pub use nexal_config_loader::McpServerRequirement;
pub use nexal_config_loader::NetworkConstraints;
pub use nexal_config_loader::NetworkRequirementsToml;
pub use nexal_config_loader::RequirementSource;
pub use nexal_config_loader::ResidencyRequirement;
pub use nexal_config_loader::SandboxModeRequirement;
pub use nexal_config_loader::Sourced;
pub use nexal_config_loader::TextPosition;
pub use nexal_config_loader::TextRange;
pub use nexal_config_loader::WebSearchModeRequirement;
pub(crate) use nexal_config_loader::build_cli_overrides_layer;
pub(crate) use nexal_config_loader::config_error_from_toml;
pub use nexal_config_loader::default_project_root_markers;
pub use nexal_config_loader::format_config_error;
pub use nexal_config_loader::format_config_error_with_source;
pub(crate) use nexal_config_loader::io_error_from_config_error;
pub use nexal_config_loader::merge_toml_values;
pub use nexal_config_loader::project_root_markers_from_config;
#[cfg(test)]
pub(crate) use nexal_config_loader::version_for_toml;
...
pub(crate) async fn first_layer_config_error_from_entries(
    ...
    nexal_config_loader::first_layer_config_error_from_entries::<ConfigToml>(layers, CONFIG_TOML_FILE)
        .await
}
...
/// - system    `/etc/nexal/requirements.toml` (Unix) or
///   `%ProgramData%\OpenAI\Nexal\requirements.toml` (Windows)
///
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/nexal/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\Nexal\config.toml` (Windows)
/// - user      `${NEXAL_HOME}/config.toml`
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.nexal/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.nexal/config.toml` (loaded but disabled when untrusted)
/// - runtime   e.g., --config flags, model selector in UI
///
/// (*) Only available on macOS via managed device profiles.
///
/// See https://developers.openai.com/nexal/security for details.
///
...
pub async fn load_config_layers_state(
    ...
    let loaded_config_layers = layer_io::load_config_layers_internal(nexal_home, overrides).await?;
    ...
    // Add a layer for $NEXAL_HOME/config.toml if it exists. Note if the file
    // exists, but is malformed, then this error should be propagated to the
    // user.
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, nexal_home)?;
...
fn project_layer_entry(
    ...
    let source = ConfigLayerSource::Project {
        dot_nexal_folder: dot_nexal_folder.clone(),
    };
...
async fn load_project_layers(
    ...
    let nexal_home_abs = AbsolutePathBuf::from_absolute_path(nexal_home)?;
    let nexal_home_normalized =
        normalize_path(nexal_home_abs.as_path()).unwrap_or_else(|_| nexal_home_abs.to_path_buf());
    let mut dirs = cwd
        .as_path()
        .ancestors()
        .scan(false, |done, a| {
    ...
    for dir in dirs {
        let dot_nexal = dir.join(".nexal");
        if !tokio::fs::metadata(&dot_nexal)
            .await
        ...
        let dot_nexal_abs = AbsolutePathBuf::from_absolute_path(&dot_nexal)?;
        let dot_nexal_normalized =
            normalize_path(dot_nexal_abs.as_path()).unwrap_or_else(|_| dot_nexal_abs.to_path_buf());
        if dot_nexal_abs == nexal_home_abs || dot_nexal_normalized == nexal_home_normalized {
            continue;
        }
        let config_file = dot_nexal_abs.join(CONFIG_TOML_FILE)?;
        match tokio::fs::read_to_string(&config_file).await {
            Ok(contents) => {
                ...
                let config =
                    resolve_relative_paths_in_config_toml(config, dot_nexal_abs.as_path())?;
...
/// from a file like `/etc/nexal/managed_config.toml` that has the same
...
impl From<LegacyManagedConfigToml> for ConfigRequirementsToml {
    fn from(legacy: LegacyManagedConfigToml) -> Self {
        ...
        if let Some(sandbox_mode) = sandbox_mode {
            ...
            // Allowing read-only is a requirement for Nexal to function correctly.
