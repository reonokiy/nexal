...
pub(super) async fn run(session: &Arc<Session>, config: Arc<Config>) {
    ...
    let root = memory_root(&config.nexal_home);
...
fn artifact_memories_for_phase2(
    selection: &nexal_rollout_state::Phase2InputSelection,
) -> Vec<Stage1Output> {
...
mod job {
    ...
    pub(super) async fn claim(
        ...
        let (token, watermark) = match claim {
            nexal_rollout_state::Phase2JobClaimOutcome::Claimed {
                ownership_token,
                input_watermark,
            } => {
            ...
            nexal_rollout_state::Phase2JobClaimOutcome::SkippedNotDirty => return Err("skipped_not_dirty"),
            nexal_rollout_state::Phase2JobClaimOutcome::SkippedRunning => return Err("skipped_running"),
        };
    ...
    pub(super) async fn succeed(
        ...
        selected_outputs: &[nexal_rollout_state::Stage1Output],
        reason: &'static str,
    ) {
        session.services.session_telemetry.counter(
            metrics::MEMORY_PHASE_TWO_JOBS,
...
mod agent {
    ...
    pub(super) fn get_config(config: Arc<Config>) -> Option<Config> {
        ...
        match AbsolutePathBuf::from_absolute_path(root) {
            ...
            Err(err) => {
                warn!(
                    "memory phase-2 consolidation could not set cwd from nexal_home {}: {err}",
                    agent_config.nexal_home.display()
                );
                return None;
            }
        }
        ...
        match AbsolutePathBuf::from_absolute_path(agent_config.nexal_home.clone()) {
            Ok(nexal_home) => writable_roots.push(nexal_home),
            Err(err) => warn!(
                "memory phase-2 consolidation could not add nexal_home writable root {}: {err}",
                agent_config.nexal_home.display()
            ),
        }
        // The consolidation agent only needs local nexal_home write access and no network.
...
pub(super) fn get_watermark(
    ...
    latest_memories: &[nexal_rollout_state::Stage1Output],
) -> i64 {
    latest_memories
        .iter()
