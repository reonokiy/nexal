...
use nexal_rollout_state::Stage1Output;
...
fn memory_root_uses_shared_global_path() {
    ...
    let nexal_home = dir.path().join("nexal");
    assert_eq!(memory_root(&nexal_home), nexal_home.join("memories"));
}
...
keywords: nexal-rollout-state, migrations
---
### Task 1: migration-test
task: add-migration-test
task_group: nexal-rollout-state
...
- Confirmed no ordering regressions."
    ...
    assert!(raw_memories.contains("task_group: nexal-rollout-state"));
...
mod phase2 {
    ...
    use nexal_rollout_state::Phase2JobClaimOutcome;
    use nexal_rollout_state::Stage1Output;
    ...
    struct DispatchHarness {
        ...
        state_db: Arc<nexal_rollout_state::StateRuntime>,
    }

    impl DispatchHarness {
        async fn new() -> Self {
            let nexal_home = tempfile::tempdir().expect("create temp nexal home");
            let mut config = test_config();
            config.nexal_home = nexal_home.path().to_path_buf();
            config.cwd = config.nexal_home.abs();
            let config = Arc::new(config);

            let state_db = nexal_rollout_state::StateRuntime::init(
    ...
    async fn dispatch_reclaims_stale_global_lock_and_starts_consolidation() {
        ...
        pretty_assertions::assert_eq!(config_snapshot.cwd, memory_root(&harness.config.nexal_home));
        match config_snapshot.sandbox_policy {
            SandboxPolicy::WorkspaceWrite { writable_roots, .. } => {
                assert!(
                    writable_roots
                        .iter()
                        .any(|root| root.as_path() == harness.config.nexal_home.as_path()),
                    "consolidation subagent should have nexal_home as writable root"
                );
            }
            other => panic!("unexpected sandbox policy: {other:?}"),
        }
        subagent.nexal.session.ensure_rollout_materialized().await;
        subagent.nexal.session.flush_rollout().await;
    ...
    async fn dispatch_with_empty_stage1_outputs_rebuilds_local_artifacts() {
        let harness = DispatchHarness::new().await;
        let root = memory_root(&harness.config.nexal_home);
    ...
    async fn dispatch_marks_job_for_retry_when_spawn_agent_fails() {
        ...
        let mut metadata_builder = ThreadMetadataBuilder::new(
            thread_id,
            config.nexal_home.join(format!("rollout-{thread_id}.jsonl")),
            Utc::now(),
            SessionSource::Cli,
        );
