...
pub async fn list_accessible_connectors_from_mcp_tools_with_options_and_status(
    ...
    let refreshed_tools = if force_refetch {
        match mcp_connection_manager
            .hard_refresh_nexal_apps_tools_cache()
            .await
        {
            Ok(tools) => Some(tools),
            Err(err) => {
                warn!(
                    "failed to force-refresh tools for MCP server '{NEXAL_APPS_MCP_SERVER_NAME}', using cached/startup tools: {err:#}"
                );
                None
            }
        }
    ...
    let nexal_apps_ready = if refreshed_tools_succeeded {
        true
    } else if let Some(cfg) = mcp_servers.get(NEXAL_APPS_MCP_SERVER_NAME) {
        let immediate_ready = mcp_connection_manager
            .wait_for_server_ready(NEXAL_APPS_MCP_SERVER_NAME, Duration::ZERO)
            .await;
        if immediate_ready {
            true
            ...
            let ready = mcp_connection_manager
                .wait_for_server_ready(NEXAL_APPS_MCP_SERVER_NAME, timeout)
                .await;
    ...
    if nexal_apps_ready || !accessible_connectors.is_empty() {
        write_cached_accessible_connectors(cache_key, &accessible_connectors);
    }
    let accessible_connectors =
...
pub(crate) fn nexal_app_tool_is_enabled(
    config: &Config,
    tool_info: &crate::mcp_connection_manager::ToolInfo,
) -> bool {
    if tool_info.server_name != NEXAL_APPS_MCP_SERVER_NAME {
        return true;
    }
...
fn plugin_app_to_app_info(connector_id: AppConnectorId) -> AppInfo {
    ...
    // connector_name values from nexal_apps tool discovery.
