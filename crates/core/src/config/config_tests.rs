...
use nexal_protocol::permissions::FileSystemSpecialPath;
...
fn default_permissions_profile_populates_runtime_sandbox_policy() -> std::io::Result<()> {
    ...
    let memories_root = nexal_home.path().join("memories").abs();
    assert_eq!(
...
fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of Nexal and will be ignored."
        )),
        "{:?}",
...
fn permissions_profiles_allow_unknown_special_paths_with_nested_entries() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of Nexal and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `workspace` does not define any recognized filesystem entries for this version of Nexal."
        )),
        "{:?}",
        config.startup_warnings
    );
...
#[test]
fn sqlite_home_defaults_to_nexal_home_for_workspace_write() -> std::io::Result<()> {
    let nexal_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite_home, nexal_home.path().to_path_buf());
...
fn workspace_write_always_includes_memories_root_once() -> std::io::Result<()> {
    let nexal_home = TempDir::new()?;
    let memories_root = nexal_home.path().join("memories");
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
...
"#,
    ...
    let project_config_dir = workspace.path().join(".nexal");
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let nexal_home = TempDir::new()?;
    let managed_path = nexal_home.path().join("managed_config.toml");
    let config_path = nexal_home.path().join(CONFIG_TOML_FILE);
    ...
    .await?;
    let cfg =
        deserialize_config_toml_with_base(config_layer_stack.effective_config(), nexal_home.path())
            .map_err(|e| {
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(nexal_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    ...
    let loaded = load_global_mcp_servers(nexal_home.path()).await?;
...
"#,
    ...
    let err = load_global_mcp_servers(nexal_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/nexal-mcp");
    let servers = BTreeMap::from([(
        "docs".to_string(),
        McpServerConfig {
            transport: McpServerTransportConfig::Stdio {
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/nexal-mcp""#),
    ...
    match &docs.transport {
        McpServerTransportConfig::Stdio { cwd, .. } => {
            assert_eq!(cwd.as_deref(), Some(Path::new("/tmp/nexal-mcp")));
        }
        other => panic!("unexpected transport {other:?}"),
    }
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(nexal_home.path())
        .set_model(Some("gpt-5.1-nexal"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized = tokio::fs::read_to_string(nexal_home.path().join(CONFIG_TOML_FILE)).await?;
    let parsed: ConfigToml = toml::from_str(&serialized)?;

    assert_eq!(parsed.model.as_deref(), Some("gpt-5.1-nexal"));
...
async fn set_model_updates_profile() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(nexal_home.path())
        ...
        .set_model(Some("gpt-5.1-nexal"), Some(ReasoningEffort::Medium))
        .apply()
        .await?;
    ...
    assert_eq!(profile.model.as_deref(), Some("gpt-5.1-nexal"));
    assert_eq!(
...
impl PrecedenceTestFixture {
    ...
    fn nexal_home(&self) -> PathBuf {
        self.nexal_home.path().to_path_buf()
    }
}
...
fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let nexal_home = TempDir::new()?;
    let workspace = nexal_home.path().join("workspace");
...
fn load_config_uses_requirements_guardian_developer_instructions() -> std::io::Result<()> {
    ...
    let config = Config::load_config_with_layer_stack(
        ConfigToml::default(),
        ConfigOverrides {
            cwd: Some(nexal_home.path().to_path_buf()),
            ..Default::default()
        },
        nexal_home.path().to_path_buf(),
        config_layer_stack,
...
fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let nexal_home = TempDir::new()?;
    let missing_path = nexal_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let nexal_home = TempDir::new()?;
    let role_config_path = nexal_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config = ConfigBuilder::default()
        .nexal_home(nexal_home.path().to_path_buf())
        .fallback_cwd(Some(nexal_home.path().to_path_buf()))
        .build()
        .await?;
    assert_eq!(
        config
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".nexal").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let nexal_home = TempDir::new()?;
    let researcher_path = nexal_home.path().join("agents").join("researcher.toml");
    let reviewer_path = nexal_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = nexal_home.path().join("agents");
...
fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let nexal_home = TempDir::new()?;
    let catalog_path = nexal_home.path().join("catalog.json");
...
"#;
    ...
    let nexal_home_temp_dir = TempDir::new().unwrap();
    ...
    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        nexal_home: nexal_home_temp_dir,
        cfg,
        model_provider_map,
        openai_provider,
...
fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
    ...
    assert_eq!(
        Config {
            ...
            nexal_home: fixture.nexal_home(),
            sqlite_home: fixture.nexal_home(),
            log_dir: fixture.nexal_home().join("log"),
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(nexal_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(nexal_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
async fn feature_requirements_reject_collab_legacy_alias() {
    let nexal_home = TempDir::new().expect("tempdir");
