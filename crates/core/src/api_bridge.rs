...
use chrono::DateTime;
use chrono::Utc;
use nexal_api::AuthProvider as ApiAuthProvider;
use nexal_api::TransportError;
use nexal_api::error::ApiError;
use nexal_api::rate_limits::parse_promo_message;
use nexal_api::rate_limits::parse_rate_limit_for_limit;
use http::HeaderMap;
...
pub(crate) fn map_api_error(err: ApiError) -> NexalErr {
    match err {
        ApiError::ContextWindowExceeded => NexalErr::ContextWindowExceeded,
        ApiError::QuotaExceeded => NexalErr::QuotaExceeded,
        ApiError::UsageNotIncluded => NexalErr::UsageNotIncluded,
        ApiError::Retryable { message, delay } => NexalErr::Stream(message, delay),
        ApiError::Stream(msg) => NexalErr::Stream(msg, None),
        ApiError::ServerOverloaded => NexalErr::ServerOverloaded,
        ApiError::Api { status, message } => NexalErr::UnexpectedStatus(UnexpectedResponseError {
            status,
            body: message,
            url: None,
            cf_ray: None,
        ...
        ApiError::InvalidRequest { message } => NexalErr::InvalidRequest(message),
        ApiError::Transport(transport) => match transport {
            TransportError::Http {
                ...
                {
                    return NexalErr::ServerOverloaded;
                }
                ...
                if status == http::StatusCode::BAD_REQUEST {
                        ...
                        .contains("The image data you provided does not represent a valid image")
                    {
                        ...
                        NexalErr::InvalidRequest(body_text)
                    }
                    ...
                    if let Ok(err) = serde_json::from_str::<UsageErrorResponse>(&body_text) {
                        if err.error.error_type.as_deref() == Some("usage_limit_reached") {
                            ...
                            return NexalErr::UsageNotIncluded;
                        }
                    }

                    NexalErr::RetryLimit(RetryLimitReachedError {
                        status,
                        request_id: extract_request_tracking_id(headers.as_ref()),
                    })
                } else {
                    NexalErr::UnexpectedStatus(UnexpectedResponseError {
                        status,
                        body: body_text,
                        url,
            ...
            TransportError::RetryLimit => NexalErr::RetryLimit(RetryLimitReachedError {
                status: http::StatusCode::INTERNAL_SERVER_ERROR,
                request_id: None,
            }),
            TransportError::Timeout => NexalErr::Timeout,
            TransportError::Network(msg) | TransportError::Build(msg) => {
                NexalErr::Stream(msg, None)
            }
        },
        ApiError::RateLimit(msg) => NexalErr::Stream(msg, None),
    }
}

const ACTIVE_LIMIT_HEADER: &str = "x-nexal-active-limit";
