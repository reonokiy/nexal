...
fn sandbox_detection_ignores_network_policy_text_in_non_sandbox_mode() {
    let output = make_exec_output(
        ...
        "",
        "",
        r#"NEXAL_NETWORK_POLICY_DECISION {"decision":"ask","reason":"not_allowed","source":"decider","protocol":"http","host":"google.com","port":80}"#,
    );
    assert!(!is_likely_sandbox_denied(SandboxType::None, &output));
}
...
fn sandbox_detection_ignores_network_policy_text_with_zero_exit_code() {
    let output = make_exec_output(
        ...
        "",
        "",
        r#"NEXAL_NETWORK_POLICY_DECISION {"decision":"ask","source":"decider","protocol":"http","host":"google.com","port":80}"#,
    );
...
fn windows_restricted_token_skips_external_sandbox_policies() {
    let policy = SandboxPolicy::ExternalSandbox {
        network_access: nexal_protocol::protocol::NetworkAccess::Restricted,
    };
...
fn windows_restricted_token_allows_legacy_workspace_write_policies() {
    let policy = SandboxPolicy::WorkspaceWrite {
        writable_roots: vec![],
        read_only_access: nexal_protocol::protocol::ReadOnlyAccess::FullAccess,
...
fn windows_restricted_token_rejects_split_only_filesystem_policies() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        nexal_protocol::permissions::FileSystemSandboxEntry {
            path: nexal_protocol::permissions::FileSystemPath::Special {
                value: nexal_protocol::permissions::FileSystemSpecialPath::CurrentWorkingDirectory,
            },
            access: nexal_protocol::permissions::FileSystemAccessMode::Write,
        },
        nexal_protocol::permissions::FileSystemSandboxEntry {
            path: nexal_protocol::permissions::FileSystemPath::Path {
                path: nexal_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&docs)
                    .expect("absolute docs"),
            },
            access: nexal_protocol::permissions::FileSystemAccessMode::Read,
        },
    ]);
...
fn windows_restricted_token_rejects_root_write_read_only_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        nexal_protocol::permissions::FileSystemSandboxEntry {
            path: nexal_protocol::permissions::FileSystemPath::Special {
                value: nexal_protocol::permissions::FileSystemSpecialPath::Root,
            },
            access: nexal_protocol::permissions::FileSystemAccessMode::Write,
        },
        nexal_protocol::permissions::FileSystemSandboxEntry {
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsRestrictedTokenFilesystemOverlay {
            additional_deny_write_paths: vec![
                nexal_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&docs)
                    .expect("absolute docs"),
            ],
        }))
    );
...
fn process_exec_tool_call_uses_platform_sandbox_for_network_only_restrictions() {
    let expected = nexal_sandboxing::get_platform_sandbox(false).unwrap_or(SandboxType::None);
...
async fn kill_child_process_group_kills_grandchildren_on_timeout() -> Result<()> {
    ...
    let params = ExecParams {
        ...
        network: None,
        sandbox_permissions: SandboxPermissions::UseDefault,
        windows_sandbox_level: nexal_protocol::config_types::WindowsSandboxLevel::Disabled,
        windows_sandbox_private_desktop: false,
        justification: None,
        arg0: None,
    };
...
async fn process_exec_tool_call_respects_cancellation_token() -> Result<()> {
    ...
    let output = match result {
        Err(NexalErr::Sandbox(SandboxErr::Timeout { output })) => output,
