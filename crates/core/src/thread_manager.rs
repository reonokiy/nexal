...
use crate::nexal_thread::NexalThread;
...
use nexal_app_server_protocol::ThreadHistoryBuilder;
...
struct TempNexalHomeGuard {
    path: PathBuf,
}

impl Drop for TempNexalHomeGuard {
    fn drop(&mut self) {
...
/// Represents a newly created Nexal thread (formerly called a conversation), including the first event
/// (which is [`EventMsg::SessionConfigured`]).
pub struct NewThread {
    pub thread_id: ThreadId,
    pub thread: Arc<NexalThread>,
...
pub struct ThreadManager {
    ...
    _test_nexal_home_guard: Option<TempNexalHomeGuard>,
}
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<NexalThread>>>>,
...
impl ThreadManager {
    ...
    pub(crate) fn with_models_provider_for_tests(auth: NexalAuth) -> Self {
        ...
        manager._test_nexal_home_guard = Some(TempNexalHomeGuard { path: nexal_home });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided NexalAuth and nexal home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> NexalResult<Arc<NexalThread>> {
        self.state.get_thread(thread_id).await
    }

    pub async fn start_thread(&self, config: Config) -> NexalResult<NewThread> {
    ...
    /// as `Arc<NexalThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<NexalThread>> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(&self, thread_id: ThreadId) -> NexalResult<Arc<NexalThread>> {
    ...
    pub(crate) async fn send_op(&self, thread_id: ThreadId, op: Op) -> NexalResult<String> {
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<NexalThread>> {
    ...
    pub(crate) async fn spawn_thread_with_source(
        ...
        .await?;
        self.finalize_thread_spawn(nexal, thread_id, watch_registration)
            .await
    }
    ...
    async fn finalize_thread_spawn(
        ...
        let event = nexal.next_event().await?;
        let session_configured = match event {
            ...
            _ => {
                return Err(NexalErr::SessionConfiguredNotFirstEvent);
            }
        };

        let thread = Arc::new(NexalThread::new(
            nexal,
