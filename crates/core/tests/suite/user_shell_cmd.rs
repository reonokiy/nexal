...
async fn user_shell_cmd_ls_and_cat_in_temp_dir() {
    ...
    nexal
        ...
        .await
        .unwrap();
    let msg = wait_for_event(&nexal, |ev| matches!(ev, EventMsg::ExecCommandEnd(_))).await;
...
async fn user_shell_cmd_can_be_interrupted() {
    ...
    let _begin = wait_for_event_match(nexal, |ev| match ev {
    ...
    .await;
    nexal.submit(Op::Interrupt).await.unwrap();
...
async fn user_shell_command_does_not_replace_active_turn() -> anyhow::Result<()> {
    ...
    let _ = wait_for_event_match(&fixture.nexal, |ev| match ev {
    ...
    for _ in 0..200 {
        let event = timeout(Duration::from_secs(20), fixture.nexal.next_event())
            .await
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    let mut builder = core_test_support::test_nexal::test_nexal().with_config(move |config| {
        config
            .features
    ...
    let begin_event = wait_for_event_match(&test.nexal, |ev| match ev {
    ...
    let delta_event = wait_for_event_match(&test.nexal, |ev| match ev {
    ...
    let end_event = wait_for_event_match(&test.nexal, |ev| match ev {
    ...
    let _ = wait_for_event(&test.nexal, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn user_shell_command_does_not_set_network_sandbox_env_var() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let mut builder = core_test_support::test_nexal::test_nexal().with_config(|config| {
