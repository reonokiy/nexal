...
use nexal_core::REVIEW_PROMPT;
use nexal_core::config::Config;
use nexal_core::review_format::render_review_output_text;
use nexal_protocol::models::ContentItem;
use nexal_protocol::models::ResponseItem;
use nexal_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
use nexal_protocol::protocol::EventMsg;
use nexal_protocol::protocol::ExitedReviewModeEvent;
use nexal_protocol::protocol::Op;
use nexal_protocol::protocol::ReviewCodeLocation;
use nexal_protocol::protocol::ReviewFinding;
use nexal_protocol::protocol::ReviewLineRange;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under Nexal sandbox network restrictions.
    skip_if_no_network!();
    ...
    let nexal = new_conversation_for_server(&server, nexal_home.clone(), |_| {}).await;
    ...
    let _entered = wait_for_event(&nexal, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let closed = wait_for_event(&nexal, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let _complete = wait_for_event(&nexal, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    // Also verify that a user message with the header and a formatted finding
    // was recorded back in the parent session's rollout.
    let path = nexal.rollout_path().expect("rollout path");
...
async fn review_filters_agent_message_related_events() {
    ...
    wait_for_event(&nexal, |event| match event {
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let nexal = new_conversation_for_server(&server, nexal_home.clone(), |cfg| {
    ...
    // Wait for completion
    let _entered = wait_for_event(&nexal, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let _closed = wait_for_event(&nexal, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = nexal_home.path().join("resume.jsonl");
    {
        ...
        let user = nexal_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = nexal_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let nexal =
        resume_conversation_for_server(&server, nexal_home.clone(), session_file.clone(), |_| {})
            .await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let nexal = new_conversation_for_server(&server, nexal_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, nexal_home, resume_path)
        .await
