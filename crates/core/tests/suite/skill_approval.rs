...
fn skill_script_command(test: &TestNexal, script_name: &str) -> Result<String> {
...
async fn wait_for_exec_approval_request(test: &TestNexal) -> Option<ExecApprovalRequestEvent> {
    wait_for_event_match(test.nexal.as_ref(), |event| match event {
...
async fn wait_for_turn_complete(test: &TestNexal) {
    wait_for_event(test.nexal.as_ref(), |event| {
...
async fn shell_zsh_fork_still_enforces_workspace_write_sandbox() -> Result<()> {
    ...
    let outside_path = "/tmp/nexal-zsh-fork-workspace-write-deny.txt";
    let workspace_write_policy = restrictive_workspace_write_policy();
