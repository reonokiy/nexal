...
fn write_sample_plugin_manifest_and_config(home: &TempDir) -> std::path::PathBuf {
    ...
    std::fs::create_dir_all(plugin_root.join(".nexal-plugin")).expect("create plugin manifest dir");
    std::fs::write(
...
async fn build_plugin_test_nexal(
    server: &MockServer,
    nexal_home: Arc<TempDir>,
) -> Result<Arc<nexal_core::NexalThread>> {
...
async fn build_analytics_plugin_test_nexal(
    server: &MockServer,
    nexal_home: Arc<TempDir>,
) -> Result<Arc<nexal_core::NexalThread>> {
...
async fn build_apps_enabled_plugin_test_nexal(
    server: &MockServer,
    nexal_home: Arc<TempDir>,
...
async fn capability_sections_render_in_developer_message_in_order() -> Result<()> {
    ...
    let nexal_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(nexal_home.as_ref());
    write_plugin_app_plugin(nexal_home.as_ref());
    let nexal = build_apps_enabled_plugin_test_nexal(
        &server,
        Arc::clone(&nexal_home),
    ...
    nexal
        .submit(Op::UserInput {
            items: vec![nexal_protocol::user_input::UserInput::Text {
                text: "hello".into(),
...
async fn explicit_plugin_mentions_inject_plugin_guidance() -> Result<()> {
    ...
    write_plugin_skill_plugin(nexal_home.as_ref());
    write_plugin_mcp_plugin(nexal_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(nexal_home.as_ref());

    let nexal =
        build_apps_enabled_plugin_test_nexal(&server, nexal_home, apps_server.chatgpt_base_url)
            .await?;

    nexal
        .submit(Op::UserInput {
            items: vec![nexal_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
    ...
    assert!(
        request_tools
            .iter()
            .any(|name| name == "mcp__nexal_apps__google_calendar_create_event"),
    ...
    let calendar_description = tool_description(
        &request_body,
        "mcp__nexal_apps__google_calendar_create_event",
    )
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let nexal_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(nexal_home.as_ref());
    let nexal = build_analytics_plugin_test_nexal(&server, nexal_home).await?;
    ...
    let analytics_request = loop {
        ...
        if let Some(request) = requests
            .into_iter()
            .find(|request| request.url.path() == "/nexal/analytics-events/events")
        {
            break request;
        }
    ...
    assert_eq!(event["event_type"], "nexal_plugin_used");
    ...
    assert_eq!(
        ...
        serde_json::json!(nexal_core::default_client::originator().value)
    );
...
async fn plugin_mcp_tools_are_listed() -> Result<()> {
    ...
    write_plugin_mcp_plugin(nexal_home.as_ref(), &rmcp_test_server_bin);
    let nexal = build_plugin_test_nexal(&server, nexal_home).await?;

    let tools_ready_deadline = Instant::now() + Duration::from_secs(30);
    loop {
        nexal.submit(Op::ListMcpTools).await?;
