...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.nexal, |ev| {
    ...
    let end_event = wait_for_event(&fixture.nexal, |ev| {
    ...
    wait_for_event(&fixture.nexal, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn stdio_image_responses_round_trip() -> anyhow::Result<()> {
    ...
    loop {
        fixture.nexal.submit(Op::ListMcpTools).await?;
...
async fn stdio_image_responses_are_sanitized_for_text_only_model() -> anyhow::Result<()> {
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: nexal_protocol::openai_models::ReasoningEffort::Medium,
    ...
    wait_for_event(&fixture.nexal, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
    wait_for_event(&fixture.nexal, |ev| {
...
/// This test writes to a fallback credentials file in NEXAL_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire NEXAL_HOME through the code.
#[test]
#[serial(nexal_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    let _nexal_home_guard = EnvVarGuard::set("NEXAL_HOME", temp_home.path().as_os_str());
