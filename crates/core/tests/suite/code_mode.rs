...
) -> Result<(TestNexal, ResponseMock)> {
    let mut builder = test_nexal()
        .with_model("test-gpt-5.1-nexal")
...
"#;
    ...
    let turn_id = wait_for_event_match(&test.nexal, |event| match event {
        EventMsg::TurnStarted(event) => Some(event.turn_id.clone()),
        _ => None,
    })
    .await;
    let request = wait_for_event_match(&test.nexal, |event| match event {
    ...
    test.nexal
        ...
        .await?;
    wait_for_event(&test.nexal, |event| match event {
