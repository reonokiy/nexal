...
use nexal_utils_cargo_bin::find_resource;
...
fn repo_root() -> std::path::PathBuf {
    ...
    nexal_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
async fn responses_mode_stream_cli() {
    ...
    let bin = nexal_utils_cargo_bin::cargo_bin("nexal").unwrap();
    ...
    //     10,
    //     None,
    //     nexal_core::ThreadSortKey::UpdatedAt,
    //     &[],
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    // Nexal to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
/// Verify that `nexal exec --profile ...` preserves the active profile when it
...
/// 2. Configures nexal to read from this fixture via NEXAL_RS_SSE_FIXTURE env var
...
async fn responses_api_stream_cli() {
    ...
    cmd.env("NEXAL_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("NEXAL_RS_SSE_FIXTURE", fixture);
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the nexal CLI and invoke `exec`, which is what records a session.
    let bin = nexal_utils_cargo_bin::cargo_bin("nexal").unwrap();
    let mut cmd = AssertCommand::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
    ...
    let bin2 = nexal_utils_cargo_bin::cargo_bin("nexal").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("NEXAL_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("NEXAL_RS_SSE_FIXTURE", &fixture);

    let output2 = cmd2.output().unwrap();
    assert!(output2.status.success(), "resume nexal-cli run failed");
