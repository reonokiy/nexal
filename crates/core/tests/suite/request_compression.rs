...
async fn request_body_is_zstd_compressed_for_nexal_backend_when_enabled() -> anyhow::Result<()> {
    ...
    let base_url = format!("{}/backend-api/nexal/v1", server.uri());
    ...
    let nexal = builder.build(&server).await?.nexal;
