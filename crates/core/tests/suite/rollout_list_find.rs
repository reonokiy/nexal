...
use chrono::Utc;
use nexal_core::EventPersistenceMode;
use nexal_core::RolloutRecorder;
use nexal_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(nexal_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = nexal_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(nexal_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(nexal_home, "sessions", id)
}

async fn upsert_thread_metadata(nexal_home: &Path, thread_id: ThreadId, rollout_path: PathBuf) {
    let runtime = StateRuntime::init(nexal_home.to_path_buf(), "test-provider".to_string())
        .await
        .unwrap();
    ...
    builder.cwd = nexal_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_nexal_home_directory() {
    let repo = TempDir::new().unwrap();
    let nexal_home = repo.path().join(".nexal");
    std::fs::create_dir_all(&nexal_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".nexal/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&nexal_home, id);

    let found = find_thread_path_by_id_str(&nexal_home, &id.to_string())
        .await
        .unwrap();
