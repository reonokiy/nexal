...
use nexal_apply_patch::APPLY_PATCH_TOOL_INSTRUCTIONS;
use nexal_core::shell::Shell;
use nexal_core::shell::default_user_shell;
...
async fn prompt_tools_are_consistent_across_requests() -> anyhow::Result<()> {
    ...
    let TestNexal {
        ...
        .with_config(|config| {
            ...
            config.model = Some("gpt-5.1-nexal-max".to_string());
...
-> anyhow::Result<()> {
    ...
    let TestNexal { nexal, config, .. } = test_nexal()
        .with_config(|config| {
            config.user_instructions = Some("be consistent and helpful".to_string());
            config
                .features
