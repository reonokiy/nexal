...
const POLICY_PATH_FOR_TEST: &str = "/nexal/policy/path";
const SNAPSHOT_PATH_FOR_TEST: &str = "/nexal/snapshot/path";
const SNAPSHOT_MARKER_VAR: &str = "NEXAL_SNAPSHOT_POLICY_MARKER";
...
async fn wait_for_snapshot(nexal_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = nexal_home.join("shell_snapshots");
...
async fn run_snapshot_command_with_options(
    ...
    let builder = test_nexal().with_config(move |config| {
    ...
    let harness = TestNexalHarness::with_builder(builder).await?;
    let args = json!({
        "cmd": command,
        "yield_time_ms": 1000,
    });
    ...
    let nexal_home = test.home.path().to_path_buf();
    ...
    .await;
    let snapshot_path = wait_for_snapshot(&nexal_home).await?;
...
async fn run_tool_turn_on_harness(
    ...
    wait_for_event_match(&nexal, |ev| match ev {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.nexal_home));
...
async fn shell_command_snapshot_preserves_shell_environment_policy_set() -> Result<()> {
    let builder = test_nexal().with_config(|config| {
        config
            .features
            .enable(Feature::ShellSnapshot)
            .expect("test config should allow feature update");
    ...
    let harness = TestNexalHarness::with_builder(builder).await?;
    let nexal_home = harness.test().home.path().to_path_buf();
    ...
    assert!(snapshot_path.starts_with(nexal_home));
...
async fn shell_command_snapshot_still_intercepts_apply_patch() -> Result<()> {
    ...
    let args = json!({
        "command": script,
        // The intercepted apply_patch path self-invokes nexal, which can take
    ...
    wait_for_event(&nexal, |ev| match ev {
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let nexal_home = home.path().to_path_buf();
