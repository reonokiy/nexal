...
use nexal_protocol::models::FunctionCallOutputContentItem;
use nexal_protocol::models::FunctionCallOutputPayload;
use nexal_protocol::models::ImageDetail;
use nexal_protocol::models::LocalShellAction;
use nexal_protocol::models::LocalShellExecAction;
use nexal_protocol::models::LocalShellStatus;
use nexal_protocol::models::MessagePhase;
use nexal_protocol::models::ReasoningItemContent;
...
use core_test_support::test_nexal::TestNexal;
...
/// Writes an `auth.json` into the provided `nexal_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        nexal_home.path().join("auth.json"),
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = nexal_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = nexal_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![nexal_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = nexal_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Nexal to resume from our file
    let nexal_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_nexal()
        .with_home(nexal_home.clone())
        .with_config(|config| {
            // Ensure user instructions are NOT delivered on resume.
            config.user_instructions = Some("be nice".to_string());
        });
    let test = builder
        .resume(&server, nexal_home, session_path.clone())
        .await
        .expect("resume conversation");
    let nexal = test.nexal.clone();
    ...
    nexal
        ...
        .await
        .unwrap();
    wait_for_event(&nexal, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let nexal_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_nexal().with_model("gpt-5.1");
    let test = builder
        .resume(&server, nexal_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_conversation_id_and_model_headers_in_request() {
    ...
    let mut builder = test_nexal().with_auth(NexalAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_nexal()
        .with_auth(NexalAuth::from_api_key("Test API Key"))
        .with_config(|config| {
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut builder = test_nexal()
        .with_auth(create_dummy_nexal_auth())
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/nexal/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager =
        match NexalAuth::from_auth_storage(nexal_home.path(), AuthCredentialsStoreMode::File) {
            Ok(Some(auth)) => nexal_core::test_support::auth_manager_from_auth(auth),
            Ok(None) => panic!("No NexalAuth found in nexal_home"),
            Err(e) => panic!("Failed to load NexalAuth: {e}"),
        };
    ...
    let NewThread { thread: nexal, .. } = thread_manager
        .start_thread(config)
        .await
        .expect("create new conversation");
...
async fn skills_append_to_developer_message() {
    ...
    let mut builder = test_nexal()
        ...
        .with_auth(NexalAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config.cwd = nexal_home_path.abs();
        });
    let nexal = builder
        .build(&server)
        .await
    ...
    let _nexal_home_guard = nexal_home;
}
...
async fn includes_configured_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestNexal { nexal, .. } = test_nexal()
        .with_model("gpt-5.1-nexal")
        .with_config(|config| {
...
-> anyhow::Result<()> {
    ...
    .await;
    let TestNexal { nexal, .. } = test_nexal().with_model("gpt-5.1").build(&server).await?;
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let model_info =
        nexal_core::test_support::construct_model_info_offline(model.as_str(), &config);
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-nexal-primary-used-percent", "12.5")
        .insert_header("x-nexal-secondary-used-percent", "40.0")
        .insert_header("x-nexal-primary-window-minutes", "10")
        .insert_header("x-nexal-secondary-window-minutes", "60")
        .insert_header("x-nexal-primary-reset-at", "1704069000")
        .insert_header("x-nexal-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_nexal()
        .with_auth(NexalAuth::from_api_key("test"))
        .with_config(move |config| {
    ...
    let first_token_event =
        wait_for_event(&nexal, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    pretty_assertions::assert_eq!(
        ...
        json!({
            "info": {
                ...
                // Default model is gpt-5.1-nexal-max in tests → 95% usable context window
    ...
    wait_for_event(&nexal, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-nexal-primary-used-percent", "100.0")
        .insert_header("x-nexal-secondary-used-percent", "87.5")
        .insert_header("x-nexal-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-nexal-primary-window-minutes", "15")
        .insert_header("x-nexal-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let mut builder = test_nexal();
    let nexal_fixture = builder.build(&server).await?;
    let nexal = nexal_fixture.nexal.clone();
    ...
    let token_event = wait_for_event(&nexal, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&nexal, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&nexal, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&nexal, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = NexalErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_nexal_auth() -> NexalAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Nexal sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
