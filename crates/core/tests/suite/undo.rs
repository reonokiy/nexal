...
async fn undo_harness() -> Result<TestNexalHarness> {
    let builder = test_nexal().with_model("gpt-5.1").with_config(|config| {
    ...
    TestNexalHarness::with_builder(builder).await
}
...
fn init_git_repo(path: &Path) -> Result<()> {
    ...
    git(path, &["config", "user.name", "Nexal Tests"])?;
    git(path, &["config", "user.email", "nexal-tests@example.com"])?;

    // Create README.txt
    let readme_path = path.join("README.txt");
    fs::write(&readme_path, "Test repository initialized by Nexal.\n")?;
...
async fn invoke_undo(nexal: &Arc<NexalThread>) -> Result<UndoCompletedEvent> {
    nexal.submit(Op::Undo).await?;
...
async fn expect_successful_undo(nexal: &Arc<NexalThread>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(nexal).await?;
    assert!(
        event.success,
...
async fn expect_failed_undo(nexal: &Arc<NexalThread>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(nexal).await?;
    assert!(
        !event.success,
...
async fn undo_removes_new_file_created_during_turn() -> Result<()> {
    ...
    let nexal = Arc::clone(&harness.test().nexal);
    let completed = expect_successful_undo(&nexal).await?;
...
async fn undo_sequential_turns_consumes_snapshots() -> Result<()> {
    ...
    let nexal = Arc::clone(&harness.test().nexal);
    expect_successful_undo(&nexal).await?;
    assert_eq!(fs::read_to_string(&story)?, "turn two\n");
    ...
    expect_failed_undo(&nexal).await?;
