...
use nexal_core::seatbelt::spawn_command_under_seatbelt;
use nexal_core::spawn::NEXAL_SANDBOX_ENV_VAR;
use nexal_core::spawn::StdioPolicy;
...
impl TestScenario {
    async fn run_test(&self, policy: &SandboxPolicy, expectations: TestExpectations) {
        if std::env::var(NEXAL_SANDBOX_ENV_VAR) == Ok("seatbelt".to_string()) {
            eprintln!("{NEXAL_SANDBOX_ENV_VAR} is set to 'seatbelt', skipping test.");
            return;
        }
...
async fn java_home_finds_runtime_under_seatbelt() {
    ...
    env.remove(NEXAL_SANDBOX_ENV_VAR);
