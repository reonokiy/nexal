...
async fn nexal_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    nexal_returns_json_result("gpt-5.1".to_string()).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn nexal_returns_json_result_for_gpt5_nexal() -> anyhow::Result<()> {
    nexal_returns_json_result("gpt-5.1-nexal".to_string()).await
}

async fn nexal_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("nexal_output_schema".into()))
    ...
    let TestNexal { nexal, cwd, .. } = test_nexal().build(&server).await?;
    ...
    let message = wait_for_event(&nexal, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;
