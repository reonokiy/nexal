...
use core_test_support::test_nexal::TestNexalBuilder;
use core_test_support::test_nexal::TestNexalHarness;
...
fn remote_realtime_test_nexal_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestNexalBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_nexal()
        .with_auth(NexalAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(nexal: &nexal_core::NexalThread) -> Result<()> {
    nexal
    ...
    wait_for_event_match(nexal, |msg| match msg {
...
async fn close_realtime_conversation(nexal: &nexal_core::NexalThread) -> Result<()> {
    nexal.submit(Op::RealtimeConversationClose).await?;
    wait_for_event_match(nexal, |msg| match msg {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestNexalHarness::with_builder(
        test_nexal().with_auth(NexalAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let nexal = harness.test().nexal.clone();
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&nexal, |event| match event {
        EventMsg::ContextCompacted(_) => Some(true),
        _ => None,
    })
    .await;
    wait_for_event(&nexal, |event| matches!(event, EventMsg::TurnComplete(_))).await;
    assert!(message);
...
async fn remote_compact_trims_function_call_history_to_fit_context_window() -> Result<()> {
    ...
    let harness = TestNexalHarness::with_builder(
        test_nexal()
            .with_auth(NexalAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
...
async fn auto_remote_compact_failure_stops_agent_loop() -> Result<()> {
    ...
    let error_message = wait_for_event_match(&nexal, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestNexalHarness::with_builder(
        test_nexal()
            .with_auth(NexalAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    .await?;
    let baseline_nexal = baseline_harness.test().nexal.clone();
    ...
    baseline_nexal
        ...
        .await?;
    wait_for_event(&baseline_nexal, |event| {
    ...
    baseline_nexal.submit(Op::Compact).await?;
    wait_for_event(&baseline_nexal, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let override_harness = TestNexalHarness::with_builder(
        test_nexal()
            .with_auth(NexalAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    .await?;
    let override_nexal = override_harness.test().nexal.clone();
    ...
    override_nexal
        ...
        .await?;
    wait_for_event(&override_nexal, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_nexal.submit(Op::Compact).await?;
    wait_for_event(&override_nexal, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_manual_compact_emits_context_compaction_items() -> Result<()> {
    ...
    {
        let event = nexal.next_event().await.unwrap();
        match event.msg {
...
async fn remote_compact_persists_replacement_history_in_rollout() -> Result<()> {
    ...
    nexal.submit(Op::Shutdown).await?;
    wait_for_event(&nexal, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    initial.nexal.submit(Op::Compact).await?;
    wait_for_event(&initial.nexal, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    initial.nexal.submit(Op::Shutdown).await?;
    wait_for_event(&initial.nexal, |ev| {
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.nexal, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn remote_compact_refreshes_stale_developer_instructions_without_resume() -> Result<()> {
    ...
    let mut builder = test_nexal().with_auth(NexalAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    test.nexal
        ...
        .await?;
    wait_for_event(&test.nexal, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    test.nexal.submit(Op::Compact).await?;
    wait_for_event(&test.nexal, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_nexal_builder(&realtime_server).with_config(|config| {
    ...
    start_realtime_conversation(test.nexal.as_ref()).await?;
    ...
    close_realtime_conversation(test.nexal.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_nexal_builder(&realtime_server).with_config({
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_nexal_builder(&realtime_server);
    let test = builder.build(&server).await?;
...
async fn snapshot_request_shape_remote_compact_resume_restates_realtime_end() -> Result<()> {
    ...
    start_realtime_conversation(initial.nexal.as_ref()).await?;
    ...
    close_realtime_conversation(initial.nexal.as_ref()).await?;
...
-> Result<()> {
    ...
    let previous_model = "gpt-5.1-nexal-max";
    let next_model = "gpt-5.2-nexal";
    let harness = TestNexalHarness::with_builder(
        test_nexal()
            .with_auth(NexalAuth::create_dummy_chatgpt_auth_for_testing())
