...
//! request payload that Nexal would send to the model and assert that the
...
fn network_disabled() -> bool {
    std::env::var(NEXAL_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn compact_resume_and_fork_preserve_model_history_view() {
    ...
    let expected_model = "gpt-5.1-nexal";
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<NexalThread>) {
    ...
    (test.home, test.config, test.thread_manager, test.nexal)
}

async fn user_turn(conversation: &Arc<NexalThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<NexalThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<NexalThread>) -> std::path::PathBuf {
...
async fn resume_conversation(
    ...
    let auth_manager = nexal_core::test_support::auth_manager_from_auth(
        nexal_core::NexalAuth::from_api_key("dummy"),
    );
