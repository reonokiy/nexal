...
use crate::test_nexal::TestNexal;
use crate::test_nexal::test_nexal;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = nexal_utils_cargo_bin::cargo_bin("nexal-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `nexal-execve-wrapper` binary");
        return Ok(None);
    };
