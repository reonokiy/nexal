...
const SERVER_NAME: &str = "nexal-apps-test";
...
async fn mount_streamable_http_json_rpc(
    ...
    Mock::given(method("POST"))
        .and(path_regex("^/api/nexal/apps/?$"))
        .respond_with(NexalAppsJsonRpcResponder {
            connector_name,
...
struct NexalAppsJsonRpcResponder {
    connector_name: String,
    connector_description: String,
    searchable: bool,
}

impl Respond for NexalAppsJsonRpcResponder {
        ...
        match method {
            ...
            "tools/call" => {
                ...
                let starts_at = body
                    ...
                    .and_then(Value::as_str)
                    .unwrap_or_default();
                let nexal_apps_meta = body.pointer("/params/_meta/_nexal_apps").cloned();
