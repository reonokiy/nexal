...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    assert_eq!(request.header("x-nexal-sandbox"), None);
}
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let test = test_nexal().build(&server).await.expect("build test nexal");
    ...
    let initial_header = first_request
        .single_request()
        .header("x-nexal-turn-metadata")
        .expect("x-nexal-turn-metadata header should be present");
    let initial_parsed: serde_json::Value =
        serde_json::from_str(&initial_header).expect("x-nexal-turn-metadata should be valid JSON");
    ...
    assert!(
        ...
        "turn_id should not be empty in x-nexal-turn-metadata"
    );
    assert_eq!(
        initial_parsed
            .get("sandbox")
    ...
    run_git(&[
        ...
        "add",
        "origin",
        "https://github.com/openai/nexal.git",
    ]);
