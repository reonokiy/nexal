...
This crate implements the business logic for Nexal. It is designed to be used by the various Nexal UIs written in Rust.

## Dependencies

Note that `nexal-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
Expects the binary containing `nexal-core` to run the equivalent of `codex sandbox linux` (legacy alias: `codex debug landlock`) when `arg0` is `nexal-linux-sandbox`. See the `nexal-arg0` crate for details.
...
vendored bubblewrap path compiled into the binary and Nexal surfaces a startup
...
Expects the binary containing `nexal-core` to simulate the virtual `apply_patch` CLI when `arg1` is `--nexal-run-as-apply-patch`. See the `nexal-arg0` crate for details.
