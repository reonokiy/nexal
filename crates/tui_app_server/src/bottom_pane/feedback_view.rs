...
use nexal_feedback::feedback_diagnostics::FEEDBACK_DIAGNOSTICS_ATTACHMENT_FILENAME;
use nexal_feedback::feedback_diagnostics::FeedbackDiagnostics;
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec!["  • ".into(), "nexal-logs.log".into()]).into(),
    ];
    ...
    super::SelectionViewParams {
        ...
        items: vec![
            super::SelectionItem {
                name: "Yes".to_string(),
                description: Some(
                    "Share the current Nexal session logs with the team for troubleshooting."
                        .to_string(),
                ),
...
mod tests {
    ...
    use crate::app_event::AppEvent;
    use crate::app_event_sender::AppEventSender;
    use nexal_feedback::feedback_diagnostics::FeedbackDiagnostic;
    ...
    fn make_view(category: FeedbackCategory) -> FeedbackNoteView {
        ...
        let snapshot = nexal_feedback::NexalFeedback::new().snapshot(None);
    ...
    fn feedback_view_with_connectivity_diagnostics() {
        ...
        let snapshot = nexal_feedback::NexalFeedback::new()
            .snapshot(None)
