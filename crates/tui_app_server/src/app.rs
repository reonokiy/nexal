...
use nexal_ansi_escape::ansi_escape_line;
use nexal_app_server_client::AppServerRequestHandle;
use nexal_app_server_client::TypedRequestError;
...
fn command_execution_decision_to_review_decision(
    decision: nexal_app_server_protocol::CommandExecutionApprovalDecision,
) -> nexal_protocol::protocol::ReviewDecision {
    match decision {
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&nexal_protocol::protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&nexal_protocol::approvals::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[nexal_protocol::approvals::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&nexal_protocol::models::PermissionProfile>,
) -> Vec<nexal_protocol::protocol::ReviewDecision> {
...
fn session_summary(
    ...
    let resume_command = nexal_core::util::resume_command(thread_name.as_deref(), thread_id);
    Some(SessionSummary {
        usage_line,
        resume_command,
    })
...
fn list_skills_response_to_core(response: SkillsListResponse) -> ListSkillsResponseEvent {
    ListSkillsResponseEvent {
        skills: response
            .data
            .into_iter()
            .map(|entry| nexal_protocol::protocol::SkillsListEntry {
                cwd: entry.cwd,
                skills: entry
                    .skills
                    .into_iter()
                    .map(|skill| nexal_protocol::protocol::SkillMetadata {
                        name: skill.name,
                        description: skill.description,
                        short_description: skill.short_description,
                        interface: skill.interface.map(|interface| {
                            nexal_protocol::protocol::SkillInterface {
                        ...
                        dependencies: skill.dependencies.map(|dependencies| {
                            nexal_protocol::protocol::SkillDependencies {
                                tools: dependencies
                                    .tools
                                    .into_iter()
                                    .map(|tool| nexal_protocol::protocol::SkillToolDependency {
                                        r#type: tool.r#type,
                                        value: tool.value,
                        ...
                        scope: match skill.scope {
                            nexal_app_server_protocol::SkillScope::User => {
                                nexal_protocol::protocol::SkillScope::User
                            }
                            nexal_app_server_protocol::SkillScope::Repo => {
                                nexal_protocol::protocol::SkillScope::Repo
                            }
                            nexal_app_server_protocol::SkillScope::System => {
                                nexal_protocol::protocol::SkillScope::System
                            }
                            nexal_app_server_protocol::SkillScope::Admin => {
                                nexal_protocol::protocol::SkillScope::Admin
                            }
                        },
                ...
                errors: entry
                    .errors
                    .into_iter()
                    .map(|error| nexal_protocol::protocol::SkillErrorInfo {
                        path: error.path,
...
fn emit_project_config_warnings(app_event_tx: &AppEventSender, config: &Config) {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        let ConfigLayerSource::Project { dot_nexal_folder } = &layer.name else {
            continue;
        };
        ...
        disabled_folders.push((
            dot_nexal_folder.as_path().display().to_string(),
            layer
                .disabled_reason
                .as_ref()
...
fn emit_system_bwrap_warning(app_event_tx: &AppEventSender) {
    let Some(message) = nexal_core::config::system_bwrap_warning() else {
        return;
    };
...
async fn prepare_startup_tooltip_override(
    ...
    if let Err(err) = ConfigEditsBuilder::new(&config.nexal_home)
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.nexal_error_info,
        Some(AppServerNexalErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
impl App {
    ...
    async fn update_feature_flags(&mut self, updates: Vec<(Feature, bool)>) {
        ...
        let mut builder = ConfigEditsBuilder::new(&self.config.nexal_home)
    ...
    fn clear_ui_header_lines(&self, width: u16) -> Vec<Line<'static>> {
        self.clear_ui_header_lines_with_version(width, NEXAL_CLI_VERSION)
    }
    ...
    /// `$NEXAL_HOME/history.jsonl`, bypassing the app-server RPC layer.
                    ...
                    app_event_tx.send(AppEvent::ThreadHistoryEntryResponse {
                        ...
                        event: GetHistoryEntryResponseEvent {
                            ...
                            entry: entry_opt.map(|entry| {
                                nexal_protocol::message_history::HistoryEntry {
        ...
        match op.view() {
            ...
            AppCommandView::ListSkills { cwds, force_reload } => {
                let response = app_server
                    .skills_list(nexal_app_server_protocol::SkillsListParams {
                        cwds: cwds.to_vec(),
                        force_reload,
    ...
    pub async fn run(
        ...
        {
            session_telemetry.counter("nexal.status_line", /*inc*/ 1, &[]);
        }
    ...
    async fn handle_event(
        ...
        match event {
            ...
            AppEvent::PersistRealtimeAudioDeviceSelection { kind, name } => {
                let builder = match kind {
                    RealtimeAudioDeviceKind::Microphone => {
                        ConfigEditsBuilder::new(&self.config.nexal_home)
            ...
            AppEvent::StatusLineSetup { items } => {
                ...
                let edit = nexal_core::config::edit::status_line_items_edit(&ids);
                let apply_result = ConfigEditsBuilder::new(&self.config.nexal_home)
                    .with_edits([edit])
                    .apply()
                    .await;
                match apply_result {
            ...
            AppEvent::TerminalTitleSetup { items } => {
                let ids = items.iter().map(ToString::to_string).collect::<Vec<_>>();
                let edit = nexal_core::config::edit::terminal_title_items_edit(&ids);
                let apply_result = ConfigEditsBuilder::new(&self.config.nexal_home)
                    .with_edits([edit])
                    .apply()
                    .await;
            ...
            AppEvent::SyntaxThemeSelected { name } => {
                let edit = nexal_core::config::edit::syntax_theme_edit(&name);
                let apply_result = ConfigEditsBuilder::new(&self.config.nexal_home)
                    .with_edits([edit])
                    .apply()
                    .await;
    ...
    fn reasoning_label_for(
        ...
        (!model.starts_with("nexal-auto-")).then(|| Self::reasoning_label(reasoning_effort))
    }

    pub(crate) fn token_usage(&self) -> nexal_protocol::protocol::TokenUsage {
        self.chat_widget.token_usage()
    }
    ...
    fn restore_runtime_theme_from_config(&self) {
        if let Some(name) = self.config.tui_theme.as_deref()
            && let Some(theme) =
                crate::render::highlight::resolve_theme_by_name(name, Some(&self.config.nexal_home))
        {
            crate::render::highlight::set_syntax_theme(theme);
            return;
        }
        ...
        let editor_cmd = match external_editor::resolve_editor_command() {
            ...
            Err(external_editor::EditorError::MissingEditor) => {
                self.chat_widget
                    ...
                    "Cannot open external editor: set $VISUAL or $EDITOR before starting Nexal."
                        .to_string(),
                ));
...
type McpInventoryMaps = (
    HashMap<String, nexal_protocol::mcp::Tool>,
    HashMap<String, Vec<nexal_protocol::mcp::Resource>>,
    HashMap<String, Vec<nexal_protocol::mcp::ResourceTemplate>>,
    ...
    for status in statuses {
        ...
        auth_statuses.insert(
            ...
            match status.auth_status {
                nexal_app_server_protocol::McpAuthStatus::Unsupported => McpAuthStatus::Unsupported,
                nexal_app_server_protocol::McpAuthStatus::NotLoggedIn => McpAuthStatus::NotLoggedIn,
                nexal_app_server_protocol::McpAuthStatus::BearerToken => McpAuthStatus::BearerToken,
                nexal_app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
            },
        );
...
mod tests {
    ...
    use nexal_app_server_protocol::AdditionalNetworkPermissions;
    use nexal_app_server_protocol::AdditionalPermissionProfile;
    ...
    use nexal_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
    use nexal_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
    use nexal_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
    use nexal_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
    use nexal_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
    use nexal_app_server_protocol::RequestId as AppServerRequestId;
    use nexal_app_server_protocol::ServerNotification;
    use nexal_app_server_protocol::ServerRequest;
    ...
    use nexal_app_server_protocol::TokenUsageBreakdown;
    use nexal_app_server_protocol::Turn;
    ...
    use nexal_protocol::protocol::McpAuthStatus;
    use nexal_protocol::protocol::NetworkApprovalContext;
    use nexal_protocol::protocol::NetworkApprovalProtocol;
    ...
    fn mcp_inventory_maps_prefix_tool_names_by_server() {
        let statuses = vec![
            McpServerStatus {
                ...
                auth_status: nexal_app_server_protocol::McpAuthStatus::Unsupported,
            },
            McpServerStatus {
    ...
    -> Result<()> {
        ...
        let model = nexal_core::test_support::get_model_offline(config.model.as_deref());
        ...
        while let Ok(event) = app_event_rx.try_recv() {
            match event {
                ...
                AppEvent::NexalOp(Op::UserTurn { items, .. }) => {
                    submitted_items = Some(items);
                }
                _ => {}
            }
    ...
    async fn update_feature_flags_enabling_guardian_selects_guardian_approvals() -> Result<()> {
        ...
        let nexal_home = tempdir()?;
        app.config.nexal_home = nexal_home.path().to_path_buf();
        ...
        let config = std::fs::read_to_string(nexal_home.path().join("config.toml"))?;
    ...
    -> Result<()> {
        ...
        let nexal_home = tempdir()?;
        app.config.nexal_home = nexal_home.path().to_path_buf();
        let config_toml_path = nexal_home.path().join("config.toml").abs();
...
"#;
    ...
    async fn inactive_thread_exec_approval_preserves_context() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                nexal_protocol::protocol::ReviewDecision::ApprovedForSession,
                nexal_protocol::protocol::ReviewDecision::NetworkPolicyAmendment {
                    network_policy_amendment: nexal_protocol::approvals::NetworkPolicyAmendment {
                        host: "example.com".to_string(),
                        action: nexal_protocol::approvals::NetworkPolicyRuleAction::Allow,
                    },
                },
                nexal_protocol::protocol::ReviewDecision::Abort,
            ]
        ...
        app.enqueue_thread_notification(
            ...
            ServerNotification::ThreadStarted(ThreadStartedNotification {
                thread: Thread {
                    ...
                    created_at: 1,
                    updated_at: 2,
                    status: nexal_app_server_protocol::ThreadStatus::Idle,
                    path: Some(rollout_path.clone()),
                    cwd: PathBuf::from("/tmp/agent"),
                    cli_version: "0.0.0".to_string(),
                    source: nexal_app_server_protocol::SessionSource::Unknown,
                    agent_nickname: Some("Robie".to_string()),
                    agent_role: Some("explorer".to_string()),
                    git_info: None,
    ...
    async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()>
    {
        ...
        assert_eq!(
            app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
                nexal_app_server_protocol::SkillsChangedNotification {},
            )),
            None
        );
        Ok(())
    ...
    async fn clear_ui_header_shows_fast_status_only_for_gpt54() {
        ...
        app.chat_widget
            .set_service_tier(Some(nexal_protocol::config_types::ServiceTier::Fast));
    ...
    fn thread_event_store_rebase_preserves_resolved_request_state() {
        ...
        store.push_notification(ServerNotification::ServerRequestResolved(
            nexal_app_server_protocol::ServerRequestResolvedNotification {
                request_id: AppServerRequestId::Integer(1),
                thread_id: thread_id.to_string(),
            },
        ));
    ...
    fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
        let turn_error = AppServerTurnError {
            ...
            nexal_error_info: Some(AppServerNexalErrorInfo::ActiveTurnNotSteerable {
    ...
    async fn model_migration_prompt_skips_when_target_missing_or_hidden() {
        ...
        let mut current = available
            .iter()
            .find(|preset| preset.model == "gpt-5-nexal")
            .cloned()
        ...
        available.retain(|preset| preset.model != "gpt-5-nexal");
        ...
        let target = with_hidden_target
            .iter_mut()
            .find(|preset| preset.model == "gpt-5.2-nexal")
            .expect("target preset present");
        ...
        assert!(target_preset_for_upgrade(&with_hidden_target, "gpt-5.2-nexal").is_none());
    }

    #[tokio::test]
    async fn model_migration_prompt_shows_for_hidden_model() {
        let nexal_home = tempdir().expect("temp nexal home");
        ...
        let current = available_models
            .iter()
            .find(|preset| preset.model == "gpt-5.1-nexal")
            .cloned()
            .expect("gpt-5.1-nexal preset present");
        assert!(
            !current.show_in_picker,
            "expected gpt-5.1-nexal to be hidden from picker for this test"
        );
    ...
    async fn refresh_in_memory_config_from_disk_loads_latest_apps_state() -> Result<()> {
        ...
        ConfigEditsBuilder::new(&app.config.nexal_home)
            .with_edits([
                ConfigEdit::SetPath {
    ...
    -> Result<()> {
        ...
        let nexal_home = tempdir()?;
        app.config.nexal_home = nexal_home.path().to_path_buf();
        std::fs::write(nexal_home.path().join("config.toml"), "[broken")?;
    ...
    async fn refresh_in_memory_config_from_disk_uses_active_chat_widget_cwd() -> Result<()> {
        ...
        app.chat_widget.handle_nexal_event(Event {
            id: String::new(),
    ...
    async fn fresh_session_config_uses_current_service_tier() {
        ...
        assert_eq!(
            config.service_tier,
            Some(nexal_protocol::config_types::ServiceTier::Fast)
        );
    }
    ...
    async fn replace_chat_widget_reseeds_collab_agent_metadata_for_replay() {
        ...
        app.replay_thread_snapshot(
            ThreadEventSnapshot {
                ...
                turns: Vec::new(),
                events: vec![ThreadBufferedEvent::Notification(
                    ServerNotification::ItemStarted(nexal_app_server_protocol::ItemStartedNotification {
                        thread_id: "thread-1".to_string(),
                        turn_id: "turn-1".to_string(),
                        item: ThreadItem::CollabAgentToolCall {
                            id: "wait-1".to_string(),
                            tool: nexal_app_server_protocol::CollabAgentTool::Wait,
                            status: nexal_app_server_protocol::CollabAgentToolCallStatus::InProgress,
    ...
    async fn session_summary_includes_resume_hint() {
        ...
        assert_eq!(
            ...
            Some("nexal resume 123e4567-e89b-12d3-a456-426614174000".to_string())
        );
    }
    ...
    async fn session_summary_prefers_name_over_id() {
        ...
        assert_eq!(
            summary.resume_command,
            Some("nexal resume my-session".to_string())
        );
    }
}
