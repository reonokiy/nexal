...
use nexal_utils_path::normalize_for_path_comparison;
...
pub type StateDbHandle = Arc<nexal_rollout_state::StateRuntime>;
...
pub async fn init(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    ...
    let runtime = match nexal_rollout_state::StateRuntime::init(
    ...
    let backfill_state = match runtime.get_backfill_state().await {
        ...
        Err(err) => {
            warn!(
                ...
                config.nexal_home.display()
            );
            return None;
        }
    };
    if backfill_state.status != nexal_rollout_state::BackfillStatus::Complete {
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = nexal_rollout_state::state_db_path(config.sqlite_home());
...
pub async fn open_if_present(nexal_home: &Path, default_provider: &str) -> Option<StateDbHandle> {
    let db_path = nexal_rollout_state::state_db_path(nexal_home);
    if !tokio::fs::try_exists(&db_path).await.unwrap_or(false) {
        return None;
    }
    let runtime =
        nexal_rollout_state::StateRuntime::init(nexal_home.to_path_buf(), default_provider.to_string())
            .await
            .ok()?;
    require_backfill_complete(runtime, nexal_home).await
}
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == nexal_rollout_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<nexal_rollout_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    ...
    let ctx = context?;
    if ctx.nexal_home() != nexal_home {
        warn!(
            "state db nexal_home mismatch: expected {}, got {}",
            ctx.nexal_home().display(),
            nexal_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => nexal_rollout_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => nexal_rollout_state::SortKey::UpdatedAt,
            },
...
) -> Option<nexal_rollout_state::ThreadsPage> {
    let ctx = context?;
    if ctx.nexal_home() != nexal_home {
        warn!(
            "state db nexal_home mismatch: expected {}, got {}",
