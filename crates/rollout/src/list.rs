...
use crate::state_db;
use nexal_file_search as file_search;
...
impl From<nexal_rollout_state::Anchor> for Cursor {
    fn from(anchor: nexal_rollout_state::Anchor) -> Self {
...
pub async fn get_threads(
    ...
    let root = nexal_home.join(SESSIONS_SUBDIR);
    get_threads_in_root(
        root,
        page_size,
        cursor,
...
///
/// Directory layout: `~/.nexal/sessions/YYYY/MM/DD/rollout-YYYY-MM-DDThh-mm-ss-<uuid>.jsonl`
...
async fn find_thread_path_by_id_str_in_subdir(
    ...
    let state_db_ctx = state_db::open_if_present(nexal_home, "").await;
    ...
    let mut root = nexal_home.to_path_buf();
    root.push(subdir);
    if !root.exists() {
        return Ok(None);
    }
...
pub async fn find_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(nexal_home, SESSIONS_SUBDIR, id_str).await
}
...
pub async fn find_archived_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(nexal_home, ARCHIVED_SESSIONS_SUBDIR, id_str).await
}
