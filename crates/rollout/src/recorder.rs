//! Persist Nexal session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use nexal_utils_string::truncate_middle_chars;
...
use nexal_protocol::protocol::GitInfo as ProtocolGitInfo;
use nexal_protocol::protocol::InitialHistory;
use nexal_protocol::protocol::ResumedHistory;
...
use nexal_utils_path as path_utils;
...
///
/// ```ignore
/// $ jq -C . ~/.nexal/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.nexal/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided Nexal home directory.
    ...
    async fn list_threads_with_db_fallback(
        ...
        let nexal_home = config.nexal_home();
        ...
        let fs_page = if archived {
            let root = nexal_home.join(ARCHIVED_SESSIONS_SUBDIR);
            get_threads_in_root(
                root,
                fs_page_size,
                cursor,
...
fn precompute_log_file_info(
    ...
    // Resolve ~/.nexal/sessions/YYYY/MM/DD path.
    let timestamp = OffsetDateTime::now_local()
        .map_err(|e| IoError::other(format!("failed to get local time: {e}")))?;
    let mut dir = config.nexal_home().to_path_buf();
...
async fn sync_thread_state_after_write(
    ...
    if new_thread_memory_mode.is_some()
        || items
            .iter()
            .any(nexal_rollout_state::rollout_item_affects_thread_metadata)
    {
...
impl From<nexal_rollout_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: nexal_rollout_state::ThreadsPage) -> Self {
        let items = db_page
            .items
            .into_iter()
            .map(|item| ThreadItem {
...
async fn select_resume_path_from_db_page(
    page: &nexal_rollout_state::ThreadsPage,
    filter_cwd: Option<&Path>,
    default_provider: &str,
) -> Option<PathBuf> {
    match filter_cwd {
