...
pub(crate) async fn backfill_sessions(
    ...
    let sessions_root = config.nexal_home().join(SESSIONS_SUBDIR);
    let archived_root = config.nexal_home().join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(config.nexal_home(), &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(nexal_home: &Path, path: &Path) -> String {
    path.strip_prefix(nexal_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
