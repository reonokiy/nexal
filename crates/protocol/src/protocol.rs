//! Defines the protocol for a Nexal session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Nexal:";
...
pub enum Op {
    ...
    /// for a turn of a [`crate::nexal_thread::NexalThread`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    /// This is a local-only operation handled by nexal-core; it does not
    /// involve the model.
    SetThreadName { name: String },

    /// Request Nexal to undo a turn (turn are stacked so it is the same effect as CMD + Z).
    Undo,

    /// Request Nexal to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down nexal instance.
    Shutdown,
...
/// running the command proposed by Nexal.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.nexal`, `.git`, notably `.git/hooks`) under
...
fn default_read_only_subpaths_for_writable_root(
    ...
    // Make .agents/skills and .nexal/config.toml and related files read-only
    // to the agent, by default.
    for subdir in &[".agents", ".nexal"] {
        #[allow(clippy::expect_used)]
        let top_level_nexal = writable_root.join(subdir).expect("valid relative path");
        if top_level_nexal.as_path().is_dir() {
            subpaths.push(top_level_nexal);
        }
    }
...
/// Nexal errors that we expose to clients.
...
impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.nexal_error_info
            .as_ref()
            .is_none_or(NexalErrorInfo::affects_turn_status)
    }
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::Nexal),
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(nexal_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                nexal_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_nexal() {
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("nexal".to_string()).restriction_product(),
            Some(Product::Nexal)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::Nexal])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::Nexal]));
        assert!(
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".nexal")).expect("create .nexal");
        let canonical_cwd = cwd.path().canonicalize().expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_nexal = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".nexal"))
            .expect("canonical .nexal");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_nexal.as_path())
        );
    }
