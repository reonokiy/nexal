...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer Nexal degrades to warn-and-ignore
    /// instead of failing to load. Nexal 0.112.0 rejected unknown values here,
...
impl FileSystemSandboxPolicy {
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            // Example: if `<root>/.nexal -> <root>/decoy`, bwrap must still see
            // `<root>/.nexal`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.nexal` so downstream sandboxes can still detect and mask
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "NEXAL_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    fn effective_runtime_roots_canonicalize_symlinked_paths() {
        ...
        let nexal_dir = real_root.join(".nexal");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&nexal_dir).expect("create .nexal");
        ...
        let expected_nexal = AbsolutePathBuf::from_absolute_path(
            nexal_dir.canonicalize().expect("canonicalize .nexal"),
        )
        .expect("absolute canonical .nexal");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-nexal");
        let dot_nexal = root.join(".nexal");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_nexal).expect("create .nexal symlink");

        let root = AbsolutePathBuf::from_absolute_path(&root).expect("absolute root");
        let expected_dot_nexal = AbsolutePathBuf::from_absolute_path(
            root.as_path()
                .canonicalize()
                .expect("canonicalize root")
                .join(".nexal"),
        )
        .expect("absolute .nexal symlink");
    ...
    fn tmpdir_special_path_canonicalizes_symlinked_tmpdir() {
        ...
        let nexal_dir = real_tmpdir.join(".nexal");
