...
pub enum AuthCredentialsStoreMode {
    ...
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in NEXAL_HOME.
    Auto,
...
const KEYRING_SERVICE: &str = "Nexal Auth";

// turns nexal_home path into a stable, short key string
fn compute_store_key(nexal_home: &Path) -> std::io::Result<String> {
    let canonical = nexal_home
        .canonicalize()
        .unwrap_or_else(|_| nexal_home.to_path_buf());
...
impl KeyringAuthStorage {
    fn new(nexal_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            nexal_home,
            keyring_store,
        }
...
impl AuthStorageBackend for KeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.nexal_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.nexal_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.nexal_home)?;
...
impl AutoAuthStorage {
    fn new(nexal_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            keyring_storage: Arc::new(KeyringAuthStorage::new(nexal_home.clone(), keyring_store)),
            file_storage: Arc::new(FileAuthStorage::new(nexal_home)),
        }
    }
}
...
// A global in-memory store for mapping nexal_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(nexal_home: PathBuf) -> Self {
        Self { nexal_home }
    }
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_keyring_store(nexal_home, mode, keyring_store)
}
...
fn create_auth_storage_with_keyring_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(nexal_home)),
        AuthCredentialsStoreMode::Keyring => {
            Arc::new(KeyringAuthStorage::new(nexal_home, keyring_store))
        }
        AuthCredentialsStoreMode::Auto => Arc::new(AutoAuthStorage::new(nexal_home, keyring_store)),
        AuthCredentialsStoreMode::Ephemeral => Arc::new(EphemeralAuthStorage::new(nexal_home)),
    }
}
