//! Default Nexal HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! reqwest/`NexalHttpClient` construction.
//!
//! Use [`crate::default_client`] or [`nexal_login::default_client`] from other crates in this
//! workspace.

use nexal_client::BuildCustomCaTransportError;
use nexal_client::NexalHttpClient;
pub use nexal_client::NexalRequestBuilder;
use nexal_client::build_reqwest_client_with_custom_ca;
...
/// Parenthesis will be added by Nexal. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "nexal_cli_rs";
pub const NEXAL_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "NEXAL_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-nexal-residency";
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(NEXAL_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(NEXAL_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "nexal_vscode"
        || originator_value.starts_with("Nexal ")
}

pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "nexal_atlas" || originator_value == "nexal_chatgpt_desktop"
}

pub fn get_nexal_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Nexal user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Nexal user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Nexal originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
pub fn create_client() -> NexalHttpClient {
    let inner = build_reqwest_client();
    NexalHttpClient::new(inner)
}

/// Builds the default reqwest client used for ordinary Nexal HTTP traffic.
///
/// This starts from the standard Nexal user agent, default headers, and sandbox-specific proxy
/// policy, then layers in shared custom CA handling from `NEXAL_CA_CERTIFICATE` /
...
/// Tries to build the default reqwest client used for ordinary Nexal HTTP traffic.
///
