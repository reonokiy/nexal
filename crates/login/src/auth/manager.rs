...
impl NexalAuth {
    fn from_auth_dot_json(
        ...
        match auth_mode {
            ApiAuthMode::Chatgpt => {
                let storage = create_auth_storage(nexal_home.to_path_buf(), storage_mode);
...
impl ChatgptAuth {
    ...
    fn client(&self) -> &NexalHttpClient {
        &self.state.client
    }
}
...
pub fn read_nexal_api_key_from_env() -> Option<String> {
    env::var(NEXAL_API_KEY_ENV_VAR)
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}

/// Delete the auth.json file inside `nexal_home` if it exists. Returns `Ok(true)`
...
pub fn login_with_api_key(
    ...
    save_auth(nexal_home, &auth_dot_json, auth_credentials_store_mode)
}
...
fn logout_all_stores(
    ...
    if auth_credentials_store_mode == AuthCredentialsStoreMode::Ephemeral {
        return logout(nexal_home, AuthCredentialsStoreMode::Ephemeral);
    }
    let removed_ephemeral = logout(nexal_home, AuthCredentialsStoreMode::Ephemeral)?;
    let removed_managed = logout(nexal_home, auth_credentials_store_mode)?;
...
fn load_auth(
    ...
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<Option<NexalAuth>> {
    let build_auth = |auth_dot_json: AuthDotJson, storage_mode| {
        NexalAuth::from_auth_dot_json(nexal_home, auth_dot_json, storage_mode)
    };

    // API key via env var takes precedence over any other auth method.
    if enable_nexal_api_key_env && let Some(api_key) = read_nexal_api_key_from_env() {
...
impl Debug for CachedAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CachedAuth")
            .field(
                "auth_mode",
                &self.auth.as_ref().map(NexalAuth::api_auth_mode),
            )
            .field(
                "external_refresher",
...
impl UnauthorizedRecovery {
    fn new(manager: Arc<AuthManager>) -> Self {
        ...
        let expected_account_id = cached_auth.as_ref().and_then(NexalAuth::get_account_id);
        let mode = if cached_auth
            .as_ref()
            .is_some_and(NexalAuth::is_external_chatgpt_tokens)
        {
...
impl AuthManager {
    ...
    /// Create an AuthManager with a specific NexalAuth, for testing only.
    pub fn from_auth_for_testing(auth: NexalAuth) -> Arc<Self> {
        ...
        Arc::new(Self {
            nexal_home: PathBuf::from("non-existent"),
    ...
    /// Create an AuthManager with a specific NexalAuth and codex home, for testing only.
    ...
    pub fn auth_cached(&self) -> Option<NexalAuth> {
        self.inner.read().ok().and_then(|c| c.auth.clone())
    }

    pub fn refresh_failure_for_auth(&self, auth: &NexalAuth) -> Option<RefreshTokenFailedError> {
    ...
    pub async fn auth(&self) -> Option<NexalAuth> {
    ...
    fn reload_if_account_id_matches(&self, expected_account_id: Option<&str>) -> ReloadOutcome {
        ...
        let new_auth = self.load_auth_from_storage();
        let new_account_id = new_auth.as_ref().and_then(NexalAuth::get_account_id);
    ...
    fn auths_equal_for_refresh(a: Option<&NexalAuth>, b: Option<&NexalAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
    ...
    fn auths_equal(a: Option<&NexalAuth>, b: Option<&NexalAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => a == b,
            _ => false,
    ...
    fn load_auth_from_storage(&self) -> Option<NexalAuth> {
        load_auth(
            &self.nexal_home,
            self.enable_nexal_api_key_env,
    ...
    fn set_cached_auth(&self, new_auth: Option<NexalAuth>) -> bool {
        if let Ok(mut guard) = self.inner.write() {
    ...
    pub fn nexal_api_key_env_enabled(&self) -> bool {
        self.enable_nexal_api_key_env
    }
    ...
    pub async fn refresh_token(&self) -> Result<(), RefreshTokenError> {
        ...
        if auth_before_reload
            .as_ref()
            .is_some_and(NexalAuth::is_api_key_auth)
        {
            return Ok(());
        }
        let expected_account_id = auth_before_reload
            .as_ref()
            .and_then(NexalAuth::get_account_id);
    ...
    async fn refresh_token_from_authority_impl(&self) -> Result<(), RefreshTokenError> {
        ...
        let result = match auth {
            NexalAuth::ChatgptAuthTokens(_) => {
                self.refresh_external_auth(ExternalAuthRefreshReason::Unauthorized)
                    .await
            }
            NexalAuth::Chatgpt(chatgpt_auth) => {
    ...
    pub fn get_api_auth_mode(&self) -> Option<ApiAuthMode> {
        self.auth_cached().as_ref().map(NexalAuth::api_auth_mode)
    }

    pub fn auth_mode(&self) -> Option<crate::AuthMode> {
        self.auth_cached().as_ref().map(NexalAuth::auth_mode)
    }

    fn is_stale_for_proactive_refresh(auth: &NexalAuth) -> bool {
        let chatgpt_auth = match auth {
            NexalAuth::Chatgpt(chatgpt_auth) => chatgpt_auth,
            _ => return false,
        };
