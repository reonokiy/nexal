...
`nexal-network-proxy` is Nexal's local network policy enforcement proxy. It runs:
...
`nexal-network-proxy` reads from Nexal's merged `config.toml` (via `nexal-core` config loading).
...
mitm = false
# CA cert/key are managed internally under $NEXAL_HOME/proxy/ (ca.pem + ca.key).
...
```bash
cargo run -p nexal-network-proxy --
```
...
`nexal-network-proxy` can be embedded as a library with a thin API:

```rust
use nexal_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `nexal-network-proxy` is embedded in managed Nexal runtime, policy decisions emit structured
OTEL-compatible events with `target=nexal_otel.network_proxy`.
...
This section documents the protections implemented by `nexal-network-proxy`, and the boundaries of
