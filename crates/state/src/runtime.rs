...
impl StateRuntime {
    /// Initialize the state runtime using the provided Nexal home and default provider.
    ///
    /// This opens (and migrates) the SQLite databases under `nexal_home`,
    /// keeping logs in a dedicated file to reduce lock contention with the
    /// rest of the state store.
    pub async fn init(nexal_home: PathBuf, default_provider: String) -> anyhow::Result<Arc<Self>> {
        ...
        .await;
        let state_path = state_db_path(nexal_home.as_path());
        let logs_path = logs_db_path(nexal_home.as_path());
    ...
    /// Return the configured Nexal home directory for this runtime.
    pub fn nexal_home(&self) -> &Path {
...
pub fn state_db_path(nexal_home: &Path) -> PathBuf {
    nexal_home.join(state_db_filename())
}
...
pub fn logs_db_path(nexal_home: &Path) -> PathBuf {
    nexal_home.join(logs_db_filename())
}
...
async fn remove_legacy_db_files(
    ...
    let mut entries = match tokio::fs::read_dir(nexal_home).await {
        Ok(entries) => entries,
        Err(err) => {
            warn!(
                "failed to read nexal_home for {db_label} db cleanup {}: {err}",
                nexal_home.display(),
            );
            return;
        }
