...
use chrono::DateTime;
use clap::Parser;
use nexal_rollout_state::LogQuery;
use nexal_rollout_state::LogRow;
use nexal_rollout_state::StateRuntime;
use dirs::home_dir;
use owo_colors::OwoColorize;

#[derive(Debug, Parser)]
#[command(name = "nexal-rollout-state-logs")]
#[command(about = "Tail Nexal logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to NEXAL_HOME. Defaults to $NEXAL_HOME or ~/.nexal.
    #[arg(long, env = "NEXAL_HOME")]
    nexal_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --nexal-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let nexal_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(nexal_home, "logs-client".to_string()).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let nexal_home = args.nexal_home.clone().unwrap_or_else(default_nexal_home);
    Ok(nexal_rollout_state::logs_db_path(nexal_home.as_path()))
}

fn default_nexal_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".nexal");
    }
    PathBuf::from(".nexal")
}
