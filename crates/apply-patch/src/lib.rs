...
/// Special argv[1] flag used when the Nexal executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `nexal-apply-patch` (to avoid forcing
/// `nexal-arg0` to depend on `nexal-core`), it is part of the "nexal core"
/// process-invocation contract between the apply-patch runtime and the arg0
/// dispatcher.
pub const NEXAL_CORE_APPLY_PATCH_ARG1: &str = "--nexal-run-as-apply-patch";
