...
use nexal_apply_patch::NEXAL_CORE_APPLY_PATCH_ARG1;
use nexal_sandboxing::landlock::NEXAL_LINUX_SANDBOX_ARG0;
use nexal_utils_home_dir::find_nexal_home;
#[cfg(unix)]
...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "nexal-execve-wrapper";
const LOCK_FILENAME: &str = ".lock";
const TOKIO_WORKER_STACK_SIZE_BYTES: usize = 16 * 1024 * 1024;

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Arg0DispatchPaths {
    /// Stable path to the current Nexal executable for child re-execs.
    ///
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime.block_on(
            nexal_shell_escalation::run_shell_escalation_execve_wrapper(file, argv),
        );
        match exit_code {
    ...
    if exe_name == NEXAL_LINUX_SANDBOX_ARG0 {
        // Safety: [`run_main`] never returns.
        nexal_linux_sandbox::run_main();
    } else if exe_name == APPLY_PATCH_ARG0 || exe_name == MISSPELLED_APPLY_PATCH_ARG0 {
        nexal_apply_patch::main();
    }

    let argv1 = args.next().unwrap_or_default();
    if argv1 == NEXAL_CORE_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                match nexal_apply_patch::apply_patch(&patch_arg, &mut stdout, &mut stderr) {
                    Ok(()) => 0,
                    Err(_) => 1,
                }
            }
            None => {
                eprintln!("Error: {NEXAL_CORE_APPLY_PATCH_ARG1} requires a UTF-8 PATCH argument.");
                1
            }
        };
    ...
    match prepend_path_entry_for_nexal_aliases() {
        Ok(path_entry) => Some(path_entry),
        Err(err) => {
            // It is possible that Nexal will proceed successfully even if
...
/// While we want to deploy the Nexal CLI as a single executable for simplicity,
...
/// `nexal-linux-sandbox` we *directly* execute
/// [`nexal_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.nexal/.env` before creating any threads.
/// 2.  Construct a Tokio multi-thread runtime.
/// 3.  Capture the current executable path and derive the
///     `nexal-linux-sandbox` helper path (falling back to the current
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    ...
    runtime.block_on(async move {
        ...
        let paths = Arg0DispatchPaths {
            nexal_self_exe: current_exe.clone(),
            nexal_linux_sandbox_exe: if cfg!(target_os = "linux") {
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `nexal-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().nexal_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "NEXAL_";

/// Load env vars from ~/.nexal/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `NEXAL_`.
fn load_dotenv() {
    if let Ok(nexal_home) = find_nexal_home()
        && let Ok(iter) = dotenvy::from_path_iter(nexal_home.join(".env"))
    {
        set_filtered(iter);
    }
}

/// Helper to set vars from a dotenvy iterator while filtering out `NEXAL_` keys.
...
///   with the "secret" --nexal-run-as-apply-patch flag.
///
...
fn prepend_path_entry_for_nexal_aliases() -> std::io::Result<Arg0PathEntryGuard> {
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        if nexal_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (nexal_home: {nexal_home:?})"
                ),
            ));
        }
    }

    std::fs::create_dir_all(&nexal_home)?;
    // Use a NEXAL_HOME-scoped temp root to avoid cluttering the top-level directory.
    let temp_root = nexal_home.join("tmp").join("arg0");
...
"#,
    ...
    let paths = Arg0DispatchPaths {
        nexal_self_exe: std::env::current_exe().ok(),
        nexal_linux_sandbox_exe: {
            #[cfg(target_os = "linux")]
            {
                Some(path.join(NEXAL_LINUX_SANDBOX_ARG0))
            }
...
mod tests {
    ...
    #[test]
    fn linux_sandbox_exe_path_prefers_nexal_linux_sandbox_alias() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let lock_file = create_lock(temp_dir.path())?;
        let alias_path = temp_dir.path().join("nexal-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                nexal_self_exe: Some(PathBuf::from("/usr/bin/nexal")),
                nexal_linux_sandbox_exe: Some(alias_path.clone()),
                main_execve_wrapper_exe: None,
            },
        );

        assert_eq!(
            linux_sandbox_exe_path(Some(&path_entry), Some(PathBuf::from("/usr/bin/nexal"))),
            Some(alias_path),
        );
        Ok(())
    }
