...
mod tests {
    ...
    {
        let frame = timeout(Duration::from_secs(1), websocket.next()).await;
        let frame = match frame {
            Ok(frame) => frame,
            Err(err) => panic!("timed out waiting for JSON-RPC message: {err}"),
        };
        let frame = match frame {
            Some(Ok(frame)) => frame,
            Some(Err(err)) => panic!("failed to read JSON-RPC websocket frame: {err}"),
            None => panic!("server connection closed before JSON-RPC message arrived"),
        };
        match frame {
            Message::Text(text) => match serde_json::from_str::<JSONRPCMessage>(text.as_ref()) {
                Ok(message) => message,
                Err(err) => panic!("failed to parse JSON-RPC text frame: {err}"),
            },
            Message::Binary(bytes) => {
                match serde_json::from_slice::<JSONRPCMessage>(bytes.as_ref()) {
                    Ok(message) => message,
                    Err(err) => panic!("failed to parse JSON-RPC binary frame: {err}"),
                }
            }
        ...
        S: AsyncRead + AsyncWrite + Unpin,
    {
        ...
        if let Err(err) = websocket.send(Message::Text(encoded.into())).await {
            panic!("failed to write JSON-RPC message: {err}");
        }
    }
    ...
    async fn rpc_client_matches_out_of_order_responses_by_request_id() {
        ...
        let server = tokio::spawn(async move {
            ...
            let first = read_jsonrpc_message(&mut websocket).await;
            let second = read_jsonrpc_message(&mut websocket).await;
        ...
        let (client, _events_rx) = RpcClient::new(JsonRpcConnection::from_websocket(
            client_ws,
            "test-rpc".to_string(),
        ));
