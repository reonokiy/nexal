...
use nexal_protocol::protocol::ListCustomPromptsResponseEvent;
...
fn rate_limit_error_kind(info: &NexalErrorInfo) -> Option<RateLimitErrorKind> {
    match info {
        NexalErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        NexalErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        NexalErrorInfo::ResponseTooManyFailedAttempts {
...
impl ChatWidget {
    ...
    fn on_session_configured(&mut self, event: nexal_protocol::protocol::SessionConfiguredEvent) {
        ...
        // Ask nexal-core to enumerate custom prompts for this session.
    ...
    fn handle_steer_rejected_error(&mut self, nexal_error_info: &NexalErrorInfo) -> bool {
        matches!(
            nexal_error_info,
            NexalErrorInfo::ActiveTurnNotSteerable { .. }
    ...
    fn on_mcp_startup_complete(&mut self, ev: McpStartupCompleteEvent) {
        let nexal_apps_ready = ev.ready.iter().any(|server| server == "nexal_apps");
        ...
        self.update_task_running_state();
        self.maybe_send_next_queued_input();
        if self.connectors_enabled() && nexal_apps_ready {
    ...
    pub(crate) fn new(common: ChatWidgetInit, thread_manager: Arc<ThreadManager>) -> Self {
        ...
        let nexal_op_tx = spawn_agent(config.clone(), app_event_tx.clone(), thread_manager);
    ...
    pub(crate) fn new_from_existing(
        common: ChatWidgetInit,
        conversation: std::sync::Arc<nexal_core::NexalThread>,
    ...
    fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::Compact => {
                ...
                self.app_event_tx.send(AppEvent::NexalOp(Op::Compact));
            }
            SlashCommand::Review => {
            ...
            SlashCommand::TestApproval => {
                ...
                self.app_event_tx.send(AppEvent::NexalEvent(Event {
                    id: "1".to_string(),
    ...
    fn dispatch_command_with_args(
        ...
        match cmd {
            ...
            SlashCommand::Rename if !trimmed.is_empty() => {
                ...
                self.request_redraw();
                self.app_event_tx
                    .send(AppEvent::NexalOp(Op::SetThreadName { name }));
                self.bottom_pane.drain_pending_submission_state();
            }
    ...
    fn show_rename_prompt(&mut self) {
        ...
        let view = CustomPromptView::new(
            ...
            Box::new(move |name: String| {
                ...
                tx.send(AppEvent::NexalOp(Op::SetThreadName { name }));
            }),
        );
    ...
    fn dispatch_event_msg(
        ...
        match msg {
            ...
            EventMsg::Error(ErrorEvent {
                ...
                {
                } else if let Some(kind) = nexal_error_info.as_ref().and_then(rate_limit_error_kind)
                {
                    match kind {
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::NexalOp(Op::OverrideTurnContext {
                cwd: None,
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(&mut self, preset: ApprovalPreset) {
        ...
        header.push(*Box::new(
            Paragraph::new(vec![
                line!["Set up the Nexal agent sandbox to protect your files and control network access. Learn more <#>"],
            ])
            .wrap(Wrap { trim: false }),
        ));
    ...
    /// Forward an `Op` directly to nexal.
    pub(crate) fn submit_op(&mut self, op: Op) -> bool {
        ...
        if let Err(e) = self.nexal_op_tx.send(op) {
    ...
    pub(crate) fn open_review_popup(&mut self) {
        ...
        items.push(SelectionItem {
            ...
            actions: vec![Box::new(move |tx: &AppEventSender| {
                tx.send(AppEvent::NexalOp(Op::Review {
                    review_request: ReviewRequest {
        ...
        for option in branches {
            ...
            items.push(SelectionItem {
                ...
                actions: vec![Box::new(move |tx3: &AppEventSender| {
                    tx3.send(AppEvent::NexalOp(Op::Review {
                        review_request: ReviewRequest {
