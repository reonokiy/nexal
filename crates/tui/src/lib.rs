...
// The standalone `nexal-tui` binary prints a short help message before the
...
use nexal_core::find_thread_path_by_name_str;
...
pub(crate) use nexal_tui_render::key_hint;
mod markdown_stream;
mod mention_codec;
mod model_migration;
mod multi_agents;
...
mod selection_list;
mod session_log;
pub(crate) use nexal_tui_render::shimmer;
mod skills_helpers;
pub(crate) use nexal_tui_render::slash_command;
mod status;
mod status_indicator_widget;
mod streaming;
pub(crate) use nexal_tui_render::style;
mod terminal_title;
...
mod tooltips;
mod tui;
pub(crate) use nexal_tui_render::ui_consts;
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = nexal_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    let nexal_home = match find_nexal_home() {
        Ok(nexal_home) => nexal_home.to_path_buf(),
        Err(err) => {
    ...
    if let Err(err) =
        nexal_core::personality_migration::maybe_migrate_personality(&nexal_home, &config_toml)
            .await
    {
    ...
    let model_provider_override = if cli.oss {
        ...
        if let Some(provider) = resolved {
            ...
            let provider = oss_selection::select_oss_provider(&nexal_home).await?;
    ...
    // Nexal CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("nexal-tui.log"))?;

    // Wrap file in non‑blocking writer.
    let (non_blocking, _guard) = non_blocking(log_file);

    // use RUST_LOG env var, default to info for nexal crates.
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("nexal_core=info,nexal_tui=info,nexal_rmcp_client=info")
        })
    };
...
async fn run_ratatui_app(
            ...
            match update_prompt::run_update_prompt_if_needed(&mut tui, &initial_config).await? {
                ...
                UpdatePromptOutcome::RunUpdate(action) => {
                    ...
                    return Ok(AppExitInfo {
                        token_usage: nexal_protocol::protocol::TokenUsage::default(),
                        thread_id: None,
                        thread_name: None,
    ...
    let mut missing_session_exit = |id_str: &str, action: &str| {
        ...
        Ok(AppExitInfo {
            ...
            exit_reason: ExitReason::Fatal(format!(
                "No saved session found with ID {id_str}. Run `nexal {action}` without an ID to choose from existing sessions."
            )),
        })
    };
    ...
    let session_selection = if use_fork {
        if let Some(id_str) = cli.fork_session_id.as_deref() {
            ...
            let path = if is_uuid {
                find_thread_path_by_id_str(&config.nexal_home, id_str).await?
            } else {
                find_thread_path_by_name_str(&config.nexal_home, id_str).await?
            };
            match path {
                Some(path) => {
                    let thread_id =
                        ...
                        .await
                        {
                            ...
                            None => {
                                ...
                                return Ok(AppExitInfo {
                                    ...
                                    update_action: None,
                                    exit_reason: ExitReason::Fatal(format!(
                                        "Found latest saved session at {rollout_path}, but failed to read its metadata. Run `nexal fork` to choose from existing sessions."
                                    )),
                                });
                            }
                        }
                ...
                match resolve_session_thread_id(path.as_path(), /*id_str_if_uuid*/ None).await {
                    ...
                    None => {
                        ...
                        return Ok(AppExitInfo {
                            ...
                            update_action: None,
                            exit_reason: ExitReason::Fatal(format!(
                                "Found latest saved session at {rollout_path}, but failed to read its metadata. Run `nexal resume` to choose from existing sessions."
                            )),
                        });
                    }
                }
...
mod tests {
    ...
    use nexal_core::config::ConfigBuilder;
    use nexal_core::config::ConfigOverrides;
    use nexal_core::config::ProjectConfig;
    ...
    fn build_turn_context(config: &Config, cwd: PathBuf) -> TurnContextItem {
        ...
        TurnContextItem {
            ...
            summary: config
                ...
                .unwrap_or(nexal_protocol::config_types::ReasoningSummary::Auto),
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let nexal_home = temp_dir.path().to_path_buf();
...
"#
    ...
    async fn read_session_cwd_prefers_sqlite_when_thread_id_present() -> std::io::Result<()> {
        ...
        let runtime = nexal_rollout_state::StateRuntime::init(
        ...
        let mut builder = nexal_rollout_state::ThreadMetadataBuilder::new(
            thread_id,
            rollout_path.clone(),
            chrono::Utc::now(),
            SessionSource::Cli,
