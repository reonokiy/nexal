...
pub(crate) fn migration_copy_for_models(
    ...
    let heading_text = Span::from(format!(
        "Nexal just got an upgrade. Introducing {target_display_name}."
    ))
    .bold();
...
mod tests {
    ...
    /// Outcome of the migration prompt (test-only; production path is stubbed).
    #[derive(Clone, Copy, Debug, PartialEq, Eq)]
    enum ModelMigrationOutcome {
        Accepted,
        Rejected,
        Exit,
    }
    ...
    impl ModelMigrationScreen {
        ...
        fn render_menu(&self, column: &mut ColumnRenderable) {
            ...
            column.push(
                Paragraph::new("Choose how you'd like Nexal to proceed.")
                    .wrap(Wrap { trim: false })
                    .inset(Insets::tlbr(
                        /*top*/ 0, /*left*/ 2, /*bottom*/ 0, /*right*/ 0,
                    )),
    ...
    fn prompt_snapshot() {
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                ...
                None,
                Some(
                    "Upgrade to gpt-5.2-nexal for the latest and greatest agentic coding model."
                        .to_string(),
                ),
                None,
                "gpt-5.1-nexal-max".to_string(),
                Some("Nexal-optimized flagship for deep and fast reasoning.".to_string()),
                true,
            ),
        );
    ...
    fn prompt_snapshot_gpt5_family() {
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-5",
                "gpt-5.1",
                Some("https://www.nexal.com/models/gpt-5.1".to_string()),
                None,
                None,
                "gpt-5.1".to_string(),
    ...
    fn prompt_snapshot_gpt5_nexal() {
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-5-nexal",
                "gpt-5.1-nexal-max",
                Some("https://www.nexal.com/models/gpt-5.1-nexal-max".to_string()),
                None,
                None,
                "gpt-5.1-nexal-max".to_string(),
                Some("Nexal-optimized flagship for deep and fast reasoning.".to_string()),
        ...
        terminal.flush().expect("flush");
        assert_snapshot!("model_migration_prompt_gpt5_nexal", terminal.backend());
    }

    #[test]
    fn prompt_snapshot_gpt5_nexal_mini() {
        let backend = VT100Backend::new(60, 22);
        let mut terminal = Terminal::with_options(backend).expect("terminal");
        terminal.set_viewport_area(Rect::new(0, 0, 60, 22));
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-5-nexal-mini",
                "gpt-5.1-nexal-mini",
                Some("https://www.nexal.com/models/gpt-5.1-nexal-mini".to_string()),
                None,
                None,
                "gpt-5.1-nexal-mini".to_string(),
                Some("Optimized for nexal. Cheaper, faster, but less capable.".to_string()),
                false,
            ),
        );
        {
        ...
        terminal.flush().expect("flush");
        assert_snapshot!("model_migration_prompt_gpt5_nexal_mini", terminal.backend());
    }
    ...
    fn escape_key_accepts_prompt() {
        let mut screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-old",
                "gpt-new",
                Some("https://www.nexal.com/models/gpt-new".to_string()),
                None,
                None,
                "gpt-new".to_string(),
