...
pub enum Feature {
    ...
    ImageDetailOriginal,
    /// Compress request bodies (zstd) when sending streaming requests to nexal-backend.
    ...
    match feature {
        ...
        _ => {
            ...
            let details = if alias == canonical {
                ...
                Some(format!(
                    "Enable it with `--enable {canonical}` or `[features].{canonical}` in config.toml. See https://developers.openai.com/nexal/config-basic#feature-flags for details."
                ))
            };
            (summary, details)
        }
...
pub const FEATURES: &[FeatureSpec] = &[
    ...
    FeatureSpec {
        ...
        stage: Stage::Experimental {
            ...
            announcement: "NEW: JavaScript REPL is now available in /experimental. Enable it, then start a new chat or restart Nexal to use it.",
        },
        default_enabled: false,
    },
    FeatureSpec {
    ...
    FeatureSpec {
        id: Feature::NexalGitCommit,
        key: "nexal_git_commit",
        stage: Stage::UnderDevelopment,
        default_enabled: false,
    },
    FeatureSpec {
