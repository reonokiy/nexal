...
async fn websocket_transport_routes_per_connection_handshake_and_responses() -> Result<()> {
    ...
    let (mut process, bind_addr) = spawn_websocket_server(nexal_home.path()).await?;
...
async fn websocket_transport_rejects_missing_and_invalid_capability_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let nexal_home = TempDir::new()?;
    let token_file = nexal_home.path().join("app-server-token");
    ...
    let (mut process, bind_addr) =
        spawn_websocket_server_with_args(nexal_home.path(), "ws://127.0.0.1:0", &auth_args).await?;
...
async fn websocket_transport_verifies_signed_short_lived_bearer_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let nexal_home = TempDir::new()?;
    let shared_secret_file = nexal_home.path().join("app-server-signing-secret");
    ...
    let auth_args = vec![
        ...
        "--ws-issuer".to_string(),
        "nexal-enroller".to_string(),
        "--ws-audience".to_string(),
        "nexal-app-server".to_string(),
...
async fn websocket_transport_allows_unauthenticated_non_loopback_startup_by_default() -> Result<()>
{
    ...
    let (mut process, bind_addr) =
        spawn_websocket_server_with_args(nexal_home.path(), "ws://0.0.0.0:0", &[]).await?;
...
pub(super) async fn spawn_websocket_server(nexal_home: &Path) -> Result<(Child, SocketAddr)> {
    spawn_websocket_server_with_args(nexal_home, "ws://127.0.0.1:0", &[]).await
}
