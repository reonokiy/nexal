...
fn create_config_toml_custom_provider(nexal_home: &Path) -> std::io::Result<()> {
...
fn create_config_toml(nexal_home: &Path) -> std::io::Result<()> {
    let config_toml = nexal_home.join("config.toml");
    std::fs::write(
        config_toml,
        r#"
...
fn create_config_toml_forced_login(nexal_home: &Path, forced_method: &str) -> std::io::Result<()> {
    let config_toml = nexal_home.join("config.toml");
    let contents = format!(
        r#"
model = "mock-model"
...
async fn get_auth_status_no_auth() -> Result<()> {
    ...
    create_config_toml(nexal_home.path())?;
...
async fn get_auth_status_with_api_key_when_auth_not_required() -> Result<()> {
    let nexal_home = TempDir::new()?;
    create_config_toml_custom_provider(nexal_home.path())?;
...
async fn login_api_key_rejected_when_forced_chatgpt() -> Result<()> {
    let nexal_home = TempDir::new()?;
    create_config_toml_forced_login(nexal_home.path(), "chatgpt")?;
