...
pub const DEFAULT_CLIENT_NAME: &str = "nexal-app-server-tests";

impl McpProcess {
    pub async fn new(nexal_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(nexal_home, &[], &[]).await
    }

    pub async fn new_with_args(nexal_home: &Path, args: &[&str]) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(nexal_home, &[], args).await
    }
    ...
    pub async fn new_with_env(
        ...
        Self::new_with_env_and_args(nexal_home, env_overrides, &[]).await
    }
    ...
    async fn new_with_env_and_args(
        ...
        let program = nexal_utils_cargo_bin::cargo_bin("nexal-app-server")
            .context("should find binary for nexal-app-server")?;
        let mut cmd = Command::new(program);

        cmd.stdin(Stdio::piped());
        cmd.stdout(Stdio::piped());
        cmd.stderr(Stdio::piped());
        cmd.current_dir(nexal_home);
        cmd.env("NEXAL_HOME", nexal_home);
        cmd.env("RUST_LOG", "info");
        cmd.env_remove(NEXAL_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("nexal-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: nexal_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `nexal-app-server` child process.
        //
