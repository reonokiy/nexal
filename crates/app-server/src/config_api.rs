...
use nexal_core::config::ConfigServiceError;
...
impl ConfigApi {
    ...
    pub(crate) async fn load_latest_config(
        ...
        let mut config = nexal_core::config::ConfigBuilder::default()
            .nexal_home(self.nexal_home.clone())
...
pub(crate) fn protected_feature_keys(
    config_layer_stack: &nexal_core::config_loader::ConfigLayerStack,
) -> BTreeSet<String> {
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(nexal_app_server_protocol::AskForApproval::from)
                .collect()
        }),
...
) -> nexal_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => nexal_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: nexal_core::config_loader::NetworkRequirementsToml,
) -> NetworkRequirements {
    NetworkRequirements {
    ...
    fn map_requirements_toml_to_api_converts_core_enums() {
        let requirements = ConfigRequirementsToml {
            ...
            allowed_web_search_modes: Some(vec![
                nexal_core::config_loader::WebSearchModeRequirement::Cached,
            ]),
            guardian_developer_instructions: None,
            feature_requirements: Some(nexal_core::config_loader::FeatureRequirementsToml {
        ...
        assert_eq!(
            ...
            Some(vec![
                nexal_app_server_protocol::AskForApproval::Never,
                nexal_app_server_protocol::AskForApproval::OnRequest,
            ])
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(nexal_app_server_protocol::ResidencyRequirement::Us),
        );
        assert_eq!(
            mapped.network,
            ...
            .cloud_requirements(CloudRequirementsLoader::new(async {
                Ok(Some(ConfigRequirementsToml {
                    feature_requirements: Some(
                        nexal_core::config_loader::FeatureRequirementsToml {
    ...
    async fn batch_write_reloads_user_config_when_requested() {
        ...
        let response = config_api
            .batch_write(ConfigBatchWriteParams {
                edits: vec![nexal_app_server_protocol::ConfigEdit {
                    key_path: "model".to_string(),
                    value: json!("gpt-5"),
                    merge_strategy: nexal_app_server_protocol::MergeStrategy::Replace,
                }],
        ...
        assert_eq!(
            response,
            ConfigWriteResponse {
                status: nexal_app_server_protocol::WriteStatus::Ok,
                version: response.version.clone(),
                file_path: nexal_utils_absolute_path::AbsolutePathBuf::try_from(
