...
use nexal_app_server_protocol::CommandExecOutputDeltaNotification;
use nexal_app_server_protocol::CommandExecOutputStream;
use nexal_app_server_protocol::CommandExecResizeParams;
use nexal_app_server_protocol::CommandExecResizeResponse;
use nexal_app_server_protocol::CommandExecResponse;
use nexal_app_server_protocol::CommandExecTerminalSize;
use nexal_app_server_protocol::CommandExecTerminateParams;
use nexal_app_server_protocol::CommandExecTerminateResponse;
use nexal_app_server_protocol::CommandExecWriteParams;
use nexal_app_server_protocol::CommandExecWriteResponse;
use nexal_app_server_protocol::JSONRPCErrorError;
use nexal_app_server_protocol::ServerNotification;
use nexal_core::bytes_to_string_smart;
use nexal_core::config::StartedNetworkProxy;
use nexal_core::exec::DEFAULT_EXEC_COMMAND_TIMEOUT_MS;
use nexal_core::exec::ExecExpiration;
use nexal_core::exec::IO_DRAIN_TIMEOUT_MS;
use nexal_core::sandboxing::ExecRequest;
use nexal_sandboxing::SandboxType;
use nexal_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use nexal_utils_pty::ProcessHandle;
use nexal_utils_pty::SpawnedProcess;
...
impl CommandExecManager {
    pub(crate) async fn start(
        ...
        let spawned = if tty {
            nexal_utils_pty::spawn_pty_process(
                program,
                args,
                cwd.as_path(),
                &env,
            ...
            nexal_utils_pty::spawn_pipe_process(program, args, cwd.as_path(), &env, &arg0).await
        } else {
            nexal_utils_pty::spawn_pipe_process_no_stdin(program, args, cwd.as_path(), &env, &arg0)
                .await
        };
...
mod tests {
    ...
    async fn cancellation_expiration_keeps_process_alive_until_terminated() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: nexal_app_server_protocol::RequestId::Integer(100),
        };
    ...
    async fn dropped_control_request_is_reported_as_not_running() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: nexal_app_server_protocol::RequestId::Integer(3),
        };
