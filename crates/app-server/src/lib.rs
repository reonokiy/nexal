...
use nexal_app_server_protocol::TextPosition as AppTextPosition;
use nexal_app_server_protocol::TextRange as AppTextRange;
use nexal_core::ExecPolicyError;
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        if let ConfigLayerSource::Project { dot_nexal_folder } = &layer.name {
...
pub async fn run_main_with_transport(
        ...
        .await
    {
        Ok(config) => {
            ...
            match effective_toml.try_into() {
                Ok(config_toml) => {
                    if let Err(err) = nexal_core::personality_migration::maybe_migrate_personality(
                        &config.nexal_home,
                        &config_toml,
                    )
                    .await
    ...
    if let Some(warning) = nexal_core::config::system_bwrap_warning() {
        config_warnings.push(ConfigWarningNotification {
            summary: warning,
            details: None,
            path: None,
