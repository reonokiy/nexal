...
const KEYRING_SERVICE: &str = "nexal";
...
impl SecretsManager {
    pub fn new(nexal_home: PathBuf, backend_kind: SecretsBackendKind) -> Self {
        let backend: Arc<dyn SecretsBackend> = match backend_kind {
            SecretsBackendKind::Local => {
                let keyring_store: Arc<dyn KeyringStore> = Arc::new(DefaultKeyringStore);
                Arc::new(LocalSecretsBackend::new(nexal_home, keyring_store))
            }
        };
        Self { backend }
    }
...
pub(crate) fn compute_keyring_account(nexal_home: &Path) -> String {
