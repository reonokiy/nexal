...
mod nexal_tool_config;
mod nexal_tool_runner;
mod exec_approval;
...
pub use crate::nexal_tool_config::NexalToolCallParam;
pub use crate::nexal_tool_config::NexalToolCallReplyParam;
...
const OTEL_SERVICE_NAME: &str = "nexal_mcp_server";
...
mod tests {
    ...
    use nexal_core::config::types::OtelExporterKind;
    ...
    async fn mcp_server_builds_otel_provider_with_logs_traces_and_metrics() -> anyhow::Result<()> {
        ...
        let provider = nexal_core::otel_init::build_provider(
            &config,
            "0.0.0-test",
            Some(OTEL_SERVICE_NAME),
            DEFAULT_ANALYTICS_ENABLED,
