//! Asynchronous worker that executes a **Nexal** tool-call inside a spawned
...
use nexal_core::config::Config as NexalConfig;
...
/// To adhere to MCP `tools/call` response format, include the Nexal
...
/// Run a complete Nexal session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_nexal_tool_session(
    id: RequestId,
    ...
    let NewThread {
        ...
        Err(e) => {
            let result = CallToolResult {
                content: vec![Content::text(format!("Failed to start Nexal session: {e}"))],
                is_error: Some(true),
    ...
    // Use the original MCP request ID as the `sub_id` for the Nexal submission so that
    ...
    if let Err(e) = thread.submit_with_id(submission).await {
        ...
        running_requests_id_to_nexal_uuid.lock().await.remove(&id);
        return;
    }

    run_nexal_tool_session_inner(
        thread_id,
        thread,
        outgoing,
        id,
...
pub async fn run_nexal_tool_session_reply(
    thread_id: ThreadId,
    thread: Arc<NexalThread>,
    outgoing: Arc<OutgoingMessageSender>,
...
async fn run_nexal_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<NexalThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
            ...
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(nexal_event_to_notification(&event)) above has
