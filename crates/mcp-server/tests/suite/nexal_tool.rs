...
use nexal_core::spawn::NEXAL_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use nexal_mcp_server::NexalToolCallParam;
use nexal_mcp_server::ExecApprovalElicitRequestParams;
use nexal_mcp_server::ExecApprovalResponse;
use nexal_mcp_server::PatchApprovalElicitRequestParams;
use nexal_mcp_server::PatchApprovalResponse;
use nexal_protocol::protocol::FileChange;
use nexal_protocol::protocol::ReviewDecision;
use nexal_shell_command::parse_command;
...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(NEXAL_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Nexal sandbox."
        );
        return;
    }
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "nexal" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let nexal_request_id = mcp_process
        .send_nexal_tool_call(NexalToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            nexal_request_id.to_string(),
            params.nexal_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `nexal` tool call completes and that the file was created.
    let nexal_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(nexal_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(nexal_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    nexal_mcp_tool_call_id: String,
    nexal_event_id: String,
    thread_id: nexal_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow Nexal to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let nexal_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        nexal_elicitation: "exec-approval".to_string(),
        nexal_mcp_tool_call_id,
        nexal_event_id,
        nexal_command: command,
        nexal_cwd: workdir.to_path_buf(),
        nexal_call_id: "call1234".to_string(),
        nexal_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "nexal" tool request that will trigger the apply_patch command
    let nexal_request_id = mcp_process
        .send_nexal_tool_call(NexalToolCallParam {
    ...
    // Verify the original `nexal` tool call completes
    let nexal_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(nexal_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_nexal_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = nexal_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn nexal_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `nexal mcp` with a specific config.toml.
    let nexal_home = TempDir::new()?;
    create_config_toml(nexal_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(nexal_home.path()).await?;
    ...
    .await??;
    assert_eq!(nexal_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(nexal_response.id, RequestId::Number(nexal_request_id));
    assert_eq!(
        nexal_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": nexal_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("nexal tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
...
/// Create a Nexal config that uses the mock server as the model provider.
/// It also uses `approval_policy = "untrusted"` so that we exercise the
/// elicitation code path for shell commands.
fn create_config_toml(nexal_home: &Path, server_uri: &str) -> std::io::Result<()> {
