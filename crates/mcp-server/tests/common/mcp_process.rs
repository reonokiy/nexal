...
impl McpProcess {
    pub async fn new(nexal_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(nexal_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = nexal_utils_cargo_bin::cargo_bin("nexal-mcp-server")
            .context("should find binary for nexal-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = nexal_core::default_client::originator().value;
    ...
    pub async fn send_nexal_tool_call(
        &mut self,
        params: NexalToolCallParam,
    ) -> anyhow::Result<i64> {
        let nexal_tool_call_params = CallToolRequestParams {
            meta: None,
            name: "nexal".into(),
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(nexal_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "nexal/event" with params.msg.type == "task_complete".
        ...
        loop {
            ...
            match message {
                JsonRpcMessage::Notification(notification) => {
                    let is_match = if notification.notification.method == "nexal/event" {
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `nexal-mcp-server` child process.
        //
